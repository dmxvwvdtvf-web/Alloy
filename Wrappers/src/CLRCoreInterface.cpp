...
array<BasicResult ^> ^ RIterator::Next(int p_batch) {
    ...
    if (m_Instance == nullptr) return res;
    ...
    for (int i = 0; i < results->GetResultNum(); i++)
        res[i] =
            gcnew BasicResult(new SPTAG::BasicResult(*(results->GetResult(i))));
...
void RIterator::Close() { (*m_Instance)->Close(); }

AnnIndex::AnnIndex(std::shared_ptr<SPTAG::VectorIndex> p_index)
    : ManagedObject(p_index) {
    m_dimension = p_index->GetFeatureDim();
    m_inputVectorSize =
        SPTAG::GetValueTypeSize(p_index->GetVectorValueType()) * m_dimension;
}

AnnIndex::AnnIndex(String ^ p_algoType, String ^ p_valueType, int p_dimension)
    : ManagedObject(SPTAG::VectorIndex::CreateInstance(
          string_to<SPTAG::IndexAlgoType>(p_algoType),
          string_to<SPTAG::VectorValueType>(p_valueType))) {
    m_dimension = p_dimension;
    m_inputVectorSize =
        SPTAG::GetValueTypeSize((*m_Instance)->GetVectorValueType()) *
        m_dimension;
}
...
void AnnIndex::SetBuildParam(String ^ p_name, String ^ p_value,
    ...
    if (m_Instance != nullptr)
        (*m_Instance)
            ->SetParameter(string_to_char_array(p_name),
                           string_to_char_array(p_value),
...
bool AnnIndex::LoadQuantizer(String ^ p_quantizerFile) {
    ...
    auto ret =
        ((*m_Instance)->LoadQuantizer(string_to_char_array(p_quantizerFile)) ==
...
void AnnIndex::SetQuantizerADC(bool p_adc) {
    if (m_Instance != nullptr) (*m_Instance)->SetQuantizerADC(p_adc);
}

array<Byte> ^ AnnIndex::QuantizeVector(array<Byte> ^ p_data, int p_num) {
    array<Byte> ^ res;
    if (m_Instance != nullptr && (*m_Instance)->GetQuantizer() != nullptr) {
        res = gcnew array<Byte>(
            (*m_Instance)->GetQuantizer()->GetNumSubvectors() * (size_t)p_num);
        pin_ptr<Byte> ptr = &res[0], optr = &p_data[0];
        (*m_Instance)
            ->QuantizeVector(optr, p_num,
                             SPTAG::ByteArray(ptr, res->LongLength, false));
    }
    return res;
}
...
array<Byte> ^ AnnIndex::ReconstructVector(array<Byte> ^ p_data, int p_num) {
    ...
    if (m_Instance != nullptr && (*m_Instance)->GetQuantizer() != nullptr) {
        res = gcnew array<Byte>(
            (*m_Instance)->GetQuantizer()->ReconstructSize() * (size_t)p_num);
        pin_ptr<Byte> ptr = &res[0], optr = &p_data[0];
        (*m_Instance)
            ->ReconstructVector(optr, p_num,
                                SPTAG::ByteArray(ptr, res->LongLength, false));
    }
    return res;
}
...
bool AnnIndex::BuildSPANN(bool p_normalized) {
    if (m_Instance == nullptr ||
        (*m_Instance)->GetIndexAlgoType() != SPTAG::IndexAlgoType::SPANN)
        return false;

    return (*m_Instance)->BuildIndex(p_normalized) == SPTAG::ErrorCode::Success;
}

bool AnnIndex::BuildSPANNWithMetaData(array<Byte> ^ p_meta, int p_num,
    ...
    if (!SPTAG::MetadataSet::GetMetadataOffsets(metaptr, p_meta->LongLength,
                                                offsets, p_num + 1, '\n'))
        return false;
    (*m_Instance)
        ->SetMetadata(new SPTAG::MemMetadataSet(
            SPTAG::ByteArray(metaptr, p_meta->LongLength, false),
            SPTAG::ByteArray((std::uint8_t*)offsets,
                             (p_num + 1) * sizeof(std::uint64_t), true),
            p_num, (*m_Instance)->m_iDataBlockSize,
            (*m_Instance)->m_iDataCapacity, (*m_Instance)->m_iMetaRecordSize));

    if (p_withMetaIndex) (*m_Instance)->BuildMetaMapping(false);

    return (SPTAG::ErrorCode::Success ==
            (*m_Instance)->BuildIndex(p_normalized));
}
...
bool AnnIndex::Build(array<Byte> ^ p_data, int p_num, bool p_normalized) {
    if (m_Instance == nullptr || p_num == 0 || m_dimension == 0 ||
        p_data->LongLength != p_num * m_inputVectorSize)
        return false;

    pin_ptr<Byte> ptr = &p_data[0];
    return (SPTAG::ErrorCode::Success ==
            (*m_Instance)->BuildIndex(ptr, p_num, m_dimension, p_normalized));
}

bool AnnIndex::BuildWithMetaData(array<Byte> ^ p_data, array<Byte> ^ p_meta,
                                 int p_num, bool p_withMetaIndex) {
...
bool AnnIndex::BuildWithMetaData(array<Byte> ^ p_data, array<Byte> ^ p_meta,
    ...
    auto vectorType = (*m_Instance)->m_pQuantizer
                          ? SPTAG::VectorValueType::UInt8
                          : (*m_Instance)->GetVectorValueType();
    auto vectorSize = (*m_Instance)->m_pQuantizer
                          ? (*m_Instance)->m_pQuantizer->GetNumSubvectors()
                          : m_dimension;
    std::shared_ptr<SPTAG::VectorSet> vectors(new SPTAG::BasicVectorSet(
        SPTAG::ByteArray(dataptr, p_data->LongLength, false), vectorType,
        vectorSize, p_num));
    ...
    std::shared_ptr<SPTAG::MetadataSet> meta(new SPTAG::MemMetadataSet(
        ...
        SPTAG::ByteArray((std::uint8_t*)offsets,
                         (p_num + 1) * sizeof(std::uint64_t), true),
        p_num, (*m_Instance)->m_iDataBlockSize, (*m_Instance)->m_iDataCapacity,
        (*m_Instance)->m_iMetaRecordSize));
    return (SPTAG::ErrorCode::Success ==
            (*m_Instance)
                ->BuildIndex(vectors, meta, p_withMetaIndex, p_normalized));
}
...
array<BasicResult ^> ^
    AnnIndex::SearchWithMetaData(array<Byte> ^ p_data, int p_resultNum) {
    array<BasicResult ^> ^ res;
    if (m_Instance == nullptr) return res;
...
RIterator ^ AnnIndex::GetIterator(array<Byte> ^ p_data) {
    RIterator ^ res;
    if (m_Instance == nullptr || m_dimension == 0 ||
        p_data->LongLength != m_inputVectorSize)
        return res;

    pin_ptr<Byte> ptr = &p_data[0];
    std::shared_ptr<ResultIterator> result_iterator =
        (*m_Instance)->GetIterator(ptr);
...
bool AnnIndex::Save(String ^ p_saveFile) {
    return SPTAG::ErrorCode::Success ==
           (*m_Instance)->SaveIndex(string_to_char_array(p_saveFile));
}

array<array<Byte> ^> ^ AnnIndex::Dump() {
    std::shared_ptr<std::vector<std::uint64_t>> buffersize =
        (*m_Instance)->CalculateBufferSize();
    array<array<Byte> ^> ^ res =
        gcnew array<array<Byte> ^>(buffersize->size() + 1);
    std::vector<SPTAG::ByteArray> indexBlobs;
    for (int i = 1; i < res->Length; i++) {
        res[i] = gcnew array<Byte>(buffersize->at(i - 1));
        pin_ptr<Byte> ptr = &res[i][0];
        indexBlobs.push_back(
            SPTAG::ByteArray((std::uint8_t*)ptr, res[i]->LongLength, false));
    }
    std::string config;
    if (SPTAG::ErrorCode::Success !=
        (*m_Instance)->SaveIndex(config, indexBlobs)) {
...
bool AnnIndex::Add(array<Byte> ^ p_data, int p_num, bool p_normalized) {
    ...
    pin_ptr<Byte> ptr = &p_data[0];
    return (SPTAG::ErrorCode::Success == (*m_Instance)
                                             ->AddIndex(ptr, p_num, m_dimension,
                                                        nullptr, false,
                                                        p_normalized));
}

bool AnnIndex::AddWithMetaData(array<Byte> ^ p_data, array<Byte> ^ p_meta,
                               int p_num, bool p_withMetaIndex) {
...
bool AnnIndex::AddWithMetaData(array<Byte> ^ p_data, array<Byte> ^ p_meta,
    ...
    pin_ptr<Byte> dataptr = &p_data[0];
    std::shared_ptr<SPTAG::VectorSet> vectors(new SPTAG::BasicVectorSet(
        SPTAG::ByteArray(dataptr, p_data->LongLength, false),
        (*m_Instance)->GetVectorValueType(), m_dimension, p_num));
    ...
    return (
        SPTAG::ErrorCode::Success ==
        (*m_Instance)->AddIndex(vectors, meta, p_withMetaIndex, p_normalized));
}
...
bool AnnIndex::Delete(array<Byte> ^ p_data, int p_num) {
    ...
    pin_ptr<Byte> ptr = &p_data[0];
    return (SPTAG::ErrorCode::Success ==
            (*m_Instance)->DeleteIndex(ptr, p_num));
}
...
AnnIndex ^ AnnIndex::Load(String ^ p_loaderFile) {
    ...
    AnnIndex ^ res;
    if (SPTAG::ErrorCode::Success !=
            SPTAG::VectorIndex::LoadIndex(string_to_char_array(p_loaderFile),
                                          vecIndex) ||
        nullptr == vecIndex) {
...
AnnIndex ^ AnnIndex::Load(array<array<Byte> ^> ^ p_index) {
    ...
    for (int i = 1; i < p_index->Length; i++) {
        ...
        p_indexBlobs.push_back(SPTAG::ByteArray((std::uint8_t*)ptr,
                                                p_index[i]->LongLength, false));
    }
    pin_ptr<Byte> configptr = &p_index[0][0];

    std::shared_ptr<SPTAG::VectorIndex> vecIndex;
    if (SPTAG::ErrorCode::Success !=
            SPTAG::VectorIndex::LoadIndex(
                std::string((char*)configptr, p_index[0]->LongLength),
                p_indexBlobs, vecIndex) ||
        nullptr == vecIndex) {
...
AnnIndex ^
    AnnIndex::Merge(String ^ p_indexFilePath1, String ^ p_indexFilePath2) {
...
MultiIndexScan::MultiIndexScan(
    std::shared_ptr<SPTAG::MultiIndexScan> multi_index_scan)
    : ManagedObject(multi_index_scan) {}

MultiIndexScan::MultiIndexScan(array<AnnIndex ^> ^ indice,
                               array<array<Byte> ^> ^ p_data,
                               array<float> ^ weight, int p_resultNum,
    ...
    for (int i = 0; i < p_data->Length; i++) {
        ...
        SPTAG::ByteArray byte_target =
            SPTAG::ByteArray::Alloc(p_data[i]->LongLength);
    ...
    (*m_Instance)
        ->Init(vecIndices, data_array, weight_array, p_resultNum, useTimer,
               termCondVal, searchLimit);
}
...
void MultiIndexScan::Close() { (*m_Instance)->Close(); }
