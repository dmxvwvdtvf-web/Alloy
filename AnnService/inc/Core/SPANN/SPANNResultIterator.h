...
class SPANNResultIterator : public ResultIterator {
   public:
    SPANNResultIterator(const Index<T>* p_spannIndex,
                        const VectorIndex* p_index, const void* p_target,
        ...
        m_headQueryResult =
            std::make_unique<QueryResult>(p_target, p_batch, false);
    }
    ...
    virtual std::shared_ptr<QueryResult> Next(int batch) {
        if (m_queryResult == nullptr) {
            m_queryResult =
                std::make_unique<QueryResult>(m_target, batch, true);
        ...
        m_spannIndex->SearchIndexIterative(*m_headQueryResult, *m_queryResult,
                                           (COMMON::WorkSpace*)GetWorkSpace(),
                                           m_extraWorkspace.get(), batch,
                                           resultCount, m_isFirstResult);
