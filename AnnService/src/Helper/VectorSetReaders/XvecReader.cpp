...
XvecVectorReader::XvecVectorReader(std::shared_ptr<ReaderOptions> p_options)
    ...
    std::srand(clock());
    m_vectorOutput =
        tempFolder + FolderSep + "vectorset.bin." + std::to_string(std::rand());
}
...
ErrorCode XvecVectorReader::LoadFile(const std::string& p_filePaths) {
    ...
    auto fp = f_createIO();
    if (fp == nullptr || !fp->Initialize(m_vectorOutput.c_str(),
    ...
    IOBINARY(fp, WriteBinary, sizeof(m_options->m_dimension),
    ...
    for (std::string file : files) {
        auto ptr = f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(file.c_str(), std::ios::binary | std::ios::in)) {
        ...
        while (true) {
            ...
            if (dim != m_options->m_dimension) {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                             "Xvec file %s has No.%d vector whose dims are not "
                             "as many as expected. Expected: %d, Fact: %d\n",
                             file.c_str(), vectorCount, m_options->m_dimension,
                             dim);
...
std::shared_ptr<VectorSet> XvecVectorReader::GetVectorSet(SizeType start,
                                                          SizeType end) const {
    auto ptr = f_createIO();
