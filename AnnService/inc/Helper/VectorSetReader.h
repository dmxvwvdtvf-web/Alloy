...
class ReaderOptions : public ArgumentsParser {
   public:
    ReaderOptions(VectorValueType p_valueType, DimensionType p_dimension,
                  VectorFileType p_fileType,
                  std::string p_vectorDelimiter = "|",
                  std::uint32_t p_threadNum = 32, bool p_normalized = false);
...
class VectorSetReader {
   public:
    ...
    virtual std::shared_ptr<VectorSet> GetVectorSet(
        SizeType start = 0, SizeType end = -1) const = 0;
