...
DynamicNeighbors::DynamicNeighbors(const int* p_data, const int p_length)
    : c_data(p_data), c_length(p_length) {}
...
int DynamicNeighbors::Size() const { return c_length; }

DynamicNeighborsSet::DynamicNeighborsSet(const char* p_filePath) {
    auto fp = f_createIO();
    if (fp == nullptr ||
        !fp->Initialize(p_filePath, std::ios::binary | std::ios::in)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed open graph file: %s\n",
                     p_filePath);
        throw std::runtime_error("Opening graph file failed");
    }

    if (fp->ReadBinary(sizeof(m_vectorCount), (char*)&m_vectorCount) !=
        sizeof(m_vectorCount)) {
    ...
    if (fp->ReadBinary(m_vectorCount * sizeof(int),
                       (char*)(m_neighborOffset.get() + 1)) !=
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                 "Vector count: %d, Graph size: %zu\n", m_vectorCount,
                 graphSize);

    m_data.reset(new int[graphSize]);
    auto readSize =
        fp->ReadBinary(graphSize * sizeof(int), (char*)(m_data.get()));
...
DynamicNeighbors DynamicNeighborsSet::operator[](const int p_id) const {
    ...
    return DynamicNeighbors(
        m_data.get() + static_cast<uint64_t>(m_neighborOffset[p_id]),
