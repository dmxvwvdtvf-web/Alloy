...
MultiIndexScan::MultiIndexScan(
    std::vector<std::shared_ptr<VectorIndex>> vectorIndices,
    std::vector<void*> p_targets, unsigned int k,
    float (*rankFunction)(std::vector<float>), bool useTimer, int termCondVal,
    int searchLimit) {
    // copy parameters
    ...
    // topK status
    this->pq =
        std::priority_queue<pq_item, std::vector<pq_item>, pq_item_compare>();
...
void MultiIndexScan::Init(
    std::vector<std::shared_ptr<VectorIndex>> vectorIndices,
    std::vector<ByteArray> p_targets, std::vector<float> weight, unsigned int k,
    ...
    for (int i = 0; i < fwdLUTs.size(); i++) {
        ...
        indexIters.push_back(
            vectorIndices[i]->GetIterator(p_data_array[i].Data(), true));
    }
    printf("(%zu, %zu, %zu)\n", fwdLUTs.size(), p_targets.size(),
           indexIters.size());
}

MultiIndexScan::~MultiIndexScan() { Close(); }
...
bool MultiIndexScan::Next(BasicResult& result) {
    ...
    while (!terminate) {
        for (int i = 0; i < numCols; i++) {
            ...
            if (seenSet.find(vid) == seenSet.end()) {
                ...
                for (int j = 0; j < numCols; j++) {
                    if (i != j) {
                        dists[j] = fwdLUTs[j]->GetDistance(
                            indexIters[j]->GetTarget(), vid);
                    }
                }
                ...
                if (pq.size() == k && pq.top().first <= score) {
                    // printf("%f >= largest score in heap %f, drop", score,
                    // pq.top().first);
                    consecutive_drops++;
                } else {
                    consecutive_drops = 0;
                    if (pq.size() == k)
                        pq.pop();  // the largest score is squeezed out by the
                                   // current score
        ...
        // checkout terminate condition and check if we need to start stream out
        // output why we check term cond outside for loop? because we don't want
        // to check that often~
        if (useTimer) {
