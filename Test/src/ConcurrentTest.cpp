...
template <typename T>
void ConcurrentAddSearchSave(SPTAG::IndexAlgoType algo,
    ...
    auto AddThread = [&stop, &vecIndex, &vec, &meta]() {
        ...
        while (!stop) {
            ...
            std::shared_ptr<SPTAG::MetadataSet> metaset(
                new SPTAG::MemMetadataSet(
                    metaarr,
                    SPTAG::ByteArray((std::uint8_t*)offset,
                                     2 * sizeof(std::uint64_t), false),
                    1));
            SPTAG::ErrorCode ret = vecIndex->AddIndex(
                vec->GetVector(i), 1, vec->Dimension(), metaset, true);
            if (SPTAG::ErrorCode::Success != ret)
                std::cerr << "Error AddIndex(" << (int)(ret) << ") for vector "
                          << i << std::endl;
            i = (i + 1) % vec->Count();
    ...
    auto SearchThread = [&stop, &vecIndex, &vec]() {
        while (!stop) {
            SPTAG::QueryResult res(
                vec->GetVector(SPTAG::COMMON::Utils::rand(vec->Count())), 5,
                true);
...
void CTest(SPTAG::IndexAlgoType algo, std::string distCalcMethod) {
    ...
    ConcurrentAddSearchSave<T>(algo, distCalcMethod, vecset, metaset,
                               "testindices");
}

BOOST_AUTO_TEST_SUITE(ConcurrentTest)

BOOST_AUTO_TEST_CASE(BKTTest) { CTest<float>(SPTAG::IndexAlgoType::BKT, "L2"); }

BOOST_AUTO_TEST_CASE(KDTTest) { CTest<float>(SPTAG::IndexAlgoType::KDT, "L2"); }
