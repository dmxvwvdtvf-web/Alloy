...
std::uint8_t* RemoteQuery::Write(std::uint8_t* p_buffer) const {
    ...
    p_buffer =
        SimpleSerialization::SimpleWriteBuffer(MirrorVersion(), p_buffer);
...
std::uint8_t* RemoteSearchResult::Write(std::uint8_t* p_buffer) const {
    ...
    for (const auto& indexRes : m_allIndexResults) {
        p_buffer = SimpleSerialization::SimpleWriteBuffer(indexRes.m_indexName,
                                                          p_buffer);
        ...
        for (const auto& res : indexRes.m_results) {
            p_buffer =
                SimpleSerialization::SimpleWriteBuffer(res.VID, p_buffer);
            p_buffer =
                SimpleSerialization::SimpleWriteBuffer(res.Dist, p_buffer);
        }
...
const std::uint8_t* RemoteSearchResult::Read(const std::uint8_t* p_buffer) {
    ...
    for (auto& indexRes : m_allIndexResults) {
        p_buffer = SimpleSerialization::SimpleReadBuffer(p_buffer,
                                                         indexRes.m_indexName);
        ...
        for (auto& res : indexRes.m_results) {
            ...
            p_buffer =
                SimpleSerialization::SimpleReadBuffer(p_buffer, res.Dist);
        }
        ...
        if (withMeta) {
            for (int i = 0; i < indexRes.m_results.GetResultNum(); ++i) {
                ByteArray meta;
                p_buffer =
                    SimpleSerialization::SimpleReadBuffer(p_buffer, meta);
