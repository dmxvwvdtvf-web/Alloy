    ...
    if (Socket::c_invalidConnectionID != m_connectionID &&
        ...
        auto connectCallback = [callback, this](bool p_connectSucc) {
            if (!p_connectSucc) {
                RemoteSearchResult result;
                result.m_status =
                    RemoteSearchResult::ResultStatus::FailedNetwork;
...
std::string AnnClient::CreateSearchQuery(const ByteArray& p_data,
    ...
    out << " $datatype:"
        << SPTAG::Helper::Convert::ConvertToString(p_valueType);
