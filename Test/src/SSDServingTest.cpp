...
template <typename T>
void GenerateVectors(std::string fileName, SPTAG::SizeType rows,
                     SPTAG::DimensionType dims,
    ...
    if (fileType == SPTAG::VectorFileType::DEFAULT) {
        ...
        for (size_t i = 0; i < rows; i++) {
            ...
            SPTAG::COMMON::Utils::Normalize(tmp.data(), dims,
                                            SPTAG::COMMON::Utils::GetBase<T>());
...
void GenVec(std::string vectorsName, SPTAG::VectorValueType vecType,
            SPTAG::VectorFileType vecFileType, SPTAG::SizeType rows = 1000,
            SPTAG::DimensionType dims = 100) {
    switch (vecType) {
...
std::string CreateBaseConfig(
    SPTAG::VectorValueType p_valueType, SPTAG::DistCalcMethod p_distCalcMethod,
    SPTAG::IndexAlgoType p_indexAlgoType, SPTAG::DimensionType p_dim,
    std::string p_vectorPath, SPTAG::VectorFileType p_vectorType,
    SPTAG::SizeType p_vectorSize, std::string p_vectorDelimiter,
    std::string p_queryPath, SPTAG::VectorFileType p_queryType,
    SPTAG::SizeType p_querySize, std::string p_queryDelimiter,
    std::string p_warmupPath, SPTAG::VectorFileType p_warmupType,
    SPTAG::SizeType p_warmupSize, std::string p_warmupDelimiter,
    std::string p_truthPath, SPTAG::TruthFileType p_truthType,
    bool p_generateTruth, std::string p_headIDFile,
    std::string p_headVectorsFile, std::string p_headIndexFolder,
    std::string p_ssdIndex) {
    std::ostringstream config;
    config << "[Base]" << std::endl;
    config << "ValueType="
           << SPTAG::Helper::Convert::ConvertToString(p_valueType) << std::endl;
    config << "DistCalcMethod="
           << SPTAG::Helper::Convert::ConvertToString(p_distCalcMethod)
           << std::endl;
    config << "IndexAlgoType="
           << SPTAG::Helper::Convert::ConvertToString(p_indexAlgoType)
           << std::endl;
    config << "Dim=" << p_dim << std::endl;
    config << "VectorPath=" << p_vectorPath << std::endl;
    config << "VectorType="
           << SPTAG::Helper::Convert::ConvertToString(p_vectorType)
           << std::endl;
    ...
    config << "QueryType="
           << SPTAG::Helper::Convert::ConvertToString(p_queryType) << std::endl;
    ...
    config << "WarmupType="
           << SPTAG::Helper::Convert::ConvertToString(p_warmupType)
           << std::endl;
    ...
    config << "TruthType="
           << SPTAG::Helper::Convert::ConvertToString(p_truthType) << std::endl;
    config << "GenerateTruth="
           << SPTAG::Helper::Convert::ConvertToString(p_generateTruth)
           << std::endl;
...
void TestHead(std::string configName, std::string OutputIDFile,
              std::string OutputVectorFile, std::string baseConfig) {
...
void TestBuildHead(std::string configName, std::string p_headVectorFile,
                   std::string p_headIndexFile,
                   SPTAG::IndexAlgoType p_indexAlgoType,
                   std::string p_builderFile, std::string baseConfig) {
    std::ofstream config(configName);
    if (!config.is_open()) {
        fprintf(stderr, "%s can't be opened.\n", configName.c_str());
        BOOST_CHECK(false);
...
void TestBuildSSDIndex(std::string configName, std::string p_vectorIDTranslate,
                       std::string p_headIndexFolder, std::string p_headConfig,
                       std::string p_ssdIndex, bool p_outputEmptyReplicaID,
...
void TestSearchSSDIndex(std::string configName, std::string p_vectorIDTranslate,
                        std::string p_headIndexFolder, std::string p_headConfig,
                        std::string p_ssdIndex, std::string p_searchResult,
...
void RunFromMap() {
    ...
    SPTAG::SSDServing::BootProgram(
        true, &myMap, nullptr, SPTAG::VectorValueType::Int8,
        SPTAG::DistCalcMethod::L2, dataFilePath.c_str(), indexFilePath.c_str());
    ...
    TestSearchSSDIndex("run_from_map_search_config.ini", "", "", "", "", "", "",
                       config.str());
}
...
#define TRUTHSET(VT, DM, FT, TFT) \
    SSDTEST_DIRECTORY "vectors_" #VT "_" #DM "_" #FT "_" #TFT ".truth"
#define HEAD_IDS(VT, DM, FT) "head_ids_" #VT "_" #DM "_" #FT ".bin"
#define HEAD_VECTORS(VT, DM, FT) "head_vectors_" #VT "_" #DM "_" #FT ".bin"
#define HEAD_INDEX(VT, DM, ALGO, FT) \
    "head_" #VT "_" #DM "_" #ALGO "_" #FT ".head_index"
#define SSD_INDEX(VT, DM, ALGO, FT) \
    "ssd_" #VT "_" #DM "_" #ALGO "_" #FT ".ssd_index"

#define SELECT_HEAD_CONFIG(VT, DM, FT) \
    SSDTEST_DIRECTORY "test_head_" #VT "_" #DM "_" #FT ".ini"
#define BUILD_HEAD_CONFIG(VT, DM, ALGO) \
    SSDTEST_DIRECTORY "test_build_head_" #VT "_" #DM "_" #ALGO ".ini"
#define BUILD_HEAD_BUILDER_CONFIG(VT, DM, ALGO) \
    SSDTEST_DIRECTORY "test_build_head_" #VT "_" #DM "_" #ALGO ".builder.ini"
#define BUILD_SSD_CONFIG(VT, DM, ALGO) \
    SSDTEST_DIRECTORY "test_build_ssd" #VT "_" #DM "_" #ALGO ".ini"
#define BUILD_SSD_BUILDER_CONFIG(VT, DM, ALGO) \
    SSDTEST_DIRECTORY "test_build_ssd" #VT "_" #DM "_" #ALGO ".builder.ini"
#define SEARCH_SSD_CONFIG(VT, DM, ALGO) \
    SSDTEST_DIRECTORY "test_search_ssd_" #VT "_" #DM "_" #ALGO ".ini"
...
#define WTEV(VT, DM, FT)                                                   \
    BOOST_AUTO_TEST_CASE(TestHead##VT##DM##FT) {                           \
        ...
        std::string base_config = CreateBaseConfig(                        \
            ...
            SPTAG::IndexAlgoType::Undefined, VECTOR_DIM, VECTORS(VT, FT),  \
            SPTAG::VectorFileType::FT, VECTOR_NUM, "", "",                 \
            SPTAG::VectorFileType::Undefined, -1, "", "",                  \
            SPTAG::VectorFileType::Undefined, -1, "", "",                  \
            SPTAG::TruthFileType::Undefined, false, OutputIDFile,          \
            OutputVectorFile, "", "");                                     \
...
#define BDHD(VT, DM, ALGO, FT)                                               \
    BOOST_AUTO_TEST_CASE(TestBuildHead##VT##DM##ALGO##FT) {                  \
        ...
        std::string base_config = CreateBaseConfig(                          \
            SPTAG::VectorValueType::VT, SPTAG::DistCalcMethod::DM,           \
            SPTAG::IndexAlgoType::ALGO, VECTOR_DIM, VECTORS(VT, FT),         \
            SPTAG::VectorFileType::FT, VECTOR_NUM, "", "",                   \
            SPTAG::VectorFileType::Undefined, -1, "", "",                    \
            SPTAG::VectorFileType::Undefined, -1, "", "",                    \
            SPTAG::TruthFileType::Undefined, false, "",                      \
            HEAD_VECTORS(VT, DM, FT), HEAD_INDEX(VT, DM, ALGO, FT), "");     \
        TestBuildHead(configName, HEAD_VECTORS(VT, DM, FT),                  \
                      HEAD_INDEX(VT, DM, ALGO, FT),                          \
                      SPTAG::IndexAlgoType::ALGO, builderFile, base_config); \
    }
...
#define BDSSD(VT, DM, ALGO, FT)                                               \
    BOOST_AUTO_TEST_CASE(TestBuildSSDIndex##VT##DM##ALGO##FT) {               \
        ...
        std::string base_config = CreateBaseConfig(                           \
            ...
            SPTAG::VectorFileType::Undefined, -1, "", "",                     \
            SPTAG::VectorFileType::Undefined, -1, "", "",                     \
            SPTAG::TruthFileType::Undefined, false, HEAD_IDS(VT, DM, FT), "", \
            HEAD_INDEX(VT, DM, ALGO, FT), SSD_INDEX(VT, DM, ALGO, FT));       \
        TestBuildSSDIndex(configName, HEAD_IDS(VT, DM, FT),                   \
                          HEAD_INDEX(VT, DM, ALGO, FT),                       \
                          BUILD_SSD_BUILDER_CONFIG(VT, DM, ALGO),             \
                          SSD_INDEX(VT, DM, ALGO, FT), true, base_config);    \
    }
...
#define SCSSD(VT, DM, ALGO, FT, TFT)                                        \
    BOOST_AUTO_TEST_CASE(TestSearchSSDIndex##VT##DM##ALGO##FT##TFT) {       \
        ...
        std::string base_config = CreateBaseConfig(                         \
            SPTAG::VectorValueType::VT, SPTAG::DistCalcMethod::DM,          \
            SPTAG::IndexAlgoType::ALGO, VECTOR_DIM, VECTORS(VT, FT),        \
            SPTAG::VectorFileType::FT, VECTOR_NUM, "", QUERIES(VT, FT),     \
            SPTAG::VectorFileType::FT, QUERY_NUM, "", QUERIES(VT, FT),      \
            SPTAG::VectorFileType::FT, QUERY_NUM, "",                       \
            TRUTHSET(VT, DM, FT, TFT), SPTAG::TruthFileType::TFT, true,     \
            HEAD_IDS(VT, DM, FT), "", HEAD_INDEX(VT, DM, ALGO, FT),         \
            SSD_INDEX(VT, DM, ALGO, FT));                                   \
        TestSearchSSDIndex(                                                 \
            configName, HEAD_IDS(VT, DM, FT), HEAD_INDEX(VT, DM, ALGO, FT), \
...
BOOST_AUTO_TEST_CASE(RUN_FROM_MAP) { RunFromMap(); }
