...
namespace SPTAG {
namespace COMMON {
std::shared_ptr<IQuantizer> IQuantizer::LoadIQuantizer(
    std::shared_ptr<Helper::DiskIO> p_in) {
    ...
    if (p_in->ReadBinary(sizeof(QuantizerType), (char*)&quantizerType) !=
        sizeof(QuantizerType))
        return ret;
    if (p_in->ReadBinary(sizeof(VectorValueType), (char*)&reconstructType) !=
        sizeof(VectorValueType))
        return ret;
    SPTAGLIB_LOG(
        Helper::LogLevel::LL_Info,
        "Loading quantizer of type %s with reconstructtype %s.\n",
        Helper::Convert::ConvertToString<QuantizerType>(quantizerType).c_str(),
        Helper::Convert::ConvertToString<VectorValueType>(reconstructType)
            .c_str());
...
IQuantizer::DistanceCalcSelector<std::uint8_t>(
    ...
    if (p_method == SPTAG::DistCalcMethod::L2) {
        ...
        return (
            [this](const std::uint8_t* pX, const std::uint8_t* pY,
...
IQuantizer::DistanceCalcSelector(SPTAG::DistCalcMethod p_method) const {
    return SPTAG::COMMON::DistanceCalcSelector<T>(p_method);
}

#define DefineVectorValueType(Name, Type)                                  \
    template std::function<float(const Type*, const Type*, SizeType)>      \
    IQuantizer::DistanceCalcSelector<Type>(SPTAG::DistCalcMethod p_method) \
        const;
#include "inc/Core/DefinitionList.h"
#undef DefineVectorValueType
}  // namespace COMMON
