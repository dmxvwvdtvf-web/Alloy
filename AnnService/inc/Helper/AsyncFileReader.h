...
void SetThreadAffinity(int threadID, std::thread& thread,
                       NumaStrategy socketStrategy = NumaStrategy::LOCAL,
...
class RequestQueue {
   public:
    ...
    bool pop(AsyncReadRequest*& j) {
        ...
        ULONG_PTR key;
        OVERLAPPED* ol;
        BOOL ret = ::GetQueuedCompletionStatus(m_handle.GetHandle(), &cBytes,
                                               &key, &ol, INFINITE);
    ...
    void* handle() { return m_handle.GetHandle(); }
...
class AsyncFileIO : public DiskIO {
   public:
    ...
    virtual bool Initialize(const char* filePath, int openMode,
        ...
        SPTAGLIB_LOG(LogLevel::LL_Info,
                     "Success open file handle: %s DiskSectorSize: %u\n",
                     filePath, m_diskSectorSize);

        PreAllocQueryContext();

        int iocpThreads = threadPoolSize;
        m_fileIocp.Reset(::CreateIoCompletionPort(m_fileHandle.GetHandle(),
                                                  NULL, NULL, iocpThreads));
    ...
    virtual void Wait(AsyncReadRequest& readRequest) {
        // currently not used anywhere, effective only when ASYNC_READ is
        // defined and BATCH_READ is not defined
    ...
    virtual bool BatchReadFile(AsyncReadRequest* readRequests,
        ...
        while (remaining > 0) {
            ...
            ULONG_PTR key;
            OVERLAPPED* ol;
            BOOL ret = ::GetQueuedCompletionStatus(completeQueue, &cBytes, &key,
                                                   &ol, INFINITE);
   ...
   private:
            ...
            if (lpBuffer) {
                ...
                if (dwResult > 0 && dwResult <= dwSize) {
                    ...
                    if (nameValid) {
                        ...
                        GetDiskFreeSpace(lpBuffer, &dwSPC, &dwSectorSize,
                                         &dwNOFC, &dwTNOC);
                    }
                }
                delete[] lpBuffer;
    ...
    void ListionIOCP(int i) {
        SetThreadAffinity(i, m_fileIocpThreads[i], NumaStrategy::SCATTER,
                          OrderStrategy::DESC);  // avoid IO threads overlap
                                                 // with search threads
        ...
        while (true) {
            BOOL ret = ::GetQueuedCompletionStatus(
                this->m_fileIocp.GetHandle(), &cBytes, &key, &ol, INFINITE);
            ...
            if (nullptr != req) {
                ::PostQueuedCompletionStatus(
                    req->m_extension, 0, NULL,
                    reinterpret_cast<LPOVERLAPPED>(req));
            }
        }
    }
    ...
    virtual bool Initialize(const char* filePath, int openMode,
        ...
        if (m_fileHandle <= 0) {
            SPTAGLIB_LOG(LogLevel::LL_Error,
                         "Failed to create file handle: %s\n", filePath);
            return false;
        }
    ...
    virtual bool ReadFileAsync(AsyncReadRequest& readRequest) {
        ...
        while (
            ...
            syscall(__NR_io_submit,
                    m_iocps[(readRequest.m_status & 0xffff) % m_iocps.size()],
                    1, iocbs) < 1) {
