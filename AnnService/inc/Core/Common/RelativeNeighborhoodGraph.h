...
class RelativeNeighborhoodGraph : public NeighborhoodGraph {
   public:
    ...
    void RebuildNeighbors(VectorIndex* index, const SizeType node,
        ...
        for (int j = 0; j < numResults && count < m_iNeighborhoodSize; j++) {
            ...
            for (DimensionType k = 0; k < count; k++) {
                if (m_fRNGFactor *
                        index->ComputeDistance(index->GetSample(nodes[k]),
                                               index->GetSample(item.VID)) <
                    item.Dist) {
                    good = false;
                    break;
                }
    ...
    void InsertNeighbors(VectorIndex* index, const SizeType node,
                         SizeType insertNode, float insertDist) {
        ...
        for (DimensionType i = 0; i < m_iNeighborhoodSize; i++) {
            ...
            _mm_prefetch((const char*)(index->GetSample(futureNode)),
                         _MM_HINT_T0);
        }
        ...
        float tmpDist;
        const void* tmpVec;
        int checkNeighborhoodSize = (nodes[m_iNeighborhoodSize - 1] < -1)
        ...
        for (DimensionType k = 0; k < checkNeighborhoodSize; k++) {
            ...
            if (tmpDist > insertDist ||
                (insertDist == tmpDist && insertNode < tmpNode)) {
                nodes[k] = insertNode;
                while (++k < checkNeighborhoodSize &&
                       index->ComputeDistance(tmpVec, nodeVec) <=
                           index->ComputeDistance(tmpVec, insertVec)) {
