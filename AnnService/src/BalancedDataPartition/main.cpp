...
class PartitionOptions : public Helper::ReaderOptions {
   public:
    PartitionOptions()
        : Helper::ReaderOptions(VectorValueType::Float, 0, VectorFileType::TXT,
                                "|", 32) {
        AddRequiredOption(m_inputFiles, "-i", "--input", "Input raw data.");
        AddRequiredOption(m_clusterNum, "-c", "--numclusters",
                          "Number of clusters.");
        AddOptionalOption(m_stopDifference, "-df", "--diff",
                          "Clustering stop center difference.");
        AddOptionalOption(m_maxIter, "-r", "--iters",
                          "Max clustering iterations.");
        AddOptionalOption(m_localSamples, "-s", "--samples",
                          "Number of samples for fast clustering.");
        AddOptionalOption(m_lambda, "-l", "--lambda",
                          "lambda for balanced size level.");
        AddOptionalOption(m_distMethod, "-m", "--dist",
                          "Distance method (L2 or Cosine).");
        AddOptionalOption(m_outdir, "-o", "--outdir", "Output directory.");
        AddOptionalOption(m_weightfile, "-w", "--weight",
                          "vector weight file.");
        AddOptionalOption(m_wlambda, "-lw", "--wlambda",
                          "lambda for balanced weight level.");
        AddOptionalOption(m_seed, "-e", "--seed", "Random seed.");
        AddOptionalOption(m_initIter, "-x", "--init",
                          "Number of iterations for initialization.");
        AddOptionalOption(m_clusterassign, "-a", "--assign",
                          "Number of clusters to be assigned.");
        AddOptionalOption(m_vectorfactor, "-vf", "--vectorscale",
                          "Max vector number scale factor.");
        AddOptionalOption(m_closurefactor, "-cf", "--closurescale",
                          "Max closure factor");
        AddOptionalOption(m_stage, "-g", "--stage",
                          "Running function (Clustering or LocalPartition)");
        AddOptionalOption(m_centers, "-ct", "--centers",
                          "File path to store centers.");
        AddOptionalOption(m_labels, "-lb", "--labels",
                          "File path to store labels.");
        AddOptionalOption(m_status, "-st", "--status",
                          "Cosmos path to store intermediate centers.");
        AddOptionalOption(m_totalparts, "-tp", "--parts", "Total partitions.");
        AddOptionalOption(m_syncscript, "-ss", "--script", "Run sync script.");
        AddOptionalOption(m_recoveriter, "-ri", "--recover",
                          "Recover iteration.");
        AddOptionalOption(m_newp, "-np", "--newpenalty",
                          "old penalty: 0, new penalty: 1");
...
bool LoadCenters(T* centers, SizeType row, DimensionType col,
                 const std::string& centerpath, float* lambda = nullptr,
                 float* diff = nullptr, float* mindist = nullptr,
                 int* noimprovement = nullptr) {
    if (fileexists(centerpath.c_str())) {
        auto ptr = f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(centerpath.c_str(),
        ...
        if (ptr->ReadBinary(sizeof(SizeType), (char*)&r) != sizeof(SizeType))
            return false;
        if (ptr->ReadBinary(sizeof(DimensionType), (char*)&c) !=
        ...
        if (ptr->ReadBinary(sizeof(T) * row * col, (char*)centers) !=
            sizeof(T) * row * col)
            return false;
...
template <typename T>
void SaveCenters(T* centers, SizeType row, DimensionType col,
                 const std::string& centerpath, float lambda = 0.0,
                 float diff = 0.0, float mindist = 0.0, int noimprovement = 0) {
    auto ptr = f_createIO();
    if (ptr == nullptr || !ptr->Initialize(centerpath.c_str(),
                                           std::ios::binary | std::ios::out)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Failed to open center file %s to write.\n",
                     centerpath.c_str());
        exit(1);
    }
...
inline float MultipleClustersAssign(
    const COMMON::Dataset<T>& data, std::vector<SizeType>& indices,
    ...
    auto func = [&](int tid) {
        ...
        float idist = 0;
        std::vector<SPTAG::NodeDistPair> centerDist(args._K,
                                                    SPTAG::NodeDistPair());
        for (SizeType i = istart; i < iend; i++) {
            for (int k = 0; k < args._K; k++) {
                float penalty = lambda * (((options.m_newp == 1) &&
                                           (args.counts[k] < avgCount))
                                              ? avgCount
                                              : args.counts[k]) +
                                wlambda * args.weightedCounts[k];
                float dist =
                    args.fComputeDistance(data[indices[i]],
            ...
            std::sort(
                centerDist.begin(), centerDist.end(),
            ...
            for (int k = 0; k < label.C(); k++) {
                if (centerDist[k].distance <=
                    centerDist[0].distance * options.m_closurefactor) {
    ...
    for (int i = 1; i < args._T; i++) {
        for (int k = 0; k < args._K; k++) {
            ...
            args.newWeightedCounts[k] +=
                args.newWeightedCounts[i * args._K + k];
...
inline float HardMultipleClustersAssign(
    ...
    auto func1 = [&](int tid) {
        ...
        for (SizeType i = istart; i < iend; i++) {
            for (int k = 0; k < args._K; k++) {
                float dist = args.fComputeDistance(
                    data[indices[i]], args.centers + k * args._D, args._D);
                centerDist[k].node = k;
                centerDist[k].distance = dist;
            }
            std::sort(
    ...
    for (int i = 0; i < args._K; ++i) {
        std::size_t endIdx = std::lower_bound(items, items + last - first,
                                              i + 1, g_edgeComparer) -
                             items;
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "cluster %d: avgdist:%f limit:%d, drop:%zu - %zu\n",
                     items[startIdx].node,
                     args.clusterDist[i] / (endIdx - startIdx), mylimit[i],
                     startIdx + mylimit[i], endIdx);
    ...
    auto func2 = [&, subsize](int tid) {
        ...
        for (SizeType i = istart; i < iend; i++) {
            if (items[i].tonode >= 0) {
                ...
                inewWeightedCounts[items[i].node] +=
                    weights[indices[items[i].tonode]];
                idist += items[i].distance;
            } else {
                items[i].tonode = -items[i].tonode - 1;
                label[items[i].tonode][clusternum] =
                    (std::numeric_limits<LabelType>::max)();
            }

            if (fill) {
                for (int k = clusternum + 1; k < label.C(); k++) {
                    label[items[i].tonode][k] =
                        (std::numeric_limits<LabelType>::max)();
                }
            }
        }
...
void Process(MPI_Datatype type) {
    ...
    auto vectorReader = Helper::VectorSetReader::CreateInstance(
        std::make_shared<Helper::ReaderOptions>(options));
    options.m_inputFiles = Helper::StrUtils::ReplaceAll(
        options.m_inputFiles, "*", std::to_string(rank));
    ...
    if (options.m_weightfile.compare("-") != 0) {
        options.m_weightfile = Helper::StrUtils::ReplaceAll(
            options.m_weightfile, "*", std::to_string(rank));
        ...
        if (rows != vectors->Count()) {
            win.close();
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Number of weights (%d) is not equal to number of "
                         "vectors (%d).\n",
                         rows, vectors->Count());
            exit(1);
        }
    ...
    COMMON::Dataset<T> data(vectors->Count(), vectors->Dimension(), 1024 * 1024,
                            vectors->Count() + 1, (T*)vectors->GetData());
    COMMON::KmeansArgs<T> args(options.m_clusterNum, vectors->Dimension(),
                               vectors->Count(), options.m_threadNum,
                               options.m_distMethod);
    COMMON::Dataset<LabelType> label(vectors->Count(), options.m_clusterassign,
                                     vectors->Count(), vectors->Count());
    ...
    MPI_Allreduce(&localCount, &totalCount, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM,
                  MPI_COMM_WORLD);
    totalCount = static_cast<unsigned long long>(totalCount * 1.0 / args._K *
                                                 options.m_vectorfactor);

    if (rank == 0 && options.m_maxIter > 0 && options.m_lambda < -1e-6f) {
        float fBalanceFactor = COMMON::DynamicFactorSelect<T>(
            data, localindices, 0, data.R(), args, data.R());
        options.m_lambda = COMMON::Utils::GetBase<T>() *
                           COMMON::Utils::GetBase<T>() / fBalanceFactor /
                           data.R();
    }
    MPI_Bcast(&(options.m_lambda), 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "rank %d  data:(%d,%d) machines:%d clusters:%d type:%d "
                 "threads:%d lambda:%f samples:%d maxcountperpartition:%d\n",
                 rank, data.R(), data.C(), size, options.m_clusterNum,
                 ((int)options.m_inputValueType), options.m_threadNum,
                 options.m_lambda, options.m_localSamples, totalCount);

    if (rank == 0) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "rank 0 init centers\n");
        if (!LoadCenters(args.newTCenters, args._K, args._D, options.m_centers,
                         &(options.m_lambda))) {
            if (options.m_seed >= 0) std::srand(options.m_seed);
            COMMON::InitCenters<T, T>(data, localindices, 0, data.R(), args,
                                      options.m_localSamples,
                                      options.m_initIter);
        }
    }
    ...
    int iteration = 0;
    int noImprovement = 0;
    while (currDiff > options.m_stopDifference &&
           iteration < options.m_maxIter) {
        if (rank == 0) {
            std::memcpy(args.centers, args.newTCenters,
                        sizeof(T) * args._K * args._D);
        }
        ...
        d = MultipleClustersAssign<T>(
            data, localindices, 0, data.R(), args, label, true,
            (iteration == 0) ? 0.0f : options.m_lambda, weights,
            (iteration == 0) ? 0.0f : options.m_wlambda);
        MPI_Allreduce(args.newCounts, args.counts, args._K, MPI_INT, MPI_SUM,
                      MPI_COMM_WORLD);
        MPI_Allreduce(args.newWeightedCounts, args.weightedCounts, args._K,
        ...
        if (rank == 0) {
            MPI_Reduce(MPI_IN_PLACE, args.newCenters, args._K * args._D,
            ...
            MPI_Reduce(args.newCenters, args.newCenters, args._K * args._D,
                       MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (options.m_maxIter == 0) {
        ...
        if (rank == 0) {
            for (int i = 0; i < args._K; i++)
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                             "cluster %d contains vectors:%d weights:%f\n", i,
    ...
    std::vector<SizeType> myLimit(
        args._K, (options.m_hardcut == 0)
                     ? data.R()
                     : (SizeType)(options.m_hardcut * totalCount / size));
    std::memset(args.counts, 0, sizeof(SizeType) * args._K);
    args.ClearCounts();
    args.ClearDists(0);
    for (int i = 0; i < options.m_clusterassign - 1; i++) {
        d += HardMultipleClustersAssign<T>(data, localindices, 0, data.R(),
                                           args, label, myLimit.data(), weights,
                                           i, false);
        std::memcpy(myLimit.data(), args.counts, sizeof(SizeType) * args._K);
        MPI_Allreduce(MPI_IN_PLACE, args.counts, args._K, MPI_INT, MPI_SUM,
                      MPI_COMM_WORLD);
        MPI_Allreduce(MPI_IN_PLACE, args.weightedCounts, args._K, MPI_FLOAT,
    ...
    d += HardMultipleClustersAssign<T>(data, localindices, 0, data.R(), args,
                                       label, myLimit.data(), weights,
    ...
    if (label.Save(options.m_labels + "." + std::to_string(rank)) !=
        ErrorCode::Success) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to save labels.\n");
        exit(1);
    }
    if (rank == 0) {
        SaveCenters(args.centers, args._K, args._D, options.m_centers,
                    options.m_lambda);
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "final dist:%f\n", currDist);
        for (int i = 0; i < args._K; i++)
            SPTAGLIB_LOG(Helper::LogLevel::LL_Status,
                         "cluster %d contains vectors:%d weights:%f\n", i,
                         args.counts[i], args.weightedCounts[i]);
    }
    ...
    if (options.m_outdir.compare("-") != 0) {
        for (int i = 0; i < args._K; i++) {
            ...
            noImprovement = 0;
            std::string vecfile = options.m_outdir + "/" + options.m_outfile +
                                  "." + std::to_string(i + 1);
            if (fileexists(vecfile.c_str())) noImprovement = 1;
            MPI_Allreduce(MPI_IN_PLACE, &noImprovement, 1, MPI_INT, MPI_SUM,
                          MPI_COMM_WORLD);
            if (noImprovement) continue;

            if (i % size == rank) {
                std::string vecfile = options.m_outdir + "/" +
                                      options.m_outfile + "." +
                                      std::to_string(i);
                std::string metafile = options.m_outdir + "/" +
                                       options.m_outmetafile + "." +
                                       std::to_string(i);
                std::string metaindexfile = options.m_outdir + "/" +
                                            options.m_outmetaindexfile + "." +
                                            std::to_string(i);
                std::shared_ptr<Helper::DiskIO> out = f_createIO(),
                                                metaout = f_createIO(),
                                                metaindexout = f_createIO();
                if (out == nullptr ||
                    !out->Initialize(vecfile.c_str(),
                                     std::ios::binary | std::ios::out)) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                ...
                if (metaout == nullptr ||
                    !metaout->Initialize(metafile.c_str(),
                                         std::ios::binary | std::ios::out)) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                 "Cannot open %s to write.\n",
                                 metafile.c_str());
                ...
                CHECKIO(out, WriteBinary, sizeof(int),
                        (char*)(&args.counts[i]));
                CHECKIO(out, WriteBinary, sizeof(int), (char*)(&args._D));
                if (metas != nullptr)
                    CHECKIO(metaindexout, WriteBinary, sizeof(int),
                            (char*)(&args.counts[i]));
                ...
                for (int j = 0; j < size; j++) {
                    ...
                    if (j != rank) {
                        ...
                        for (int k = 0; k < recv; k++) {
                            MPI_Recv(recvbuf, args._D, type, j, 1,
                                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                            CHECKIO(out, WriteBinary, sizeof(T) * args._D,
                                    (char*)recvbuf);

                            if (metas != nullptr) {
                                int len;
                                MPI_Recv(&len, 1, MPI_INT, j, 2, MPI_COMM_WORLD,
                                         MPI_STATUS_IGNORE);
                                if (len > recvmetabuflen) {
                                    SPTAGLIB_LOG(
                                        Helper::LogLevel::LL_Info,
                                        "enlarge recv meta buf to %d\n", len);
                                    delete[] recvmetabuf;
                                    recvmetabuflen = len;
                                    recvmetabuf = new char[recvmetabuflen];
                                }
                                MPI_Recv(recvmetabuf, len, MPI_CHAR, j, 3,
                                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                                CHECKIO(metaout, WriteBinary, len, recvmetabuf);
                                CHECKIO(metaindexout, WriteBinary,
                                        sizeof(std::uint64_t),
                                        (char*)(&offset));
                                offset += len;
                            }
                        ...
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Info,
                            "rank %d <- rank %d: %d vectors, %llu bytes meta\n",
                            ...
                            for (int kk = 0; kk < label.C(); kk++) {
                                if (label[k][kk] == (LabelType)i) {
                                    CHECKIO(out, WriteBinary,
                                            sizeof(T) * args._D,
                                            (char*)(data[localindices[k]]));
                                    if (metas != nullptr) {
                                        ByteArray meta =
                                            metas->GetMetadata(localindices[k]);
                                        CHECKIO(metaout, WriteBinary,
                                                meta.Length(),
                        ...
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                                     "rank %d <- rank %d: %d(%d) vectors, %llu "
                                     "bytes meta\n",
                                     rank, j, args.newCounts[i], total_rec,
                                     (offset - offset_before));
                    }
                }
                delete[] recvmetabuf;
                if (metas != nullptr)
                    CHECKIO(metaindexout, WriteBinary, sizeof(std::uint64_t),
                            (char*)(&offset));
                out->ShutDown();
                metaout->ShutDown();
                ...
                int dest = i % size;
                MPI_Send(&args.newCounts[i], 1, MPI_INT, dest, 0,
                         MPI_COMM_WORLD);
                size_t total_len = 0;
                size_t total_rec = 0;
                for (int j = 0; j < data.R(); j++) {
                    for (int kk = 0; kk < label.C(); kk++) {
                        if (label[j][kk] == (LabelType)i) {
                            MPI_Send(data[localindices[j]], args._D, type, dest,
                                     1, MPI_COMM_WORLD);
                            if (metas != nullptr) {
                                ByteArray meta =
                                    metas->GetMetadata(localindices[j]);
                                int len = (int)meta.Length();
                                MPI_Send(&len, 1, MPI_INT, dest, 2,
                                         MPI_COMM_WORLD);
                                MPI_Send(meta.Data(), len, MPI_CHAR, dest, 3,
                                         MPI_COMM_WORLD);
                                total_len += len;
                            }
                            total_rec++;
                ...
                SPTAGLIB_LOG(
                    Helper::LogLevel::LL_Info,
                    "rank %d -> rank %d: %d(%d) vectors, %llu bytes meta\n",
...
template <typename T>
ErrorCode SyncSaveCenter(COMMON::KmeansArgs<T>& args, int rank, int iteration,
                         unsigned long long localCount, float localDist,
                         float lambda, float diff, float mindist,
                         int noimprovement, int savecenters,
                         bool assign = false) {
    if (!direxists(options.m_status.c_str())) mkdir(options.m_status.c_str());
    std::string folder =
        options.m_status + FolderSep + std::to_string(iteration);
    ...
    if (rank == 0 && savecenters > 0) {
        SaveCenters(args.newTCenters, args._K, args._D,
                    folder + FolderSep + "centers.bin", lambda, diff, mindist,
                    noimprovement);
    }

    std::string savePath = folder + FolderSep + "status." +
                           std::to_string(iteration) + "." +
                           std::to_string(rank);
    auto out = f_createIO();
    if (out == nullptr ||
        !out->Initialize(savePath.c_str(), std::ios::binary | std::ios::out)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Cannot open %s to write status.\n", savePath.c_str());
        exit(1);
    }

    CHECKIO(out, WriteBinary, sizeof(unsigned long long),
            (const char*)&localCount);
    CHECKIO(out, WriteBinary, sizeof(float), (const char*)&localDist);
    CHECKIO(out, WriteBinary, sizeof(float) * args._K * args._D,
            (const char*)args.newCenters);
    if (assign) {
        CHECKIO(out, WriteBinary, sizeof(int) * args._K,
                (const char*)args.counts);
        CHECKIO(out, WriteBinary, sizeof(float) * args._K,
                (const char*)args.weightedCounts);
    } else {
        CHECKIO(out, WriteBinary, sizeof(int) * args._K,
                (const char*)args.newCounts);
        CHECKIO(out, WriteBinary, sizeof(float) * args._K,
                (const char*)args.newWeightedCounts);
    }
    out->ShutDown();
    ...
    if (!options.m_syncscript.empty()) {
        try {
            int return_value =
                system((options.m_syncscript + " upload " + folder + " " +
                        std::to_string(options.m_totalparts) + " " +
                        std::to_string(savecenters))
                           .c_str());
            if (return_value != 0)
                throw std::system_error(errno, std::generic_category(),
                                        "error executing command");
        } catch (const std::system_error& e) {
            std::cerr << "error executing command: " << options.m_syncscript
                      << e.what() << '\n';
            return ErrorCode::Fail;
        }
    } else {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Error: Sync script is empty.\n");
    }
    return ErrorCode::Success;
}

template <typename T>
ErrorCode SyncLoadCenter(COMMON::KmeansArgs<T>& args, int rank, int iteration,
                         unsigned long long& totalCount, float& currDist,
                         float& lambda, float& diff, float& mindist,
                         int& noimprovement, bool loadcenters) {
    std::string folder =
        options.m_status + FolderSep + std::to_string(iteration);
    ...
    if (!options.m_syncscript.empty()) {
        try {
            int return_value =
                system((options.m_syncscript + " download " + folder + " " +
                        std::to_string(options.m_totalparts) + " " +
                        std::to_string(loadcenters))
                           .c_str());
            if (return_value != 0)
                throw std::system_error(errno, std::generic_category(),
                                        "error executing command");
    ...
    if (loadcenters) {
        if (!LoadCenters(args.newTCenters, args._K, args._D,
                         folder + FolderSep + "centers.bin", &lambda, &diff,
    ...
    for (int part = 0; part < options.m_totalparts; part++) {
        std::string loadPath = folder + FolderSep + "status." +
                               std::to_string(iteration) + "." +
                               std::to_string(part);
        auto input = f_createIO();
        if (input == nullptr ||
            !input->Initialize(loadPath.c_str(),
                               std::ios::binary | std::ios::in)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Cannot open %s to read status.", loadPath.c_str());
            exit(1);
        }

        CHECKIO(input, ReadBinary, sizeof(unsigned long long),
                (char*)&localCount);
        ...
        CHECKIO(input, ReadBinary, sizeof(float) * args._K * args._D,
                buf.get());
        ...
        for (int i = 0; i < args._K; i++) {
            ...
            if (partsize >= 0 && args.counts[i] <= MaxSize - partsize)
                ...
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                             "Cluster %d counts overflow:%d + %d(%d)! Set it "
                             "to MaxSize.\n",
...
void ProcessWithoutMPI() {
    ...
    if (vectors->Dimension() != options.m_dimension) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "vector dimension %d is not equal to the dimension %d of "
                     "the option.\n",
    ...
    if (iteration < 0) {
        iteration = 0;
        SyncSaveCenter(args, rank, iteration, data.R(), d, options.m_lambda,
                       currDiff, minClusterDist, noImprovement, 2);
    } else {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "recover from iteration:%d\n",
                     iteration);
    }
    SyncLoadCenter(args, rank, iteration, totalCount, currDist,
                   options.m_lambda, currDiff, minClusterDist, noImprovement,
                   true);

    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "rank %d  data:(%d,%d) machines:%d clusters:%d type:%d "
                 "threads:%d lambda:%f samples:%d maxcountperpartition:%d\n",
                 rank, data.R(), data.C(), options.m_totalparts,
                 options.m_clusterNum, ((int)options.m_inputValueType),
                 options.m_threadNum, options.m_lambda, options.m_localSamples,
                 static_cast<unsigned long long>(totalCount * 1.0 / args._K *
                                                 options.m_vectorfactor));

    while (noImprovement < 10 && currDiff > options.m_stopDifference &&
           iteration < options.m_maxIter) {
        std::memcpy(args.centers, args.newTCenters,
                    sizeof(T) * args._K * args._D);
        ...
        SyncSaveCenter(args, rank, iteration + 1, data.R(), d, options.m_lambda,
                       currDiff, minClusterDist, noImprovement, 0);
        if (rank == 0) {
            SyncLoadCenter(args, rank, iteration + 1, totalCount, currDist,
                           options.m_lambda, currDiff, minClusterDist,
                           noImprovement, false);
            currDiff = COMMON::RefineCenters<T, T>(data, args);
            if (currDist < minClusterDist) {
                noImprovement = 0;
            ...
            SyncSaveCenter(args, rank, iteration + 1, data.R(), d,
                           options.m_lambda, currDiff, minClusterDist,
                           noImprovement, 1);
        } else {
            SyncLoadCenter(args, rank, iteration + 1, totalCount, currDist,
    ...
    std::vector<SizeType> myLimit(
        args._K, (options.m_hardcut == 0)
                     ? data.R()
                     : (SizeType)(options.m_hardcut * totalCount /
                                  options.m_totalparts));
    std::memset(args.counts, 0, sizeof(SizeType) * args._K);
    args.ClearCounts();
    args.ClearDists(0);
    for (int i = 0; i < options.m_clusterassign - 1; i++) {
        d += HardMultipleClustersAssign<T>(data, localindices, 0, data.R(),
                                           ...
                                           i, false);
        std::memcpy(myLimit.data(), args.counts, sizeof(SizeType) * args._K);
        SyncSaveCenter(args, rank, 10000 + iteration + 1 + i, data.R(), d,
                       options.m_lambda, currDiff, minClusterDist,
                       noImprovement, 0, true);
        SyncLoadCenter(args, rank, 10000 + iteration + 1 + i, tmpTotalCount,
                       currDist, options.m_lambda, currDiff, minClusterDist,
                       noImprovement, false);
        if (rank == 0) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                         "assign %d....................d:%f\n", i, d);
        ...
        for (int k = 0; k < args._K; k++)
            if (totalCount > args.counts[k])
                myLimit[k] += (SizeType)((totalCount - args.counts[k]) /
                                         options.m_totalparts);
    }
    d += HardMultipleClustersAssign<T>(data, localindices, 0, data.R(), args,
                                       label, myLimit.data(), weights,
                                       options.m_clusterassign - 1, true);
    std::memcpy(args.newCounts, args.counts, sizeof(SizeType) * args._K);
    SyncSaveCenter(args, rank, 10000 + iteration + options.m_clusterassign,
                   data.R(), d, options.m_lambda, currDiff, minClusterDist,
                   noImprovement, 0, true);
    SyncLoadCenter(args, rank, 10000 + iteration + options.m_clusterassign,
                   tmpTotalCount, currDist, options.m_lambda, currDiff,
...
void Partition() {
    ...
    if (label.Load(options.m_labels, vectors->Count(), vectors->Count()) !=
        ErrorCode::Success) {
    ...
    for (int i = 0; i < options.m_clusterNum; i++) {
        ...
        std::string metafile = options.m_outdir + "/" + options.m_outmetafile +
                               "." + taskId + "." + std::to_string(i);
        std::string metaindexfile = options.m_outdir + "/" +
                                    options.m_outmetaindexfile + "." + taskId +
                                    "." + std::to_string(i);
        std::shared_ptr<Helper::DiskIO> out = f_createIO(),
                                        metaout = f_createIO(),
                                        metaindexout = f_createIO();
        ...
        if (metaindexout == nullptr ||
            !metaindexout->Initialize(metaindexfile.c_str(),
                                      std::ios::binary | std::ios::out)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Cannot open %s to write.\n", metaindexfile.c_str());
            exit(1);
        ...
        for (int k = 0; k < data.R(); k++) {
            for (int kk = 0; kk < label.C(); kk++) {
                if (label[k][kk] == (LabelType)i) {
                    CHECKIO(out, WriteBinary, sizeof(T) * cols,
                            (char*)(data[k]));
                    if (metas != nullptr) {
                        ByteArray meta = metas->GetMetadata(k);
                        CHECKIO(metaout, WriteBinary, meta.Length(),
                                (const char*)meta.Data());
                        CHECKIO(metaindexout, WriteBinary,
                                sizeof(std::uint64_t), (char*)(&offset));
                        offset += meta.Length();
                    }
                    records++;
                }
        ...
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "part %s cluster %d: %d vectors, %llu bytes meta.\n",
