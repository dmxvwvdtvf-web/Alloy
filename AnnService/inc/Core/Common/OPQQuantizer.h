...
class OPQQuantizer : public PQQuantizer<OPQMatrixType> {
   public:
    ...
    OPQQuantizer(DimensionType NumSubvectors, SizeType KsPerSubvector,
                 DimensionType DimPerSubvector, bool EnableADC,
                 std::unique_ptr<T[]>&& Codebooks,
                 std::unique_ptr<OPQMatrixType[]>&& OPQMatrix);
    ...
    virtual ErrorCode SaveQuantizer(
        std::shared_ptr<Helper::DiskIO> p_out) const;
    ...
    VectorValueType GetReconstructType() const { return GetEnumValueType<T>(); }
   ...
   protected:
    ...
    const std::function<float(const OPQMatrixType*, const OPQMatrixType*,
                              DimensionType)>
        m_fdot = SPTAG::COMMON::DistanceCalcSelector<OPQMatrixType>(
            SPTAG::DistCalcMethod::Cosine);
    const int m_base = COMMON::Utils::GetBase<OPQMatrixType>() *
                       COMMON::Utils::GetBase<OPQMatrixType>();

    void m_InitMatrixTranspose();

    template <typename O>
    inline void m_VectorMatrixMultiply(OPQMatrixType* mat,
                                       const OPQMatrixType* vec,
                                       O* mat_vec) const;
...
template <typename T>
OPQQuantizer<T>::OPQQuantizer(DimensionType NumSubvectors,
                              SizeType KsPerSubvector,
                              DimensionType DimPerSubvector, bool EnableADC,
                              std::unique_ptr<T[]>&& Codebooks,
                              std::unique_ptr<OPQMatrixType[]>&& OPQMatrix)
    : m_OPQMatrix(std::move(OPQMatrix)),
      PQQuantizer<T>::PQQuantizer(NumSubvectors, KsPerSubvector,
                                  DimPerSubvector, EnableADC,
                                  std::move(Codebooks)),
...
void OPQQuantizer<T>::m_InitMatrixTranspose() {
    m_OPQMatrix_T =
        std::make_unique<OPQMatrixType[]>(m_matrixDim * m_matrixDim);
    for (int i = 0; i < m_matrixDim; i++) {
        for (int j = 0; j < m_matrixDim; j++) {
            m_OPQMatrix_T[i * m_matrixDim + j] =
                m_OPQMatrix[j * m_matrixDim + i];
        }
    }
}
...
void OPQQuantizer<T>::QuantizeVector(const void* vec, std::uint8_t* vecout,
                                     bool ADC) const {
    OPQMatrixType* mat_vec =
        (OPQMatrixType*)ALIGN_ALLOC(sizeof(OPQMatrixType) * m_matrixDim);
    ...
    m_VectorMatrixMultiply<OPQMatrixType>(m_OPQMatrix_T.get(), typed_vec,
                                          mat_vec);
    PQQuantizer<OPQMatrixType>::QuantizeVector(mat_vec, vecout, ADC);
    ALIGN_FREE(mat_vec);

    ISNOTSAME(T, OPQMatrixType) { ALIGN_FREE(typed_vec); }
}

template <typename T>
void OPQQuantizer<T>::ReconstructVector(const std::uint8_t* qvec,
                                        void* vecout) const {
    OPQMatrixType* pre_mat_vec =
        (OPQMatrixType*)ALIGN_ALLOC(sizeof(OPQMatrixType) * m_matrixDim);
...
template <typename T>
ErrorCode OPQQuantizer<T>::SaveQuantizer(
    std::shared_ptr<Helper::DiskIO> p_out) const {
    ...
    IOBINARY(p_out, WriteBinary, sizeof(DimensionType),
             (char*)&m_NumSubvectors);
    IOBINARY(p_out, WriteBinary, sizeof(SizeType), (char*)&m_KsPerSubvector);
    IOBINARY(p_out, WriteBinary, sizeof(DimensionType),
             (char*)&m_DimPerSubvector);
    IOBINARY(p_out, WriteBinary,
             sizeof(OPQMatrixType) * m_NumSubvectors * m_KsPerSubvector *
                 m_DimPerSubvector,
             (char*)m_codebooks.get());
    IOBINARY(p_out, WriteBinary,
             sizeof(OPQMatrixType) * m_matrixDim * m_matrixDim,
             (char*)m_OPQMatrix.get());
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Saving quantizer: Subvectors:%d KsPerSubvector:%d "
                 "DimPerSubvector:%d\n",
...
ErrorCode OPQQuantizer<T>::LoadQuantizer(std::uint8_t* raw_bytes) {
    ...
    raw_bytes += sizeof(DimensionType) + sizeof(SizeType) +
                 sizeof(DimensionType) +
                 (sizeof(OPQMatrixType) * m_NumSubvectors * m_KsPerSubvector *
                  m_DimPerSubvector);
    m_matrixDim = m_NumSubvectors * m_DimPerSubvector;
    m_OPQMatrix = std::make_unique<OPQMatrixType[]>(m_matrixDim * m_matrixDim);
    std::memcpy(m_OPQMatrix.get(), raw_bytes,
                sizeof(OPQMatrixType) * m_matrixDim * m_matrixDim);
...
std::uint64_t OPQQuantizer<T>::BufferSize() const {
    return PQQuantizer<OPQMatrixType>::BufferSize() +
           (sizeof(OPQMatrixType) * m_matrixDim * m_matrixDim);
}
...
template <typename T>
template <typename O>
inline void OPQQuantizer<T>::m_VectorMatrixMultiply(OPQMatrixType* mat,
                                                    const OPQMatrixType* vec,
                                                    O* mat_vec) const {
    for (int i = 0; i < m_matrixDim; i++) {
