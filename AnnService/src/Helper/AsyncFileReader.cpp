...
void SetThreadAffinity(int threadID, std::thread& thread,
                       NumaStrategy socketStrategy, OrderStrategy idStrategy) {
#ifdef NUMA
                ...
                if (cpuid == nodecpu) {
                    ...
                    if (rc != 0) {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                     "Error calling pthread_setaffinity_np for "
                                     "thread %d: %d\n",
                                     threadID, rc);
                    }
                    break;
...
void BatchReadFileAsync(
    const std::vector<std::shared_ptr<Helper::DiskIO>>& handlers,
    ...
    auto f_check_event_result = [](const struct io_event& event) {
        if (event.res < 0) {
            LOG_ERROR("aio error: %s\n", strerror(-event.res));
            AsyncReadRequest* req =
                reinterpret_cast<AsyncReadRequest*>(event.data);
            auto listInfo = reinterpret_cast<SPANN::ListInfo*>(req->m_payload);

            LOG_ERROR("cid=%lu, mid=%u, ele-cnt=%lu, offset=%llu, size=%lluB\n",
                      listInfo->cid, listInfo->mid, listInfo->listEleCount,
                      req->m_offset, req->m_readSize);
            throw std::runtime_error("aio event returned error!");
        }
    };
    ...
    while (totalDone < totalToSubmit) {
                ...
                if (submitted[i] < iocbs[i].size()) {
                    ...
                    if (s > 0) {
                        ...
                        SPTAGLIB_LOG(
                            ...
                            "fid:%d channel %d, to submit:%d, submitted:%s\n",
        ...
        for (int i = 0; i < handlers.size(); i++) {
            if (done[i] < submitted[i]) {
                ...
                AsyncFileIO* handler = (AsyncFileIO*)(handlers[i].get());
                auto d =
                    syscall(__NR_io_getevents, handler->GetIOCP(channel), wait,
...
void SetThreadAffinity(int threadID, std::thread& thread,
    ...
    if (!res) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Failed SetThreadGroupAffinity for group %d and mask "
                     "%I64x for thread %d.\n",
    ...
    // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "numGroup:%d numCPUs:%d
    // threadID:%d group:%d cpuid:%d\n", (int)(numGroups), (int)numCpus,
    // threadID, (int)(group), (int)(pn.Number));
    YieldProcessor();
}

void BatchReadFileAsync(std::vector<std::shared_ptr<Helper::DiskIO>>& handlers,
                        AsyncReadRequest* readRequests, int num) {
    if (handlers.size() == 1) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (fileid != currFileId) {
                handlers[currFileId]->BatchReadFile(readRequests + currReqStart,
                                                    i - currReqStart);
                currFileId = fileid;
                currReqStart = i;
            }
