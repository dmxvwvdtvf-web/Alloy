...
template <typename T>
void MultiIndexSearch(unsigned int n, std::vector<std::vector<T>>& queries,
                      int k, bool userTimer, int termCond) {
    ...
    for (unsigned int i = 0; i < n; i++) {
        ...
        BOOST_CHECK(
            ...
            SPTAG::VectorIndex::LoadIndex(indexName(i).c_str(), vecIndex));
        BOOST_CHECK(nullptr != vecIndex);
        vecIndices.push_back(vecIndex);
        p_targets.push_back(queries[i].data());
    }

    SPTAG::MultiIndexScan scan(vecIndices, p_targets, k, &rankFunc, false, 10,
                               10);
