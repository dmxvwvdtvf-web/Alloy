...
struct KmeansArgs {
    ...
    KmeansArgs(int k, DimensionType dim, SizeType datasize, int threadnum,
        ...
        if (m_pQuantizer) {
            ...
            fComputeDistance =
                m_pQuantizer->DistanceCalcSelector<T>(distMethod);
        } else {
    ...
    void Shuffle(std::vector<SizeType>& indices, SizeType first,
                 SizeType last) {
...
void RefineLambda(KmeansArgs<T>& args, float& lambda, int size) {
    ...
    float avgDist =
        args.newWeightedCounts[maxcluster] / args.newCounts[maxcluster];
    // lambda = avgDist / 10 / args.counts[maxcluster];
    // lambda = (args.clusterDist[maxcluster] - avgDist) /
    // args.newCounts[maxcluster];
...
float RefineCenters(const Dataset<T>& data, KmeansArgs<T>& args) {
    ...
    for (int k = 0; k < args._DK; k++) {
        if (args.counts[k] > maxCount && args.newCounts[k] > 0 &&
            DistanceUtils::ComputeDistance((T*)data[args.clusterIdx[k]],
                                           args.centers + k * args._D, args._D,
                                           DistCalcMethod::L2) > 1e-6) {
            maxcluster = k;
            maxCount = args.counts[k];
        }
    }

    if (maxcluster != -1 && (args.clusterIdx[maxcluster] < 0 ||
                             args.clusterIdx[maxcluster] >= data.R()))
        SPTAGLIB_LOG(Helper::LogLevel::LL_Debug,
                     "maxcluster:%d(%d) Error dist:%f\n", maxcluster,
                     args.newCounts[maxcluster], args.clusterDist[maxcluster]);
    ...
    for (int k = 0; k < args._DK; k++) {
        ...
        if (args.counts[k] == 0) {
            if (maxcluster != -1) {
                ...
                std::memcpy(TCenter, args.centers + k * args._D,
                            sizeof(T) * args._D);
            }
        } else {
            ...
            if (args._M == DistCalcMethod::Cosine) {
                COMMON::Utils::Normalize(currCenters, args._RD,
            ...
            if (args.m_pQuantizer) {
                ...
                args.m_pQuantizer->QuantizeVector(reconstructVector.data(),
                                                  (uint8_t*)TCenter);
            } else {
        ...
        diff += DistanceUtils::ComputeDistance(
            TCenter, args.centers + k * args._D, args._D, DistCalcMethod::L2);
    }
    return diff;
}
...
inline float KmeansAssign(const Dataset<T>& data,
                          std::vector<SizeType>& indices, const SizeType first,
                          const SizeType last, KmeansArgs<T>& args,
                          const bool updateCenters, float lambda) {
    float currDist = 0;
    SizeType totalSize = last - first;

    // TODO - compile-time options for MAX_DIM and metric
    computeKmeansGPU<T, float, 100>(
        data, indices, first, last, args._K, args._D, args._DK, lambda,
        args.centers, args.label, args.counts, args.newCounts, args.newCenters,
        args.clusterIdx, args.clusterDist, args.weightedCounts,
        args.newWeightedCounts, 0, updateCenters);
}
...
#pragma omp parallel for num_threads(args._T) shared(data, indices) \
    reduction(+ : currDist)
    for (int tid = 0; tid < args._T; tid++) {
        ...
        if (args.m_pQuantizer)
            reconstructVector =
                (R*)ALIGN_ALLOC(args.m_pQuantizer->ReconstructSize());
        ...
        for (SizeType i = istart; i < iend; i++) {
            ...
            if (updateCenters) {
                if (args.m_pQuantizer) {
                    args.m_pQuantizer->ReconstructVector(
                        (const uint8_t*)data[indices[i]], reconstructVector);
                } else {
...
template <typename T, typename R>
inline float InitCenters(const Dataset<T>& data, std::vector<SizeType>& indices,
    ...
    for (int numKmeans = 0; numKmeans < tryIters; numKmeans++) {
        for (int k = 0; k < args._DK; k++) {
            ...
            std::memcpy(args.centers + k * args._D, data[indices[randid]],
                        sizeof(T) * args._D);
        }
        args.ClearCounts();
        args.ClearDists(-MaxDist);
        currDist =
            KmeansAssign<T, R>(data, indices, first, batchEnd, args, true, 0);
...
template <typename T, typename R>
float TryClustering(const Dataset<T>& data, std::vector<SizeType>& indices,
                    const SizeType first, const SizeType last,
                    KmeansArgs<T>& args, int samples = 1000,
                    float lambdaFactor = 100.0f, bool debug = false,
                    IAbortOperation* abort = nullptr) {
    float adjustedLambda =
        InitCenters<T, R>(data, indices, first, last, args, samples, 3);
    if (abort && abort->ShouldAbort()) return 0;

    SizeType batchEnd = min(first + samples, last);
    float currDiff, currDist, minClusterDist = MaxDist;
    int noImprovement = 0;
    float originalLambda = COMMON::Utils::GetBase<T>() *
                           COMMON::Utils::GetBase<T>() / lambdaFactor /
                           (batchEnd - first);
    for (int iter = 0; iter < 100; iter++) {
        ...
        args.ClearDists(-MaxDist);
        currDist =
            KmeansAssign<T, R>(data, indices, first, batchEnd, args, true,
                               min(adjustedLambda, originalLambda));
        ...
        if (debug) {
            ...
            SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "iter %d dist:%f
        lambda:(%f,%f) counts:%s\n", iter, currDist, originalLambda,
        adjustedLambda, log.c_str());
        }
        */

        currDiff = RefineCenters<T, R>(data, args);
        // if (debug) SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "iter %d dist:%f
    ...
    for (int k = 0; k < args._DK; k++) {
        if (args.clusterIdx[k] != -1)
            std::memcpy(args.centers + k * args._D, data[args.clusterIdx[k]],
                        sizeof(T) * args._D);
    }
    ...
    if (debug)
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "Lambda:min(%g,%g) Max:%d Min:%d Avg:%f Std/Avg:%f "
                     "Dist:%f NonZero/Total:%d/%d\n",
                     originalLambda, adjustedLambda, maxCount, minCount,
                     CountAvg, CountStd, currDist, availableClusters, args._DK);
...
float DynamicFactorSelect(const Dataset<T>& data,
    ...
    float bestLambdaFactor = 100.0f,
          bestCountStd = (std::numeric_limits<float>::max)();
    for (float lambdaFactor = 0.001f; lambdaFactor <= 1000.0f + 1e-3;
         lambdaFactor *= 10) {
        float CountStd = 0.0;
        if (args.m_pQuantizer) {
...
#define DefineVectorValueType(Name, Type)                                   \
    case VectorValueType::Name:                                             \
        CountStd = TryClustering<T, Type>(data, indices, first, last, args, \
                                          samples, lambdaFactor, true);     \
        break;
...
#undef DefineVectorValueType
            ...
            CountStd = TryClustering<T, T>(data, indices, first, last, args,
                                           samples, lambdaFactor, true);
        }
    ...
    for (float lambdaFactor : tries) {
        float CountStd = TryClustering(data, indices, first, last, args,
    samples, lambdaFactor, true); if (CountStd < bestCountStd) {
            bestLambdaFactor = lambdaFactor;
            bestCountStd = CountStd;
        }
    }
...
template <typename T>
int KmeansClustering(const Dataset<T>& data, std::vector<SizeType>& indices,
                     const SizeType first, const SizeType last,
                     KmeansArgs<T>& args, int samples = 1000,
                     float lambdaFactor = 100.0f, bool debug = false,
                     IAbortOperation* abort = nullptr) {
...
#define DefineVectorValueType(Name, Type)                                 \
    case VectorValueType::Name:                                           \
        TryClustering<T, Type>(data, indices, first, last, args, samples, \
                               lambdaFactor, debug, abort);               \
        break;
...
#undef DefineVectorValueType
        ...
        TryClustering<T, T>(data, indices, first, last, args, samples,
                            lambdaFactor, debug, abort);
    }
...
class BKTree {
   public:
    ...
    void Rebuild(const Dataset<T>& data, DistCalcMethod distMethod,
                 IAbortOperation* abort) {
        BKTree newTrees(*this);
        newTrees.BuildTrees<T>(data, distMethod, 1, nullptr, nullptr, false,
                               abort);
    ...
    template <typename T>
    void BuildTrees(const Dataset<T>& data, DistCalcMethod distMethod,
                    int numOfThreads, std::vector<SizeType>* indices = nullptr,
                    std::vector<SizeType>* reverseIndices = nullptr,
                    bool dynamicK = false, IAbortOperation* abort = nullptr) {
        struct BKTStackItem {
            SizeType index, first, last;
            bool debug;
            BKTStackItem(SizeType index_, SizeType first_, SizeType last_,
                         bool debug_ = false)
        ...
        KmeansArgs<T> args(m_iBKTKmeansK, data.C(),
                           (SizeType)localindices.size(), numOfThreads,
                           distMethod, m_pQuantizer);

        if (m_fBalanceFactor < 0)
            m_fBalanceFactor = DynamicFactorSelect(
                data, localindices, 0, (SizeType)localindices.size(), args,
                m_iSamples);
        ...
        for (char i = 0; i < m_iTreeNumber; i++) {
            ...
            SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                         "Start to build BKTree %d\n", i + 1);

            ss.push(BKTStackItem(m_pTreeStart[i], 0,
                                 (SizeType)localindices.size(), true));
            while (!ss.empty()) {
                ...
                if (item.last - item.first <= m_iBKTLeafSize) {
                    ...
                    if (numClusters <= 1) {
                        ...
                        for (SizeType j = item.first + 1; j < end; j++) {
                            SizeType cid =
                                (reverseIndices == nullptr)
                                    ...
                                    : reverseIndices->at(localindices[j]);
                            m_pTreeRoots.emplace_back(cid);
                            m_pSampleCenterMap[cid] =
                                m_pTreeRoots[item.index].centerid;
                        }
                        m_pSampleCenterMap[-1 -
                                           m_pTreeRoots[item.index].centerid] =
                            item.index;
                    } else {
                        SizeType maxCount = 0;
                        ...
                        for (int k = 0; k < m_iBKTKmeansK; k++) {
                            ...
                            SizeType cid =
                                (reverseIndices == nullptr)
                                    ? localindices[item.first + args.counts[k] -
                                                   1]
                                    : reverseIndices->at(
                                          localindices[item.first +
                                                       args.counts[k] - 1]);
                            m_pTreeRoots.emplace_back(cid);
                            if (args.counts[k] > 1)
                                ss.push(BKTStackItem(
                                    (SizeType)(m_pTreeRoots.size() - 1),
                                    item.first, item.first + args.counts[k] - 1,
                                    item.debug &&
                                        (args.counts[k] == maxCount)));
                            item.first += args.counts[k];
                        }
                    }
                }
                m_pTreeRoots[item.index].childEnd =
                    (SizeType)m_pTreeRoots.size();
            }
            m_pTreeRoots.emplace_back(-1);
            SPTAGLIB_LOG(
                Helper::LogLevel::LL_Info, "%d BKTree built, %zu %zu\n", i + 1,
                m_pTreeRoots.size() - m_pTreeStart[i], localindices.size());
        }
    }
    ...
    ErrorCode SaveTrees(std::shared_ptr<Helper::DiskIO> p_out) const {
        std::shared_lock<std::shared_timed_mutex> lock(*m_lock);
        IOBINARY(p_out, WriteBinary, sizeof(m_iTreeNumber),
                 (char*)&m_iTreeNumber);
        IOBINARY(p_out, WriteBinary, sizeof(SizeType) * m_iTreeNumber,
                 (char*)m_pTreeStart.data());
        SizeType treeNodeSize = (SizeType)m_pTreeRoots.size();
        IOBINARY(p_out, WriteBinary, sizeof(treeNodeSize),
                 (char*)&treeNodeSize);
        IOBINARY(p_out, WriteBinary, sizeof(BKTNode) * treeNodeSize,
    ...
    ErrorCode SaveTrees(std::string sTreeFileName) const {
        ...
        auto ptr = f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(sTreeFileName.c_str(),
    ...
    ErrorCode LoadTrees(char* pBKTMemFile) {
        ...
        memcpy(m_pTreeStart.data(), pBKTMemFile,
               sizeof(SizeType) * m_iTreeNumber);
        ...
        memcpy(m_pTreeRoots.data(), pBKTMemFile,
    ...
    ErrorCode LoadTrees(std::shared_ptr<Helper::DiskIO> p_input) {
        IOBINARY(p_input, ReadBinary, sizeof(m_iTreeNumber),
                 (char*)&m_iTreeNumber);
        m_pTreeStart.resize(m_iTreeNumber);
        IOBINARY(p_input, ReadBinary, sizeof(SizeType) * m_iTreeNumber,
                 (char*)m_pTreeStart.data());

        SizeType treeNodeSize;
        IOBINARY(p_input, ReadBinary, sizeof(treeNodeSize),
                 (char*)&treeNodeSize);
        m_pTreeRoots.resize(treeNodeSize);
    ...
    ErrorCode LoadTrees(std::string sTreeFileName) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Load BKT From %s\n",
                     sTreeFileName.c_str());
        auto ptr = f_createIO();
        if (ptr == nullptr || !ptr->Initialize(sTreeFileName.c_str(),
    ...
    template <typename T>
    void InitSearchTrees(const Dataset<T>& data,
        ...
        for (char i = 0; i < m_iTreeNumber; i++) {
            ...
            if (node.childStart < 0) {
                p_space.m_SPTQueue.insert(NodeDistPair(
                    m_pTreeStart[i],
                    fComputeDistance(p_query.GetQuantizedTarget(),
                                     data[node.centerid], data.C())));
            } else if (m_bfs) {
                float FactorQ = 1.1f;
                int MaxBFSNodes = 100;
                p_space.m_currBSPTQueue.Resize(MaxBFSNodes);
                p_space.m_nextBSPTQueue.Resize(MaxBFSNodes);
                Heap<NodeDistPair>*p_curr = &p_space.m_currBSPTQueue,
                *p_next = &p_space.m_nextBSPTQueue;
                p_curr->Top().distance = 1e9;

                for (SizeType begin = node.childStart; begin < node.childEnd;
                     begin++) {
                    _mm_prefetch(
                        (const char*)(data[m_pTreeRoots[begin].centerid]),
                        _MM_HINT_T0);
                }
                ...
                for (SizeType begin = node.childStart; begin < node.childEnd;
                    ...
                    float dist = fComputeDistance(p_query.GetQuantizedTarget(),
                                                  data[index], data.C());
                    if (dist <= FactorQ * p_curr->Top().distance &&
                        p_curr->size() < MaxBFSNodes) {
                ...
                for (int level = 1; level <= m_bfs; level++) {
                    ...
                    while (!p_curr->empty()) {
                        ...
                        if (tnode.childStart < 0) {
                            ...
                            for (SizeType begin = tnode.childStart;
                                 begin < tnode.childEnd; begin++) {
                                _mm_prefetch(
                            ...
                            if (!p_space.CheckAndSet(tnode.centerid)) {
                                p_space.m_NGQueue.insert(
                                    NodeDistPair(tnode.centerid, tmp.distance));
                            }
                            for (SizeType begin = tnode.childStart;
                                 begin < tnode.childEnd; begin++) {
                                SizeType index = m_pTreeRoots[begin].centerid;
                                float dist = fComputeDistance(
                                    p_query.GetQuantizedTarget(), data[index],
                                    data.C());
                                if (dist <= FactorQ * p_next->Top().distance &&
                                    p_next->size() < MaxBFSNodes) {
                                    p_next->insert(NodeDistPair(begin, dist));
                                } else {
                                    p_space.m_SPTQueue.insert(
                                        NodeDistPair(begin, dist));
                                }
                            }
                        }
                    }
                ...
                for (SizeType begin = node.childStart; begin < node.childEnd;
                    ...
                    SizeType index = m_pTreeRoots[begin].centerid;
                    p_space.m_SPTQueue.insert(NodeDistPair(
                        begin, fComputeDistance(p_query.GetQuantizedTarget(),
                                                data[index], data.C())));
                }
            }
        }
    ...
    template <typename T>
    void SearchTrees(const Dataset<T>& data,
                     std::function<float(const T*, const T*, DimensionType)>
                         fComputeDistance,
                     COMMON::QueryResultSet<T>& p_query,
        ...
        while (!p_space.m_SPTQueue.empty()) {
            ...
            if (tnode.childStart < 0) {
                if (!p_space.CheckAndSet(tnode.centerid)) {
                    ...
                    p_space.m_NGQueue.insert(
                        NodeDistPair(tnode.centerid, bcell.distance));
                }
                if (p_space.m_iNumberOfCheckedLeaves >= p_limits) break;
            } else {
                for (SizeType begin = tnode.childStart; begin < tnode.childEnd;
                     begin++) {
                    _mm_prefetch(
                        (const char*)(data[m_pTreeRoots[begin].centerid]),
                        _MM_HINT_T0);
