...
int BootProgram(
    ...
    const char* configurationPath, VectorValueType valueType,
    DistCalcMethod distCalcMethod, const char* dataFilePath,
    const char* indexFilePath) {
    bool searchSSD = false;
    std::string QuantizerFilePath = "";
    if (forANNIndexTestTool) {
        (*config_map)[SEC_BASE]["ValueType"] =
            Helper::Convert::ConvertToString(valueType);
        (*config_map)[SEC_BASE]["DistCalcMethod"] =
            Helper::Convert::ConvertToString(distCalcMethod);
        ...
        if ((iter = (*config_map)[SEC_BASE].find("QuantizerFilePath")) !=
            (*config_map)[SEC_BASE].end()) {
        ...
        (*config_map)[SEC_SELECT_HEAD] =
            iniReader.GetParameters(SEC_SELECT_HEAD);
        ...
        distCalcMethod =
            iniReader.GetParameter(SEC_BASE, "DistCalcMethod", distCalcMethod);
        bool buildSSD =
            iniReader.GetParameter(SEC_BUILD_SSD_INDEX, "isExecute", false);
        searchSSD =
            iniReader.GetParameter(SEC_SEARCH_SSD_INDEX, "isExecute", false);
        QuantizerFilePath = iniReader.GetParameter(
            SEC_BASE, "QuantizerFilePath", std::string(""));
    ...
    std::shared_ptr<VectorIndex> index =
        VectorIndex::CreateInstance(IndexAlgoType::SPANN, valueType);
    if (!QuantizerFilePath.empty() &&
        index->LoadQuantizer(QuantizerFilePath) != ErrorCode::Success) {
        exit(1);
    }
    if (index == nullptr) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Cannot create Index with ValueType %s!\n",
...
#undef DefineVectorValueType
    ...
    if (opts->m_generateTruth) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "Start generating truth. It's maybe a long time.\n");
        ...
        std::shared_ptr<Helper::ReaderOptions> vectorOptions(
            new Helper::ReaderOptions(valueType, dim, opts->m_vectorType,
                                      opts->m_vectorDelimiter));
        auto vectorReader =
...
#define DefineVectorValueType(Name, Type)                               \
    if (opts->m_valueType == VectorValueType::Name) {                   \
        COMMON::TruthSet::GenerateTruth<Type>(                          \
            querySet, vectorSet, opts->m_truthPath, distCalcMethod,     \
