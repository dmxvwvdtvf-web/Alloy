...
struct SearchResult : SearchResultT<SearchResult> {
    winrt::com_array<uint8_t> Metadata() {
        return winrt::com_array(m_metadata);
    }
    std::vector<uint8_t> m_metadata;
    ReadOnlyProperty<float> Distance;

    SearchResult() = default;
    SearchResult(winrt::array_view<uint8_t> metadata, float d)
        : m_metadata(metadata.begin(), metadata.end()), Distance(d) {}
    SearchResult(uint8_t* metadata, size_t length, float d)
        : m_metadata(metadata, metadata + length), Distance(d) {}
};
...
struct AnnIndex : AnnIndexT<AnnIndex> {
    ...
    AnnIndex() {
        sptag::SetLogger(std::make_shared<sptag::Helper::SimpleLogger>(
            sptag::Helper::LogLevel::LL_Empty));
        m_index = sptag::VectorIndex::CreateInstance(
            sptag::IndexAlgoType::BKT, sptag::GetEnumValueType<float>());
    }

    void AddWithMetadata(array_view<float const> data,
    ...
    SPTAG::SearchResult GetResultFromMetadata(
        const sptag::BasicResult& r) const;

    winrt::Windows::Foundation::Collections::IVector<SPTAG::SearchResult>
    Search(EmbeddingVector p_data, uint32_t p_resultNum) const;
