...
std::shared_ptr<VectorSet> TxtVectorReader::GetVectorSet(SizeType start,
                                                         SizeType end) const {
    auto ptr = f_createIO();
    if (ptr == nullptr || !ptr->Initialize(m_vectorOutput.c_str(),
    ...
    if (ptr->ReadBinary(sizeof(DimensionType), (char*)&col) !=
    ...
    std::uint64_t totalRecordVectorBytes =
        ((std::uint64_t)GetValueTypeSize(m_options->m_inputValueType)) *
        (end - start) * col;
    ByteArray vectorSet;
    if (totalRecordVectorBytes > 0) {
        ...
        if (ptr->ReadBinary(totalRecordVectorBytes, vecBuf, offset) !=
...
ErrorCode TxtVectorReader::LoadFileInternal(const std::string& p_filePath,
    ...
    std::shared_ptr<Helper::DiskIO> input = f_createIO(), output = f_createIO(),
                                    meta = f_createIO(),
                                    metaIndex = f_createIO();
    if (input == nullptr ||
        !input->Initialize(p_filePath.c_str(),
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Begin Subtask: %u, start offset position: %lld\n",
    ...
    if (output == nullptr ||
        !output->Initialize((m_vectorOutput + subFileSuffix).c_str(),
                            std::ios::binary | std::ios::out) ||
        meta == nullptr ||
        !meta->Initialize((m_metadataConentOutput + subFileSuffix).c_str(),
                          std::ios::binary | std::ios::out) ||
        metaIndex == nullptr ||
        !metaIndex->Initialize((m_metadataIndexOutput + subFileSuffix).c_str(),
                               std::ios::binary | std::ios::out)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Unable to create files: %s %s %s\n",
                     (m_vectorOutput + subFileSuffix).c_str(),
                     (m_metadataConentOutput + subFileSuffix).c_str(),
    ...
    if (p_fileBlockID != 0) {
        totalRead +=
            input->ReadString(lineBufferSize, currentLine, '\n', startpos);
    }
...
#undef DefineVectorValueType
        ...
        ++recordCount;
        if (output->WriteBinary(vectorByteSize, (char*)vector.get()) !=
                vectorByteSize ||
            meta->WriteBinary(tabIndex, currentLine.get()) != tabIndex ||
            metaIndex->WriteBinary(sizeof(metaOffset),
                                   (const char*)&metaOffset) !=
                sizeof(metaOffset)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
    ...
    if (metaIndex->WriteBinary(sizeof(metaOffset), (const char*)&metaOffset) !=
        sizeof(metaOffset)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
...
ErrorCode TxtVectorReader::MergeData() {
    ...
    if (output == nullptr ||
        !output->Initialize(m_vectorOutput.c_str(),
                            std::ios::binary | std::ios::out) ||
        meta == nullptr ||
        !meta->Initialize(m_metadataConentOutput.c_str(),
                          std::ios::binary | std::ios::out) ||
        metaIndex == nullptr ||
        !metaIndex->Initialize(m_metadataIndexOutput.c_str(),
                               std::ios::binary | std::ios::out)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Unable to create files: %s %s %s\n",
                     m_vectorOutput.c_str(), m_metadataConentOutput.c_str(),
    ...
    if (output->WriteBinary(sizeof(totalRecordCount),
                            (char*)(&totalRecordCount)) !=
        sizeof(totalRecordCount)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Unable to write file: %s\n",
                     m_vectorOutput.c_str());
        return ErrorCode::DiskIOFail;
    }
    if (output->WriteBinary(sizeof(m_options->m_dimension),
                            (char*)&(m_options->m_dimension)) !=
    ...
    for (std::uint32_t i = 0; i < m_subTaskCount; ++i) {
        ...
        if (input == nullptr ||
            !input->Initialize(file.c_str(), std::ios::binary | std::ios::in)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
        ...
        std::uint64_t readSize =
            input->ReadBinary(bufferSize, bufferHolder.get());
        while (readSize != 0) {
    ...
    if (metaIndex->WriteBinary(sizeof(totalRecordCount),
                               (char*)(&totalRecordCount)) !=
        sizeof(totalRecordCount)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Unable to write file: %s\n",
                     m_metadataIndexOutput.c_str());
    ...
    for (std::uint32_t i = 0; i < m_subTaskCount; ++i) {
        ...
        for (SizeType remains = m_subTaskRecordCount[i]; remains > 0;) {
            ...
            for (std::uint64_t j = 0;
                 j < readBytesCount / sizeof(std::uint64_t); ++j) {
            ...
            remains -=
                static_cast<SizeType>(readBytesCount / sizeof(std::uint64_t));
        }
        if (input->ReadBinary(sizeof(std::uint64_t), buf) !=
            sizeof(std::uint64_t)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Unable to read file: %s\n", file.c_str());
            return ErrorCode::DiskIOFail;
    ...
    if (metaIndex->WriteBinary(sizeof(totalOffset), (char*)&totalOffset) !=
        sizeof(totalOffset)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Unable to write file: %s\n",
                     m_metadataIndexOutput.c_str());
        return ErrorCode::DiskIOFail;
...
std::vector<TxtVectorReader::FileInfoPair> TxtVectorReader::GetFileSizes(
