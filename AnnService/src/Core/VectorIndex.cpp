...
typedef typename SPTAG::Helper::Concurrent::ConcurrentMap<std::string,
                                                          SPTAG::SizeType>
    MetadataMap;
...
Helper::LoggerHolder& SPTAG::GetLoggerHolder() {
    ...
    if (auto exeHandle = GetModuleHandleW(nullptr)) {
        if (auto SPTAG_GetLoggerLevel =
                reinterpret_cast<SPTAG::Helper::LogLevel (*)()>(
                    GetProcAddress(exeHandle, "SPTAG_GetLoggerLevel"))) {
            logLevel = SPTAG_GetLoggerLevel();
        }
    }
#endif  //  _WINDOWS_
    static Helper::LoggerHolder s_pLoggerHolder(
        std::make_shared<Helper::SimpleLogger>(logLevel));
...
std::shared_ptr<Helper::DiskIO> (*SPTAG::f_createIO)() =
...
bool copyfile(const char* oldpath, const char* newpath) {
    ...
    if (input == nullptr ||
        !input->Initialize(oldpath, std::ios::binary | std::ios::in) ||
        output == nullptr ||
...
std::string VectorIndex::GetParameter(const std::string& p_param,
...
void VectorIndex::SetMetadata(MetadataSet* p_new) { m_pMetadata.reset(p_new); }

MetadataSet* VectorIndex::GetMetadata() const { return m_pMetadata.get(); }
...
std::shared_ptr<std::vector<std::uint64_t>> VectorIndex::CalculateBufferSize()
    const {
...
ErrorCode VectorIndex::LoadIndexConfig(Helper::IniReader& p_reader) {
    ...
    if (p_reader.DoesSectionExist(metadataSection)) {
        m_sMetadataFile = p_reader.GetParameter(
            metadataSection, "MetaDataFilePath", std::string());
        m_sMetadataIndexFile = p_reader.GetParameter(
    ...
    if (p_reader.DoesSectionExist(quantizerSection)) {
        m_sQuantizerFile = p_reader.GetParameter(
            quantizerSection, "QuantizerFilePath", std::string());
    }
    return LoadConfig(p_reader);
}

ErrorCode VectorIndex::SaveIndexConfig(
    std::shared_ptr<Helper::DiskIO> p_configOut) {
    if (nullptr != m_pMetadata) {
        IOSTRING(p_configOut, WriteString, "[MetaData]\n");
        IOSTRING(p_configOut, WriteString,
                 ("MetaDataFilePath=" + m_sMetadataFile + "\n").c_str());
        IOSTRING(p_configOut, WriteString,
                 ("MetaDataIndexPath=" + m_sMetadataIndexFile + "\n").c_str());
    ...
    if (m_pQuantizer) {
        ...
        IOSTRING(p_configOut, WriteString,
                 ("QuantizerFilePath=" + m_sQuantizerFile + "\n").c_str());
        IOSTRING(p_configOut, WriteString, "\n");
    }
...
ErrorCode VectorIndex::SaveIndex(std::string& p_config,
    ...
    {
        std::shared_ptr<Helper::DiskIO> p_configStream(
            new Helper::SimpleBufferIO());
        auto bufsize = 2 << 20;
        std::vector<char> buf(bufsize);  // Allocate 1 MB scratch space
        if (p_configStream == nullptr ||
            !p_configStream->Initialize(buf.data(), std::ios::out, bufsize))
            return ErrorCode::EmptyDiskIO;
        if ((ret = SaveIndexConfig(p_configStream)) != ErrorCode::Success)
            return ret;
        p_config.resize(p_configStream->TellP());
        IOBINARY(p_configStream, ReadBinary, p_config.size(),
                 (char*)p_config.c_str(), 0);
    }
    ...
    for (size_t i = 0; i < p_indexBlobs.size(); i++) {
        ...
        if (ptr == nullptr || !ptr->Initialize((char*)p_indexBlobs[i].Data(),
    ...
    if (NeedRefine()) {
        ...
        if (m_pMetadata != nullptr && p_indexStreams.size() >= metaStart + 2) {
            ret = m_pMetadata->SaveMetadata(p_indexStreams[metaStart],
                                            p_indexStreams[metaStart + 1]);
        }
...
ErrorCode VectorIndex::SaveIndex(const std::string& p_folderPath) {
    ...
    if (GetIndexAlgoType() == IndexAlgoType::SPANN &&
        GetParameter("IndexDirectory", "Base") != p_folderPath) {
        ...
        for (auto file : files) {
            size_t firstSep = oldFolder.length(),
                   lastSep = file.find_last_of(FolderSep);
            std::string newFolder =
                            folderPath +
                            ((lastSep > firstSep)
                                 ? file.substr(firstSep, lastSep - firstSep)
                                 : ""),
    ...
    ErrorCode ret = ErrorCode::Success;
    {
        ...
        if (configFile == nullptr ||
            !configFile->Initialize((folderPath + "indexloader.ini").c_str(),
                                    std::ios::out))
    ...
    for (std::string& f : *indexfiles) {
        ...
        if (!direxists(
                newfile.substr(0, newfile.find_last_of(FolderSep)).c_str()))
            mkdir(newfile.substr(0, newfile.find_last_of(FolderSep)).c_str());

        auto ptr = SPTAG::f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(newfile.c_str(), std::ios::binary | std::ios::out))
            return ErrorCode::FailedCreateFile;
    ...
    if (NeedRefine()) {
        ...
        if (m_pMetadata != nullptr)
            ret = m_pMetadata->SaveMetadata(handles[metaStart],
                                            handles[metaStart + 1]);
...
ErrorCode VectorIndex::SaveIndexToFile(const std::string& p_file,
    ...
    if (fp == nullptr ||
        !fp->Initialize(p_file.c_str(), std::ios::binary | std::ios::out))
        return ErrorCode::FailedCreateFile;
    ...
    std::vector<char> buf(bufsize);  // Allocate 1 MB scratch space
    if (mp == nullptr ||
        !mp->Initialize(buf.data(), std::ios::binary | std::ios::out, bufsize))
        return ErrorCode::FailedCreateFile;
    ErrorCode ret = ErrorCode::Success;
...
ErrorCode VectorIndex::BuildIndex(std::shared_ptr<VectorSet> p_vectorSet,
    ...
    bool quantizerMatches =
        ((bool)m_pQuantizer) &&
        (p_vectorSet->GetValueType() == SPTAG::VectorValueType::UInt8);
    ...
    BuildIndex(p_vectorSet->GetData(), p_vectorSet->Count(),
               p_vectorSet->Dimension(), p_normalized, p_shareOwnership);
    return ErrorCode::Success;
}

ErrorCode VectorIndex::SearchIndex(const void* p_vector, int p_vectorCount,
                                   int p_neighborCount, bool p_withMeta,
                                   BasicResult* p_results) const {
    size_t vectorSize =
        GetValueTypeSize(GetVectorValueType()) * GetFeatureDim();
#pragma omp parallel for schedule(dynamic, 10)
    for (int i = 0; i < p_vectorCount; i++) {
        QueryResult res((char*)p_vector + i * vectorSize, p_neighborCount,
                        p_withMeta, p_results + i * p_neighborCount);
        SearchIndex(res);
    }
    return ErrorCode::Success;
}
...
ErrorCode VectorIndex::AddIndex(std::shared_ptr<VectorSet> p_vectorSet,
                                std::shared_ptr<MetadataSet> p_metadataSet,
                                bool p_withMetaIndex, bool p_normalized) {
    if (nullptr == p_vectorSet ||
        p_vectorSet->GetValueType() != GetVectorValueType()) {
        return ErrorCode::Fail;
    }

    return AddIndex(p_vectorSet->GetData(), p_vectorSet->Count(),
                    p_vectorSet->Dimension(), p_metadataSet, p_withMetaIndex,
                    p_normalized);
}
...
ErrorCode VectorIndex::MergeIndex(VectorIndex* p_addindex, int p_threadnum,
                                  IAbortOperation* p_abort) {
    ErrorCode ret = ErrorCode::Success;
        ...
        for (SizeType i = 0; i < p_addindex->GetNumSamples(); i++) {
            ...
            if (p_addindex->ContainSample(i)) {
                ...
                std::shared_ptr<MetadataSet> p_metaSet(new MemMetadataSet(
                    meta,
                    ByteArray((std::uint8_t*)offsets, sizeof(offsets), false),
                    1));
                AddIndex(p_addindex->GetSample(i), 1,
                         p_addindex->GetFeatureDim(), p_metaSet);
            }
...
#pragma omp parallel for num_threads(p_threadnum) schedule(dynamic, 128)
        for (SizeType i = 0; i < p_addindex->GetNumSamples(); i++) {
            ...
            if (p_addindex->ContainSample(i)) {
                AddIndex(p_addindex->GetSample(i), 1,
                         p_addindex->GetFeatureDim(), nullptr);
            }
...
ErrorCode VectorIndex::LoadQuantizer(std::string p_quantizerFile) {
    auto ptr = SPTAG::f_createIO();
    if (!ptr->Initialize(p_quantizerFile.c_str(),
...
ErrorCode VectorIndex::LoadIndex(const std::string& p_loaderFilePath,
    ...
    Helper::IniReader iniReader;
    {
        auto fp = SPTAG::f_createIO();
        if (fp == nullptr ||
            !fp->Initialize((folderPath + "indexloader.ini").c_str(),
                            std::ios::in))
            return ErrorCode::FailedOpenFile;
    ...
    VectorValueType valueType = iniReader.GetParameter(
        "Index", "ValueType", VectorValueType::Undefined);
    ...
    std::shared_ptr<std::vector<std::string>> indexfiles =
        p_vectorIndex->GetIndexFiles();
    ...
    for (std::string& f : *indexfiles) {
        auto ptr = SPTAG::f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize((folderPath + f).c_str(),
                             std::ios::binary | std::ios::in)) {
    ...
    if (iniReader.DoesSectionExist("MetaData")) {
        p_vectorIndex->SetMetadata(new MemMetadataSet(
            handles[metaStart], handles[metaStart + 1],
        ...
        if (iniReader.GetParameter("MetaData", "MetaDataToVectorIndex",
                                   std::string()) == "true") {
    ...
    if (iniReader.DoesSectionExist("Quantizer")) {
        p_vectorIndex->SetQuantizer(
            SPTAG::COMMON::IQuantizer::LoadIQuantizer(handles[metaStart]));
...
ErrorCode VectorIndex::LoadIndexFromFile(
    const std::string& p_file, std::shared_ptr<VectorIndex>& p_vectorIndex) {
    auto fp = SPTAG::f_createIO();
    if (fp == nullptr ||
        !fp->Initialize(p_file.c_str(), std::ios::binary | std::ios::in))
        return ErrorCode::FailedOpenFile;
    ...
    SPTAG::Helper::IniReader iniReader;
    {
        ...
        std::shared_ptr<Helper::DiskIO> bufferhandle(
            new Helper::SimpleBufferIO());
        if (bufferhandle == nullptr ||
            !bufferhandle->Initialize(config.data(), std::ios::in, configSize))
            return ErrorCode::EmptyDiskIO;
    ...
    std::vector<std::shared_ptr<Helper::DiskIO>> p_indexStreams(blobs, fp);
    if ((ret = p_vectorIndex->LoadIndexData(p_indexStreams)) !=
        ErrorCode::Success)
        return ret;

    if (iniReader.DoesSectionExist("MetaData")) {
        p_vectorIndex->SetMetadata(new MemMetadataSet(
            fp, fp, p_vectorIndex->m_iDataBlockSize,
            p_vectorIndex->m_iDataCapacity, p_vectorIndex->m_iMetaRecordSize));
    ...
    if (iniReader.DoesSectionExist("Quantizer")) {
        p_vectorIndex->SetQuantizer(
            SPTAG::COMMON::IQuantizer::LoadIQuantizer(fp));
        if (!p_vectorIndex->m_pQuantizer) return ErrorCode::FailedParseValue;
    }
...
ErrorCode VectorIndex::LoadIndex(const std::string& p_config,
                                 const std::vector<ByteArray>& p_indexBlobs,
                                 std::shared_ptr<VectorIndex>& p_vectorIndex) {
    SPTAG::Helper::IniReader iniReader;
    std::shared_ptr<Helper::DiskIO> fp(new Helper::SimpleBufferIO());
    if (fp == nullptr ||
        !fp->Initialize(p_config.c_str(), std::ios::in, p_config.size()))
        return ErrorCode::EmptyDiskIO;
    ...
    if ((p_vectorIndex = CreateInstance(algoType, valueType)) == nullptr)
        return ErrorCode::FailedParseValue;
    if (!iniReader
             .GetParameter<std::string>("Base", "QuantizerFilePath",
                                        std::string())
             .empty()) {
        p_vectorIndex->SetQuantizer(
            COMMON::IQuantizer::LoadIQuantizer(p_indexBlobs[4]));
        if (!p_vectorIndex->m_pQuantizer) return ErrorCode::FailedParseValue;
    }
    ...
    if ((ret = p_vectorIndex->LoadIndexDataFromMemory(p_indexBlobs)) !=
        ErrorCode::Success)
        return ret;

    size_t metaStart = p_vectorIndex->BufferSize()->size();
    if (iniReader.DoesSectionExist("MetaData") &&
        p_indexBlobs.size() >= metaStart + 2) {
        ByteArray pMetaIndex = p_indexBlobs[metaStart + 1];
        p_vectorIndex->SetMetadata(new MemMetadataSet(
            p_indexBlobs[metaStart],
            ByteArray(pMetaIndex.Data() + sizeof(SizeType),
                      pMetaIndex.Length() - sizeof(SizeType), false),
            *((SizeType*)pMetaIndex.Data()), p_vectorIndex->m_iDataBlockSize,
            p_vectorIndex->m_iDataCapacity, p_vectorIndex->m_iMetaRecordSize));
...
std::uint64_t VectorIndex::EstimatedVectorCount(
    std::uint64_t p_memory, DimensionType p_dimension,
    VectorValueType p_valuetype, SizeType p_vectorsInBlock, SizeType p_maxmeta,
    IndexAlgoType p_algo, int p_treeNumber, int p_neighborhoodSize) {
    size_t treeNodeSize;
    if (p_algo == IndexAlgoType::BKT) {
        treeNodeSize = sizeof(SizeType) * 3;
    } else if (p_algo == IndexAlgoType::KDT) {
        treeNodeSize =
            sizeof(SizeType) * 2 + sizeof(DimensionType) + sizeof(float);
    } else {
        return 0;
    }
    std::uint64_t unit = GetValueTypeSize(p_valuetype) * p_dimension +
                         p_maxmeta + sizeof(std::uint64_t) +
                         sizeof(SizeType) * p_neighborhoodSize + 1 +
                         treeNodeSize * p_treeNumber;
    return ((p_memory / unit) / p_vectorsInBlock) * p_vectorsInBlock;
}

std::uint64_t VectorIndex::EstimatedMemoryUsage(
    std::uint64_t p_vectorCount, DimensionType p_dimension,
    VectorValueType p_valuetype, SizeType p_vectorsInBlock, SizeType p_maxmeta,
    IndexAlgoType p_algo, int p_treeNumber, int p_neighborhoodSize) {
    p_vectorCount =
        ((p_vectorCount + p_vectorsInBlock - 1) / p_vectorsInBlock) *
        p_vectorsInBlock;
    size_t treeNodeSize;
    if (p_algo == IndexAlgoType::BKT) {
        treeNodeSize = sizeof(SizeType) * 3;
    ...
    std::uint64_t ret = GetValueTypeSize(p_valuetype) * p_dimension *
                        p_vectorCount;             // Vector Size
...
void VectorIndex::SortSelections(std::vector<Edge>* selections) {
    SPTAGLIB_LOG(Helper::LogLevel::LL_Debug,
                 "Starting sort of final input on GPU\n");
    GPU_SortSelections(selections);
}

void VectorIndex::ApproximateRNG(std::shared_ptr<VectorSet>& fullVectors,
                                 std::unordered_set<SizeType>& exceptIDS,
                                 int candidateNum, Edge* selections,
                                 int replicaCount, int numThreads, int numTrees,
                                 int leafSize, float RNGFactor, int numGPUs) {
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Starting GPU SSD Index build stage...\n");

    int metric = (GetDistCalcMethod() == SPTAG::DistCalcMethod::Cosine);

    if (m_pQuantizer) {
        getTailNeighborsTPT<uint8_t, float>(
            (uint8_t*)fullVectors->GetData(), fullVectors->Count(), this,
            exceptIDS, fullVectors->Dimension(), replicaCount, numThreads,
            numTrees, leafSize, metric, numGPUs, selections);
    } else if (GetVectorValueType() != VectorValueType::Float) {
        typedef int32_t SUMTYPE;
        switch (GetVectorValueType()) {
#define DefineVectorValueType(Name, Type)                                  \
    case VectorValueType::Name:                                            \
        getTailNeighborsTPT<Type, SUMTYPE>(                                \
            (Type*)fullVectors->GetData(), fullVectors->Count(), this,     \
            exceptIDS, fullVectors->Dimension(), replicaCount, numThreads, \
            numTrees, leafSize, metric, numGPUs, selections);              \
        break;
...
#undef DefineVectorValueType
        ...
        getTailNeighborsTPT<float, float>(
            (float*)fullVectors->GetData(), fullVectors->Count(), this,
            exceptIDS, fullVectors->Dimension(), replicaCount, numThreads,
            numTrees, leafSize, metric, numGPUs, selections);
    }
}
            ...
            while (true) {
                ...
                if (m_pQuantizer) {
                    reconstructed_vector =
                        ALIGN_ALLOC(m_pQuantizer->ReconstructSize());
                    m_pQuantizer->ReconstructVector(
                        (const uint8_t*)fullVectors->GetVector(fullID),
                        reconstructed_vector);
                    switch (m_pQuantizer->GetReconstructType()) {
#define DefineVectorValueType(Name, Type)                             \
    case VectorValueType::Name:                                       \
        (*((COMMON::QueryResultSet<Type>*)&resultSet))                \
            .SetTarget(reinterpret_cast<Type*>(reconstructed_vector), \
                       m_pQuantizer);                                 \
        break;
#include "inc/Core/DefinitionList.h"
#undef DefineVectorValueType
                        default:
                            SPTAGLIB_LOG(
                                ...
                                "Unable to get quantizer reconstruct type %s",
                                Helper::Convert::ConvertToString<
                                    VectorValueType>(
                                    m_pQuantizer->GetReconstructType()));
                    }
                } else {
                ...
                size_t selectionOffset =
                    static_cast<size_t>(fullID) * replicaCount;

                BasicResult* queryResults = resultSet.GetResults();
                int currReplicaCount = 0;
                for (int i = 0;
                     i < candidateNum && currReplicaCount < replicaCount; ++i) {
                    ...
                    for (int j = 0; j < currReplicaCount; ++j) {
                        float nnDist = ComputeDistance(
                            GetSample(queryResults[i].VID),
                    ...
                    selections[selectionOffset + currReplicaCount].node =
                        queryResults[i].VID;
                    selections[selectionOffset + currReplicaCount].distance =
                        queryResults[i].Dist;
                    ++currReplicaCount;
                }
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Searching replicas ended. RNG failed count: %llu\n",
