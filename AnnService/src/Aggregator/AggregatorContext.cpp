...
AggregatorContext::AggregatorContext(const std::string& p_filePath)
    ...
    m_settings->m_centers =
        iniReader.GetParameter("Service", "Centers", std::string("centers"));
    m_settings->m_valueType =
        iniReader.GetParameter("Service", "ValueType", VectorValueType::Float);
    m_settings->m_topK =
        iniReader.GetParameter("Service", "TopK", static_cast<SizeType>(-1));
    m_settings->m_distMethod =
        iniReader.GetParameter("Service", "DistCalcMethod", DistCalcMethod::L2);
    const std::string emptyStr;

    SizeType serverNum =
        iniReader.GetParameter("Servers", "Number", static_cast<SizeType>(0));
    ...
    if (m_settings->m_topK > 0) {
        ...
        std::uint64_t totalRecordVectorBytes =
            ((std::uint64_t)GetValueTypeSize(m_settings->m_valueType)) * row *
            col;
        ...
        m_centers.reset(
            new BasicVectorSet(vectorSet, m_settings->m_valueType, col, row));
    }
    m_initialized = true;
}
