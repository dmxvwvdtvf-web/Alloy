...
class KDTree {
   public:
    ...
    template <typename T>
    void BuildTrees(const Dataset<T>& data, int numOfThreads,
                    std::vector<SizeType>* indices = nullptr,
...
#undef DefineVectorValueType
    ...
    void BuildTreesCore(const Dataset<T>& data, int numOfThreads,
                        std::vector<SizeType>* indices = nullptr,
                        IAbortOperation* abort = nullptr) {
...
#pragma omp parallel for num_threads(numOfThreads)
        for (int i = 0; i < m_iTreeNumber; i++) {
            ...
            auto rg_local = get_rg_local(i);

            std::vector<SizeType> pindices(localindices.begin(),
                                           localindices.end());
            std::shuffle(pindices.begin(), pindices.end(), rg_local);

            m_pTreeStart[i] = i * (SizeType)pindices.size();
            SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                         "Start to build KDTree %d\n", i + 1);
            SizeType iTreeSize = m_pTreeStart[i];
            DivideTree<T, R>(data, pindices, 0, (SizeType)pindices.size() - 1,
                             m_pTreeStart[i], iTreeSize, abort);
    ...
    ErrorCode SaveTrees(std::shared_ptr<Helper::DiskIO> p_out) const {
        ...
        IOBINARY(p_out, WriteBinary, sizeof(treeNodeSize),
                 (char*)&treeNodeSize);
        IOBINARY(p_out, WriteBinary, sizeof(KDTNode) * treeNodeSize,
    ...
    ErrorCode LoadTrees(char* pKDTMemFile) {
        ...
        memcpy(m_pTreeStart.data(), pKDTMemFile,
        ...
        memcpy(m_pTreeRoots.data(), pKDTMemFile,
    ...
    ErrorCode LoadTrees(std::shared_ptr<Helper::DiskIO> p_input) {
        ...
        IOBINARY(p_input, ReadBinary, sizeof(treeNodeSize),
                 (char*)&treeNodeSize);
        m_pTreeRoots.resize(treeNodeSize);
        IOBINARY(p_input, ReadBinary, sizeof(KDTNode) * treeNodeSize,
                 (char*)m_pTreeRoots.data());
    ...
    void InitSearchTrees(const Dataset<T>& p_data,
                         std::function<float(const T*, const T*, DimensionType)>
                             fComputeDistance,
                         COMMON::QueryResultSet<T>& p_query,
                         COMMON::WorkSpace& p_space) const {
        for (int i = 0; i < m_iTreeNumber; i++) {
            KDTSearch<T, Q>(p_data, fComputeDistance, p_query, p_space,
                            m_pTreeStart[i], 0);
        }
    }

    template <typename T, typename Q>
    void SearchTrees(const Dataset<T>& p_data,
                     std::function<float(const T*, const T*, DimensionType)>
                         fComputeDistance,
                     COMMON::QueryResultSet<T>& p_query,
        ...
        while (!p_space.m_SPTQueue.empty() &&
            ...
            KDTSearch<T, Q>(p_data, fComputeDistance, p_query, p_space,
                            tcell.node, tcell.distance);
        }
    }

   private:
    template <typename T, typename Q>
    void KDTSearch(const Dataset<T>& p_data,
                   std::function<float(const T*, const T*, DimensionType)>
                       fComputeDistance,
                   COMMON::QueryResultSet<T>& p_query,
                   COMMON::WorkSpace& p_space, const SizeType node,
        ...
        if (node < 0) {
            ...
            p_space.m_NGQueue.insert(NodeDistPair(
                index, fComputeDistance(p_query.GetQuantizedTarget(), data,
                                        p_data.C())));
            return;
        }
        ...
        float diff =
            ((Q*)p_query.GetTarget())[tnode.split_dim] - tnode.split_value;
        ...
        KDTSearch<T, Q>(p_data, fComputeDistance, p_query, p_space, bestChild,
                        distBound);
    }

    template <typename T, typename R>
    void DivideTree(const Dataset<T>& data, std::vector<SizeType>& indices,
                    SizeType first, SizeType last, SizeType index,
                    SizeType& iTreeSize, IAbortOperation* abort = nullptr) {
        if (abort && abort->ShouldAbort()) return;

        ChooseDivision<T, R>(data, m_pTreeRoots[index], indices, first, last);
        SizeType i =
            Subdivide<T, R>(data, m_pTreeRoots[index], indices, first, last);
        if (i - 1 <= first) {
    ...
    template <typename T, typename R>
    void ChooseDivision(const Dataset<T>& data, KDTNode& node,
        ...
        for (SizeType j = first; j <= end; j++) {
            R* v;
            if (quantizer_exists) {
                m_pQuantizer->ReconstructVector((uint8_t*)data[indices[j]],
                                                v_holder);
                v = v_holder;
            } else {
                v = (R*)data[indices[j]];
        ...
        // choose the split dimension as one of the dimension inside TOP_DIM
        // maximum variance
    ...
    DimensionType SelectDivisionDimension(
        const std::vector<float>& varianceValues) const {
        // Record the top maximum variances
        std::vector<DimensionType> topind(m_numTopDimensionKDTSplit);
        int num = 0;
        // order the variances
        for (DimensionType i = 0; i < (DimensionType)varianceValues.size();
             i++) {
            if (num < m_numTopDimensionKDTSplit ||
                ...
                while (j > 0 && varianceValues[topind[j]] >
                                    varianceValues[topind[j - 1]]) {
    ...
    template <typename T, typename R>
    SizeType Subdivide(const Dataset<T>& data, const KDTNode& node,
                       std::vector<SizeType>& indices, const SizeType first,
                       const SizeType last) const {
        SizeType i = first;
        SizeType j = last;
