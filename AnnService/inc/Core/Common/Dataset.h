...
class Dataset {
   ...
   public:
    ...
    Dataset(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_,
            SizeType capacity_, T* data_ = nullptr,
            bool shareOwnership_ = true) {
        Initialize(rows_, cols_, rowsInBlock_, capacity_, data_,
                   shareOwnership_);
    }
    ~Dataset() {
    ...
    void Initialize(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_,
                    SizeType capacity_, T* data_ = nullptr,
                    bool shareOwnership_ = true) {
        rows = rows_;
        cols = cols_;
        ...
        incBlocks.reserve(
            (static_cast<std::int64_t>(capacity_) + rowsInBlock) >>
            rowsInBlockEx);
    }
    ...
    inline const T* At(SizeType index) const {
        if (index < R() && index >= 0) {
            if (index >= rows) {
                ...
                return incBlocks[incIndex >> rowsInBlockEx] +
                       ((size_t)(incIndex & rowsInBlock)) * cols;
            }
            return data + ((size_t)index) * cols;
        } else {
            std::ostringstream oss;
            oss << "Index out of range in Dataset. Index: " << index
                << " Size: " << R();
            throw std::out_of_range(oss.str());
        }
    }

    T* operator[](SizeType index) { return (T*)At(index); }

    const T* operator[](SizeType index) const { return At(index); }
    ...
    ErrorCode AddBatch(const T* pData, SizeType num) {
        ...
        while (written < num) {
            ...
            if (curBlockIdx >= (SizeType)incBlocks.size()) {
                T* newBlock = (T*)ALIGN_ALLOC(((size_t)rowsInBlock + 1) * cols *
                                              sizeof(T));
            ...
            SizeType toWrite =
                min(rowsInBlock + 1 - curBlockPos, num - written);
            std::memcpy(incBlocks[curBlockIdx] + ((size_t)curBlockPos) * cols,
                        pData + ((size_t)written) * cols,
                        ((size_t)toWrite) * cols * sizeof(T));
            written += toWrite;
        }
        incRows += written;
    ...
    ErrorCode AddBatch(SizeType num) {
        ...
        while (written < num) {
            ...
            if (curBlockIdx >= (SizeType)incBlocks.size()) {
                T* newBlock =
                    (T*)ALIGN_ALLOC(sizeof(T) * (rowsInBlock + 1) * cols);
                if (newBlock == nullptr) return ErrorCode::MemoryOverFlow;
                std::memset(newBlock, -1, sizeof(T) * (rowsInBlock + 1) * cols);
                incBlocks.push_back(newBlock);
            }
            written +=
                min(rowsInBlock + 1 - ((incRows + written) & rowsInBlock),
                    num - written);
        }
        incRows += written;
        return ErrorCode::Success;
    ...
    ErrorCode Save(std::shared_ptr<Helper::DiskIO> p_out) const {
        ...
        for (int i = 0; i < blocks; i++)
            IOBINARY(p_out, WriteBinary, sizeof(T) * cols * (rowsInBlock + 1),
                     (char*)incBlocks[i]);

        SizeType remain = (incRows & rowsInBlock);
        if (remain > 0)
            IOBINARY(p_out, WriteBinary, sizeof(T) * cols * remain,
                     (char*)incBlocks[blocks]);
    ...
    ErrorCode Save(std::string sDataPointsFileName) const {
        ...
        auto ptr = f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(sDataPointsFileName.c_str(),
                             std::ios::binary | std::ios::out))
            return ErrorCode::FailedCreateFile;
        return Save(ptr);
    }

    ErrorCode Load(std::shared_ptr<Helper::DiskIO> pInput, SizeType blockSize,
                   SizeType capacity) {
    ...
    ErrorCode Load(std::string sDataPointsFileName, SizeType blockSize,
                   SizeType capacity) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Load %s From %s\n",
                     name.c_str(), sDataPointsFileName.c_str());
        auto ptr = f_createIO();
        if (ptr == nullptr || !ptr->Initialize(sDataPointsFileName.c_str(),
    ...
    ErrorCode Load(char* pDataPointsMemFile, SizeType blockSize,
                   SizeType capacity) {
        SizeType R;
        DimensionType C;
    ...
    ErrorCode Refine(const std::vector<SizeType>& indices,
                     Dataset<T>& p_data) const {
        SizeType R = (SizeType)(indices.size());
        p_data.Initialize(
            R, cols, rowsInBlock + 1,
            static_cast<SizeType>(incBlocks.capacity() * (rowsInBlock + 1)));
        for (SizeType i = 0; i < R; i++) {
            std::memcpy((void*)p_data.At(i), (void*)this->At(indices[i]),
                        sizeof(T) * cols);
        }
        return ErrorCode::Success;
    }
    ...
    ErrorCode Refine(const std::vector<SizeType>& indices,
                     std::shared_ptr<Helper::DiskIO> output) const {
        SizeType R = (SizeType)(indices.size());
        IOBINARY(output, WriteBinary, sizeof(SizeType), (char*)&R);
        IOBINARY(output, WriteBinary, sizeof(DimensionType), (char*)&cols);

        for (SizeType i = 0; i < R; i++) {
            IOBINARY(output, WriteBinary, sizeof(T) * cols,
                     (char*)At(indices[i]));
        }
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "Save Refine %s (%d,%d) Finish!\n", name.c_str(), R, cols);
        return ErrorCode::Success;
    }

    ErrorCode Refine(const std::vector<SizeType>& indices,
                     std::string sDataPointsFileName) const {
