...
p_space.m_iNumberOfCheckedLeaves > p_space.m_iMaxCheck) { \
    ...
    for (DimensionType i = 0; i < m_pGraph.m_iNeighborhoodSize; i++) { \
        ...
        float distance2leaf = m_fComputeDistance(p_query.GetQuantizedTarget(),
...
p_space.m_iNumberOfCheckedLeaves / 10) { \
            m_pTrees.SearchTrees<T,Q>(m_pSamples, m_fComputeDistance, p_query,
...
void Index<T>::Search(COMMON::QueryResultSet<T>& p_query,
    ...
    std::shared_lock<std::shared_timed_mutex> lock(*(m_pTrees.m_lock));
    m_pTrees.InitSearchTrees<T, Q>(m_pSamples, m_fComputeDistance, p_query,
                                   p_space);
    m_pTrees.SearchTrees<T, Q>(m_pSamples, m_fComputeDistance, p_query, p_space,
                               m_iNumberOfInitialDynamicPivots);
    while (!p_space.m_NGQueue.empty()) {
        ...
        if (p_space.m_iNumOfContinuousNoBetterPropagation >
            ...
            if (p_space.m_iNumberOfTreeCheckedLeaves <=
                p_space.m_iNumberOfCheckedLeaves / 10) {
                m_pTrees.SearchTrees<T, Q>(
                    m_pSamples, m_fComputeDistance, p_query, p_space,
...
template <typename T>
template <typename Q>
void Index<T>::SearchIndex(COMMON::QueryResultSet<T>& p_query,
...
bool Index<T>::SearchIndexIterativeFromNeareast(QueryResult& p_query,
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                 "SearchIndexIterativeFromNeareast NOT SUPPORT FOR KDT");
    return false;
}
...
#define DefineVectorValueType(Name, Type)                                 \
    case VectorValueType::Name:                                           \
        m_pTrees.InitSearchTrees<T, Type>(m_pSamples, m_fComputeDistance, \
                                          *p_results, *workSpace);        \
        m_pTrees.SearchTrees<T, Type>(m_pSamples, m_fComputeDistance,     \
                                      *p_results, *workSpace,             \
                                      m_iNumberOfInitialDynamicPivots);   \
        break;
...
#undef DefineVectorValueType
        ...
        m_pTrees.InitSearchTrees<T, T>(m_pSamples, m_fComputeDistance,
                                       *p_results, *workSpace);
        m_pTrees.SearchTrees<T, T>(m_pSamples, m_fComputeDistance, *p_results,
                                   *workSpace, m_iNumberOfInitialDynamicPivots);
    }
...
ErrorCode Index<T>::BuildIndex(const void* p_data, SizeType p_vectorNum,
    ...
    if (DistCalcMethod::Cosine == m_iDistCalcMethod && !p_normalized) {
        int base = m_pQuantizer ? m_pQuantizer->GetBase()
...
#pragma omp parallel for
    ...
    if ((ret = m_pGraph.RefineGraph<T>(this, indices, reverseIndices,
                                       p_indexStreams[2], nullptr)) !=
        ErrorCode::Success)
        return ret;
