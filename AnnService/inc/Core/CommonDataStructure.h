...
template <typename T>
Array<T>::Array(T* p_array, std::size_t p_length,
                std::shared_ptr<T> p_dataHolder)
    : m_data(p_array),
      m_length(p_length),
...
template <typename T>
Array<T>& Array<T>::operator=(Array<T>&& p_right) {
    m_data = p_right.m_data;
    m_length = p_right.m_length;
    m_dataHolder = std::move(p_right.m_dataHolder);
...
template <typename T>
Array<T>& Array<T>::operator=(const Array<T>& p_right) {
    m_data = p_right.m_data;
    m_length = p_right.m_length;
    m_dataHolder = p_right.m_dataHolder;
...
template <typename T>
const T& Array<T>::operator[](std::size_t p_index) const {
    return m_data[p_index];
}
...
template <typename T>
std::shared_ptr<T> Array<T>::DataHolder() const {
    return m_dataHolder;
}
...
template <typename T>
Array<T> Array<T>::Alloc(std::size_t p_length) {
    Array<T> arr;
    if (0 == p_length) {
        return arr;
    }
