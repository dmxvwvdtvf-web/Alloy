...
class Utils {
   public:
    static SizeType rand(SizeType high = MaxSize,
                         SizeType low = 0)  // Generates a random int value.
    {
        return low +
               (SizeType)(float(high - low) * (std::rand() / (RAND_MAX + 1.0)));
    }
    ...
    template <typename T>
    static void BatchNormalize(T* data, SizeType row, DimensionType col,
                               int base, int threads);

    static inline void AddNeighbor(SizeType idx, float dist,
                                   SizeType* neighbors, float* dists,
                                   DimensionType size) {
        size--;
        if (dist < dists[size] ||
            (dist == dists[size] && idx < neighbors[size])) {
            DimensionType nb;
