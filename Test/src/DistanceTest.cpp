...
void test(int high) {
    ...
    BOOST_CHECK_CLOSE_FRACTION(ComputeL2Distance(X, Y, dimension),
                               SPTAG::COMMON::DistanceUtils::ComputeDistance(
                                   X, Y, dimension, SPTAG::DistCalcMethod::L2),
                               1e-5);
    BOOST_CHECK_CLOSE_FRACTION(
        high * high - ComputeCosineDistance(X, Y, dimension),
        SPTAG::COMMON::DistanceUtils::ComputeDistance(
            X, Y, dimension, SPTAG::DistCalcMethod::Cosine),
        1e-5);
...
void test_dist_calc_performance(
    int high, SPTAG::DimensionType dimension = 256, SPTAG::SizeType size = 100,
...
#pragma omp parallel for
    for (SPTAG::SizeType i = 0; i < size; i++) {
        SPTAG::COMMON::DistanceUtils::ComputeDistance(X[i], Y[i], dimension,
                                                      calc_method);
    }
    end = omp_get_wtime();
    std::cout << "Time to calculate distance (ms): " << (end - start) * 1000
              << std::endl;
...
BOOST_AUTO_TEST_CASE(TestDistanceComputationPerformance) {
    ...
    std::vector<SPTAG::DistCalcMethod> calc_methods{
        SPTAG::DistCalcMethod::L2, SPTAG::DistCalcMethod::Cosine};
    ...
    for (int num_threads : nums_threads) {
        ...
        for (SPTAG::DistCalcMethod calc_method : calc_methods) {
            std::cout << "calc_method: "
                      << (calc_method == SPTAG::DistCalcMethod::L2 ? "L2"
                                                                   : "Cosine")
                      << std::endl;
            for (auto dimension : dimensions) {
                std::cout << "type: int8, dimension: " << dimension
                          << ", size: " << size << std::endl;
                test_dist_calc_performance<std::int8_t>(127, dimension, size,
                                                        calc_method);
            }
        }
    }
