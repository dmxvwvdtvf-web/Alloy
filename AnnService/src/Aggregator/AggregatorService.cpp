...
void AggregatorService::StartClient() {
    ...
    handlerMap->emplace(
        ...
        [this](Socket::ConnectionID p_srcID, Socket::Packet p_packet) {
            boost::asio::post(
                *m_threadPool,
                std::bind(&AggregatorService::SearchResponseHanlder, this,
    ...
    m_socketClient->SetEventOnConnectionClose(
        [this](Socket::ConnectionID p_cid) {
...
void AggregatorService::StartListen() {
    ...
    handlerMap->emplace(
        ...
        [this](Socket::ConnectionID p_srcID, Socket::Packet p_packet) {
            boost::asio::post(
                *m_threadPool,
                std::bind(&AggregatorService::SearchRequestHanlder, this,
                          p_srcID, std::move(p_packet)));
        });
...
void AggregatorService::WaitForShutdown() {
    ...
    m_shutdownSignals.async_wait([this](boost::system::error_code p_ec,
                                        int p_signal) {
...
void AggregatorService::ConnectToPendingServers() {
    ...
    m_pendingConnectServersTimer.expires_from_now(
...
void AggregatorService::SearchRequestHanlder(
    ...
    if (context->GetSettings()->m_topK > 0 &&
...
#define DefineVectorValueType(Name, Type)                                     \
    case VectorValueType::Name:                                               \
        if (!queryParser.GetVectorElements().empty()) {                       \
            Service::ConvertVectorFromString<Type>(                           \
                queryParser.GetVectorElements(), vector, vectorDimension);    \
        } else if (queryParser.GetVectorBase64() != nullptr &&                \
                   queryParser.GetVectorBase64Length() != 0) {                \
            vector = ByteArray::Alloc(Helper::Base64::CapacityForDecode(      \
                queryParser.GetVectorBase64Length()));                        \
            Helper::Base64::Decode(queryParser.GetVectorBase64(),             \
                                   queryParser.GetVectorBase64Length(),       \
                                   vector.Data(), vectorSize);                \
            vectorDimension =                                                 \
                (SizeType)(vectorSize /                                       \
                           GetValueTypeSize(                                  \
                               context->GetSettings()->m_valueType));         \
        }                                                                     \
        for (int i = 0; i < context->GetCenters()->Count(); i++) {            \
            servers.push_back(BasicResult(                                    \
                i,                                                            \
                COMMON::DistanceUtils::ComputeDistance(                       \
                    (Type*)vector.Data(),                                     \
                    (Type*)context->GetCenters()->GetVector(i),               \
...
#undef DefineVectorValueType
        ...
        std::sort(servers.begin(), servers.end(),
                  [](const BasicResult& a, const BasicResult& b) {
    ...
    for (std::uint32_t i = 0; i < remoteServers.size(); ++i) {
        ...
        auto timeoutCallback =
            [](std::shared_ptr<AggregatorCallback> p_callback) {
                if (nullptr != p_callback) {
                    Socket::RemoteSearchResult result;
                    result.m_status =
                        Socket::RemoteSearchResult::ResultStatus::Timeout;
...
void AggregatorService::SearchResponseHanlder(
    Socket::ConnectionID p_localConnectionID, Socket::Packet p_packet) {
    auto callback = m_aggregatorCallbackManager.GetAndRemove(
        p_packet.Header().m_resourceID);
    if (nullptr == callback) {
        return;
    }
    ...
    if (p_packet.Header().m_processStatus != Socket::PacketProcessStatus::Ok ||
        ...
        Socket::RemoteSearchResult result;
        result.m_status =
            Socket::RemoteSearchResult::ResultStatus::FailedExecute;
