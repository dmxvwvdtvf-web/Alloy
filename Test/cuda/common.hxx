...
#define GPU_CHECK_VAL(val, exp, dtype, errs)                                   \
    ...
    float eps = 0.01;                                                          \
    if ((float)temp > 0.0 && ((float)temp * (1.0 + eps) < (float)(exp) ||      \
                              (float)temp * (1.0 - eps) > (float)(exp))) {     \
        errs++;                                                                \
        SPTAGLIB_LOG(SPTAG::Helper::LogLevel::LL_Error, "%s != %s\n", #val,    \
                     #exp);                                                    \
    }
...
/*
__global__ void count_leaf_sizes(LeafNode* leafs, int* node_ids, int N, int
internal_nodes);
__global__ void assign_leaf_points_in_batch(LeafNode* leafs, int* leaf_points,
int* node_ids, int N, int internal_nodes, int min_id, int max_id);
__global__ void assign_leaf_points_out_batch(LeafNode* leafs, int* leaf_points,
int* node_ids, int N, int internal_nodes, int min_id, int max_id);
__global__ void compute_mean(KEYTYPE* split_keys, int* node_sizes, int
num_nodes);
