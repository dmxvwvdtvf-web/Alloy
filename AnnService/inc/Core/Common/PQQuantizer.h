...
class PQQuantizer : public IQuantizer {
   public:
    ...
    PQQuantizer(DimensionType NumSubvectors, SizeType KsPerSubvector,
                DimensionType DimPerSubvector, bool EnableADC,
                std::unique_ptr<T[]>&& Codebooks);

    ~PQQuantizer();

    virtual float L2Distance(const std::uint8_t* pX,
                             const std::uint8_t* pY) const;

    virtual float CosineDistance(const std::uint8_t* pX,
                                 const std::uint8_t* pY) const;
...
template <typename T>
PQQuantizer<T>::PQQuantizer(DimensionType NumSubvectors,
    ...
    : m_NumSubvectors(NumSubvectors),
      ...
      m_BlockSize(KsPerSubvector * KsPerSubvector),
      m_codebooks(std::move(Codebooks)),
...
template <typename T>
float PQQuantizer<T>::L2Distance(const std::uint8_t* pX,
                                 const std::uint8_t* pY) const
...
template <typename T>
float PQQuantizer<T>::CosineDistance(const std::uint8_t* pX,
                                     const std::uint8_t* pY) const
// pX must be query distance table for ADC
{
...
template <typename T>
void PQQuantizer<T>::ReconstructVector(const std::uint8_t* qvec,
                                       void* vecout) const {
    T* sub_vecout = (T*)vecout;
    T* subcodebooks = m_codebooks.get();
    for (int i = 0; i < m_NumSubvectors; i++) {
        memcpy(sub_vecout, subcodebooks + qvec[i] * m_DimPerSubvector,
               sizeof(T) * m_DimPerSubvector);
...
std::uint64_t PQQuantizer<T>::BufferSize() const {
    return sizeof(T) * m_NumSubvectors * m_KsPerSubvector * m_DimPerSubvector +
           sizeof(DimensionType) + sizeof(SizeType) + sizeof(DimensionType) +
           sizeof(VectorValueType) + sizeof(QuantizerType);
}

template <typename T>
ErrorCode PQQuantizer<T>::SaveQuantizer(
    std::shared_ptr<Helper::DiskIO> p_out) const {
    ...
    IOBINARY(p_out, WriteBinary,
             sizeof(T) * m_NumSubvectors * m_KsPerSubvector * m_DimPerSubvector,
...
ErrorCode PQQuantizer<T>::LoadQuantizer(std::shared_ptr<Helper::DiskIO> p_in) {
    ...
    IOBINARY(p_in, ReadBinary, sizeof(DimensionType),
             (char*)&m_DimPerSubvector);
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "After read dim: %s.\n",
                 std::to_string(m_DimPerSubvector).c_str());
    m_codebooks = std::make_unique<T[]>(m_NumSubvectors * m_KsPerSubvector *
                                        m_DimPerSubvector);
    ...
    InitializeDistanceTables();
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Loaded quantizer: Subvectors:%d KsPerSubvector:%d "
                 "DimPerSubvector:%d\n",
                 m_NumSubvectors, m_KsPerSubvector, m_DimPerSubvector);
    return ErrorCode::Success;
}
...
template <typename T>
inline SizeType PQQuantizer<T>::m_DistIndexCalc(SizeType i, SizeType j,
                                                SizeType k) const {
...
void PQQuantizer<T>::InitializeDistanceTables() {
    auto temp_m_L2DistanceTables =
        std::make_unique<float[]>(m_BlockSize * m_NumSubvectors);
    ...
    for (int i = 0; i < m_NumSubvectors; i++) {
        ...
        for (int j = 0; j < m_KsPerSubvector; j++) {
            for (int k = 0; k < m_KsPerSubvector; k++) {
                temp_m_L2DistanceTables[m_DistIndexCalc(i, j, k)] =
                    L2Dist(&m_codebooks[baseIdx + j * m_DimPerSubvector],
                           &m_codebooks[baseIdx + k * m_DimPerSubvector],
                           m_DimPerSubvector);
            }
        }
    }
