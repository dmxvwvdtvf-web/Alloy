...
void CalcLeafSize(int p_nodeID, const std::shared_ptr<COMMON::BKTree> p_tree,
...
void DfsAnalyze(int p_nodeID, const std::shared_ptr<COMMON::BKTree> p_tree,
                std::shared_ptr<VectorSet> p_vectorSet,
                const SPANN::Options& p_opts, int p_height,
    ...
    for (int nodeId = node.childStart; nodeId < node.childEnd; ++nodeId) {
        DfsAnalyze(nodeId, p_tree, p_vectorSet, p_opts, p_height + 1,
                   p_nodeInfos);
        ...
        if (p_nodeInfos[nodeId].maxDepth == 1 &&
            p_nodeInfos[nodeId].leafSize == 1) {
            ++sinlgeCount;
        }
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "CheckNode: %8d, Height: %3d, MinDepth: %3d, MaxDepth: %3d, "
                 "Children: %3d, Single: %3d\n",
                 p_nodeID, p_height, minDepth, maxDepth,
                 node.childEnd - node.childStart, sinlgeCount);

    for (int nodeId = node.childStart; nodeId < node.childEnd; ++nodeId) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "    ChildNode: %8d, MinDepth: %3d, MaxDepth: %3d, "
                     "ChildrenCount: %3d, LeafCount: %3d\n",
                     nodeId, p_nodeInfos[nodeId].minDepth,
...
void DfsSelect(int p_nodeID,
    ...
    p_candidates.emplace_back(p_nodeID, p_nodeInfos[p_nodeID].leafSize,
                              minDepth);
}
...
int DfsCovered(int p_nodeID,
               const std::shared_ptr<SPTAG::COMMON::BKTree> p_tree,
               const std::unordered_set<int>& p_candidates, bool p_covered) {
    const auto& node = (*p_tree)[p_nodeID];
...
void SelectHeadStatically(const std::shared_ptr<COMMON::BKTree> p_tree,
                          const int p_vectorCount, const SPANN::Options& p_opts,
    ...
    for (const auto& c : candidates) {
        if (c.depth > nextDepth) {
            ...
            if (p_opts.m_recursiveCheckSmallCluster && !needRevisit.empty()) {
                while (!needRevisit.empty()) {
                    ...
                    if (bktNodeInfos[nodeId].leafSize >=
                ...
                SPTAGLIB_LOG(
                    ...
                    static_cast<uint32_t>(overallSelected.size() -
                                          lastSelected),
                    overallSelected.size() * 100.0 / p_vectorCount);
        ...
        if (c.depth <= 1) {
            ...
            if (c.childrenSize > p_opts.m_splitThreshold) {
                int selectCount = static_cast<int>(
                    std::ceil(c.childrenSize * 1.0 / p_opts.m_splitFactor) +
                    0.5);
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 ...
                 overallLeafCount, bigClusterCount);
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Leaf sum count > set leaf size: %d\n",
...
int SelectHeadDynamicallyInternal(const std::shared_ptr<COMMON::BKTree> p_tree,
                                  int p_nodeID, const SPANN::Options& p_opts,
                                  std::vector<int>& p_selected) {
...
void SelectHeadDynamically_Old(const std::shared_ptr<COMMON::BKTree> p_tree,
                               int p_vectorCount, const SPANN::Options& p_opts,
                               std::vector<int>& p_selected) {
    p_selected.clear();
    p_selected.reserve(p_vectorCount);

    if (static_cast<int>(std::round(p_opts.m_ratio * p_vectorCount)) >=
        p_vectorCount) {
...
void SelectHeadDynamically(const std::shared_ptr<COMMON::BKTree> p_tree,
    ...
    while (1 <= trySelect && trySelect <= p_vectorCount) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "Start try Select Threshold: %d\n", trySelect);

        opts.m_selectThreshold = trySelect;

        if (usedTrySelect.count(trySelect) == 0) {
            int l = p_opts.m_splitFactor;
            int r = max(p_opts.m_splitFactor + 2,
                        static_cast<int>(trySelect * (c_maxSSRatio + 1)));
            ...
            while (l < r - 1) {
                ...
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                             ...
                             opts.m_splitThreshold, diff * 100.0);
        ...
        SPTAGLIB_LOG(
            Helper::LogLevel::LL_Info,
            "Best abs(diff): %.3lf%%, target offset %.3lf, SSRatio: %.3lf\n\n",
...
ErrorCode SelectHead(std::shared_ptr<VectorSet> vectorSet,
                     std::shared_ptr<COMMON::BKTree> bkt, SPANN::Options& opts,
    ...
    if (opts.m_calcStd) {
        ...
        for (SizeType& item : selected) {
            if (counter.count(item) <= 0) {
                SPTAGLIB_LOG(
                    ...
                    "selected node: %d can't be used to calculate std.", item);
                return ErrorCode::Fail;
            }
        ...
        if (sum != selected.size()) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "please recheck std computation. \n");
            return ErrorCode::Fail;
        }
...
template <typename T>
std::shared_ptr<COMMON::BKTree> BuildBKT(std::shared_ptr<VectorSet> p_vectorSet,
                                         const SPANN::Options& opts) {
    ...
    COMMON::Dataset<T> data(p_vectorSet->Count(), p_vectorSet->Dimension(),
                            dataRowsInBlock, dataCapacity,
                            (T*)(p_vectorSet->GetData()));
    bkt->BuildTrees<T>(data, opts.m_distCalcMethod, opts.m_iNumberOfThreads,
    ...
    bktFileNameBuilder << opts.m_vectorPath << ".bkt." << opts.m_iBKTKmeansK
                       << "_" << opts.m_iBKTLeafSize << "_"
                       << opts.m_iTreeNumber << "_" << opts.m_iSamples << "_"
...
void AdjustOptions(SPANN::Options& p_opts, int p_vectorCount) {
    ...
    if (headCnt == 0) {
        for (double minCnt = 1; headCnt == 0; minCnt += 0.2) {
            ...
            headCnt =
                static_cast<int>(std::round(p_opts.m_ratio * p_vectorCount));
        }
    ...
    if (p_opts.m_selectThreshold == 0) {
        p_opts.m_selectThreshold =
            min(p_vectorCount - 1, static_cast<int>(1 / p_opts.m_ratio));
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Set SelectThreshold to %d\n",
                     p_opts.m_selectThreshold);
    }

    if (p_opts.m_splitThreshold == 0) {
        p_opts.m_splitThreshold = min(
            p_vectorCount - 1, static_cast<int>(p_opts.m_selectThreshold * 2));
    ...
    if (p_opts.m_splitFactor == 0) {
        p_opts.m_splitFactor =
            min(p_vectorCount - 1,
                static_cast<int>(std::round(1 / p_opts.m_ratio) + 0.5));
...
ErrorCode Bootstrap(SPANN::Options& opts) {
    ...
    auto valueType = COMMON::DistanceUtils::Quantizer
                         ? SPTAG::VectorValueType::UInt8
                         : opts.m_valueType;
    std::shared_ptr<Helper::ReaderOptions> options(new Helper::ReaderOptions(
        valueType, opts.m_dim, opts.m_vectorType, opts.m_vectorDelimiter));
    ...
    if (Helper::StrUtils::StrEqualIgnoreCase(opts.m_selectType.c_str(),
                                             "Random")) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
        ...
        int headCnt =
            static_cast<int>(std::round(opts.m_ratio * vectorSet->Count()));
        selected.resize(headCnt);

    } else if (Helper::StrUtils::StrEqualIgnoreCase(opts.m_selectType.c_str(),
                                                    "Clustering")) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "Start generating Clustering head.\n");
        int headCnt =
            static_cast<int>(std::round(opts.m_ratio * vectorSet->Count()));
...
#undef DefineVectorValueType
        ...
        if (opts.m_analyzeOnly) {
            ...
            if (SelectHead(vectorSet, bkt, opts, counter, selected) !=
                ErrorCode::Success) {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
    ...
    if (!opts.m_noOutput) {
        ...
        if (output == nullptr || outputIDs == nullptr ||
            !output->Initialize(opts.m_headVectorFile.c_str(),
                                std::ios::binary | std::ios::out) ||
            !outputIDs->Initialize(opts.m_headIDFile.c_str(),
                                   std::ios::binary | std::ios::out)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Failed to create output file:%s %s\n",
                         opts.m_headVectorFile.c_str(),
                         opts.m_headIDFile.c_str());
            exit(1);
        }
        ...
        for (auto& ele : selected) {
            ...
            if (output->WriteBinary(
                    vectorSet->PerVectorDataSize(),
                    (char*)(vectorSet->GetVector((SizeType)vid))) !=
                vectorSet->PerVectorDataSize()) {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                             "Failed to write output file!\n");
                exit(1);
