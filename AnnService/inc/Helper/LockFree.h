   ...
   public:
    ...
    bool assign(const T* begin, const T* end) {
        ...
        while (written < length) {
            ...
            std::memcpy(m_blocks[currBlock] + curBlockPos, begin + written,
                        toWrite * sizeof(T));
            written += toWrite;
        }
        m_size += written;
    ...
    ByteArray copy(std::uint64_t offset, size_t length) const {
        ...
        while (copy < length) {
            ...
            std::memcpy(
                b.Data() + copy * sizeof(T),
                m_blocks[(offset + copy) >> m_blockSizeEx] + blockOffset,
                toCopy * sizeof(T));
            copy += toCopy;
        }
        return b;
    ...
    bool save(std::shared_ptr<Helper::DiskIO> out) {
        ...
        for (int i = 0; i < blockNum; i++)
            if (out->WriteBinary(sizeof(T) * (m_blockSize + 1),
                                 (char*)(m_blocks[i])) !=
                sizeof(T) * (m_blockSize + 1))
                return false;

        auto remainNum = (m_size & m_blockSize);
        if (remainNum > 0 && out->WriteBinary(sizeof(T) * remainNum,
                                              (char*)(m_blocks[blockNum])) !=
                                 sizeof(T) * remainNum)
            return false;
        return true;
    }
    ...
    bool load(std::shared_ptr<Helper::DiskIO> in, size_t length) {
        ...
        while (written < length) {
            ...
            auto curBlockPos = ((m_size + written) & m_blockSize);
            auto toWrite = min(m_blockSize + 1 - curBlockPos, length - written);
            if (in->ReadBinary(sizeof(T) * toWrite,
                               (char*)(m_blocks[currBlock] + curBlockPos)) !=
                sizeof(T) * toWrite)
                return false;
            written += toWrite;
        }
