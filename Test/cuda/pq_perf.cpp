...
/*
template <typename T>
void Search(std::shared_ptr<VectorIndex>& vecIndex, std::shared_ptr<VectorSet>&
queryset, int k, std::shared_ptr<VectorSet>& truth)
{
    std::vector<SPTAG::COMMON::QueryResultSet<T>> res(queryset->Count(),
SPTAG::COMMON::QueryResultSet<T>(nullptr, k * 2)); auto t1 =
std::chrono::high_resolution_clock::now(); for (SizeType i = 0; i <
queryset->Count(); i++)
    {
        res[i].Reset();
        res[i].SetTarget((const T*)queryset->GetVector(i),
vecIndex->m_pQuantizer); vecIndex->SearchIndex(res[i]);
    }
...
(float)(queryset->Count())) << "us" << std::endl;
    ...
    for (SizeType i = 0; i < queryset->Count(); i++) {
        ...
        for (int j = 0; j < truthDimension; j++) {
            float truthdist =
vecIndex->ComputeDistance(res[i].GetQuantizedTarget(),
vecIndex->GetSample(nn[j])); for (int l = 0; l < k*2; l++) { if (visited[l])
continue;

                //std::cout << res[i].GetResult(l)->Dist << " " << truthdist <<
std::endl; if (res[i].GetResult(l)->VID == nn[j]) { recall += 1.0; visited[l] =
true; break;
                }
...
/*
template<typename T>
std::shared_ptr<VectorIndex> PerfBuild(IndexAlgoType algo, std::string
distCalcMethod, std::shared_ptr<VectorSet>& vec, std::shared_ptr<MetadataSet>&
meta, std::shared_ptr<VectorSet>& queryset, int k, std::shared_ptr<VectorSet>&
truth, std::string out, std::shared_ptr<COMMON::IQuantizer> quantizer)
{
...
true)); BOOST_CHECK(SPTAG::ErrorCode::Success == vecIndex->SaveIndex(out));
...
template <typename R>
void GenerateReconstructData(std::shared_ptr<VectorSet>& real_vecset,
                             std::shared_ptr<VectorSet>& rec_vecset,
                             std::shared_ptr<VectorSet>& quan_vecset,
                             std::shared_ptr<MetadataSet>& metaset,
                             std::shared_ptr<VectorSet>& queryset,
                             std::shared_ptr<VectorSet>& truth,
                             DistCalcMethod distCalcMethod, int k,
    ...
    if (fileexists("quantest_vector.bin") && fileexists("quantest_query.bin")) {
        std::shared_ptr<Helper::ReaderOptions> options(
            new Helper::ReaderOptions(GetEnumValueType<R>(), m,
                                      VectorFileType::DEFAULT));
        auto vectorReader = Helper::VectorSetReader::CreateInstance(options);
        if (ErrorCode::Success !=
            vectorReader->LoadFile("quantest_vector.bin")) {
        ...
        if (ErrorCode::Success !=
            vectorReader->LoadFile("quantest_query.bin")) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
        ...
        real_vecset.reset(
            new BasicVectorSet(real_vec, GetEnumValueType<R>(), m, n));
        ...
        queryset.reset(
            new BasicVectorSet(real_query, GetEnumValueType<R>(), m, q));
        //        queryset->Save("quantest_query.bin");
    }

    if (fileexists(("quantest_truth." +
                    SPTAG::Helper::Convert::ConvertToString(distCalcMethod))
                       .c_str())) {
        std::shared_ptr<Helper::ReaderOptions> options(
            new Helper::ReaderOptions(GetEnumValueType<float>(), k,
                                      VectorFileType::DEFAULT));
        auto vectorReader = Helper::VectorSetReader::CreateInstance(options);
        if (ErrorCode::Success !=
            vectorReader->LoadFile(
                "quantest_truth." +
                SPTAG::Helper::Convert::ConvertToString(distCalcMethod))) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
        ...
        ByteArray tru =
            ByteArray::Alloc(sizeof(SizeType) * queryset->Count() * k);
...
#pragma omp parallel for
        ...
        truth.reset(new BasicVectorSet(tru, GetEnumValueType<float>(), k,
                                       queryset->Count()));
        //        truth->Save("quantest_truth." +
        //        SPTAG::Helper::Convert::ConvertToString(distCalcMethod));
    }

    if (fileexists(CODEBOOK_FILE.c_str()) &&
        fileexists("quantest_quan_vector.bin") &&
        fileexists("quantest_rec_vector.bin")) {
        auto ptr = SPTAG::f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(CODEBOOK_FILE.c_str(),
        ...
        auto vectorReader = Helper::VectorSetReader::CreateInstance(options);
        if (ErrorCode::Success !=
            vectorReader->LoadFile("quantest_rec_vector.bin")) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Failed to read vector file.\n");
            exit(1);
        }
        ...
        std::shared_ptr<Helper::ReaderOptions> quanOptions(
            new Helper::ReaderOptions(GetEnumValueType<std::uint8_t>(), M,
                                      VectorFileType::DEFAULT));
        vectorReader = Helper::VectorSetReader::CreateInstance(quanOptions);
        if (ErrorCode::Success !=
            vectorReader->LoadFile("quantest_quan_vector.bin")) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Failed to read vector file.\n");
            exit(1);
        }
        ...
        std::unique_ptr<R[]> codebooks =
            std::make_unique<R[]>(M * Ks * QuanDim);
        std::unique_ptr<int[]> belong(new int[n]);
        for (int i = 0; i < M; i++) {
            R* kmeans = codebooks.get() + i * Ks * QuanDim;
            for (int j = 0; j < Ks; j++) {
                std::memcpy(kmeans + j * QuanDim, vecs + j * m + i * QuanDim,
                            sizeof(R) * QuanDim);
            }
            int cnt = 100;
            while (cnt--) {
                ...
                for (int ii = 0; ii < n; ii++) {
                    ...
                    for (int jj = 0; jj < Ks; jj++) {
                        double now_dis = COMMON::DistanceUtils::ComputeDistance(
                            vecs + ii * m + i * QuanDim, kmeans + jj * QuanDim,
                            QuanDim, DistCalcMethod::L2);
                        if (now_dis < min_dis) {
                            min_dis = now_dis;
                            min_id = jj;
                        }
...
#pragma omp parallel for
                    ...
                    for (int jj = 0; jj < n; jj++) {
                        if (belong[jj] == ii) {
                            ...
                            for (int kk = 0; kk < QuanDim; kk++) {
                                kmeans[ii * QuanDim + kk] +=
                                    vecs[jj * m + i * QuanDim + kk];
                            }
                        }
                    }
        ...
        quantizer = std::make_shared<SPTAG::COMMON::PQQuantizer<R>>(
            M, Ks, QuanDim, false, std::move(codebooks));
        ...
        if (!ptr->Initialize(CODEBOOK_FILE.c_str(),
                             std::ios::binary | std::ios::in)) {
            BOOST_ASSERT("Canot Open CODEBOOK_FILE to read!" == "Error");
        }
        quantizer->LoadIQuantizer(ptr);
        ...
        rec_vecset.reset(new BasicVectorSet(ByteArray::Alloc(sizeof(R) * n * m),
                                            GetEnumValueType<R>(), m, n));
        quan_vecset.reset(
            new BasicVectorSet(ByteArray::Alloc(sizeof(std::uint8_t) * n * M),
                               GetEnumValueType<std::uint8_t>(), M, n));
        for (int i = 0; i < n; i++) {
            auto nvec = &vecs[i * m];
            quantizer->QuantizeVector(nvec,
                                      (uint8_t*)quan_vecset->GetVector(i));
            quantizer->ReconstructVector((uint8_t*)quan_vecset->GetVector(i),
                                         rec_vecset->GetVector(i));
        }
...
void GPU_nopq_alltype(std::shared_ptr<VectorSet>& real_vecset,
                      DistCalcMethod distMethod, int* res_idx, float* res_dist);

void GPU_pq_alltype(std::shared_ptr<VectorSet>& real_vecset,
                    std::shared_ptr<VectorSet>& quan_vecset,
                    DistCalcMethod distMethod,
                    std::shared_ptr<COMMON::IQuantizer>& quantizer,
                    int* res_idx, float* res_dist);

bool DEBUG_REPORT = false;

template <typename R>
void CPU_top1_nopq(std::shared_ptr<VectorSet>& real_vecset,
                   DistCalcMethod distMethod, int numThreads, int* result,
...
#pragma omp parallel for num_threads(numThreads)
        for (int i = 0; i < real_vecset->Count(); ++i) {
            ...
            for (int j = 0; j < real_vecset->Count(); ++j) {
                ...
                testDist = COMMON::DistanceUtils::ComputeDistance(
                    reinterpret_cast<R*>(real_vecset->GetVector(i)),
...
template <typename R>
void CPU_top1_pq(std::shared_ptr<VectorSet>& real_vecset,
                 std::shared_ptr<VectorSet>& quan_vecset,
                 DistCalcMethod distMethod,
                 std::shared_ptr<COMMON::IQuantizer>& quantizer, int numThreads,
    ...
    std::shared_ptr<VectorIndex> vecIndex = SPTAG::VectorIndex::CreateInstance(
        IndexAlgoType::BKT, SPTAG::GetEnumValueType<uint8_t>());
    vecIndex->SetQuantizer(quantizer);
    vecIndex->SetParameter("DistCalcMethod",
                           SPTAG::Helper::Convert::ConvertToString(distMethod));
...
#pragma omp parallel for num_threads(numThreads)
        for (int i = 0; i < quan_vecset->Count(); ++i) {
            ...
            for (int j = 0; j < quan_vecset->Count(); ++j) {
                ...
                if (i == idx) continue;
                testDist = vecIndex->ComputeDistance(
                    quan_vecset->GetVector(i), quan_vecset->GetVector(idx));
                if (testDist < res_dist[i]) {
                    res_dist[i] = testDist;
                    res_idx[i] = idx;
                }
...
#pragma omp parallel for num_threads(numThreads)
        for (int i = 0; i < quan_vecset->Count(); ++i) {
            ...
            for (int j = 0; j < quan_vecset->Count(); ++j) {
                if (i == j) continue;
                testDist = vecIndex->ComputeDistance(quan_vecset->GetVector(i),
                                                     quan_vecset->GetVector(j));
                if (testDist < res_dist[i]) {
                    res_dist[i] = testDist;
                    res_idx[i] = j;
                }
...
void verify_results(int* gt_idx, float* gt_dist, int* res_idx, float* res_dist,
                    int N) {
    ...
    for (int i = 0; i < N; ++i) {
        if (gt_idx[i] != res_idx[i] || gt_dist[i] > res_dist[i] * (1 + EPS)) {
            printf("mismatch! gt:%d (%f), result:%d (%f)\n", gt_idx[i],
                   gt_dist[i], res_idx[i], res_dist[i]);
        }
    }
}

void compute_accuracy(int* gt_idx, float* gt_dist, int* res_idx,
                      float* res_dist, int N) {
    float matches = 0.0;
    float dist_sum = 0.0;
...
void DistancePerfSuite(IndexAlgoType algo, DistCalcMethod distMethod) {
    std::shared_ptr<VectorSet> real_vecset, rec_vecset, quan_vecset, queryset,
        truth;
    std::shared_ptr<MetadataSet> metaset;
    std::shared_ptr<COMMON::IQuantizer> quantizer;
    GenerateReconstructData<R>(real_vecset, rec_vecset, quan_vecset, metaset,
                               queryset, truth, distMethod, 10, quantizer);
    ...
    start_t = std::chrono::high_resolution_clock::now();
    CPU_top1_pq<float>(real_vecset, quan_vecset, distMethod, quantizer, 1,
                       res_idx, res_dist, false);
    end_t = std::chrono::high_resolution_clock::now();
    ...
    start_t = std::chrono::high_resolution_clock::now();
    CPU_top1_pq<float>(real_vecset, quan_vecset, distMethod, quantizer, 16,
                       res_idx, res_dist, false);
    end_t = std::chrono::high_resolution_clock::now();
    ...
    start_t = std::chrono::high_resolution_clock::now();
    GPU_pq_alltype(real_vecset, quan_vecset, distMethod, quantizer, res_idx,
                   res_dist);
    end_t = std::chrono::high_resolution_clock::now();
    ...
    SPTAGLIB_LOG(SPTAG::Helper::LogLevel::LL_Info,
                 "CPU 1-thread time - baseline:%0.3lf, PQ:%0.3lf\n",
                 CPU_baseline_t, CPU_PQ_t);
    SPTAGLIB_LOG(SPTAG::Helper::LogLevel::LL_Info,
                 "CPU 16-thread time - baseline:%0.3lf, PQ:%0.3lf\n",
                 CPU_parallel_t, CPU_PQ_parallel_t);
    SPTAGLIB_LOG(SPTAG::Helper::LogLevel::LL_Info,
                 "GPU time - baseline:%0.3lf, PQ:%0.3lf\n", GPU_baseline_t,
                 GPU_PQ_t);

    // LoadReconstructData<R>(real_vecset, rec_vecset, quan_vecset, metaset,
    // queryset, truth, distMethod, 10);

    /*
        auto real_idx = PerfBuild<R>(algo,
       Helper::Convert::ConvertToString<DistCalcMethod>(distMethod),
       real_vecset, metaset, queryset, 10, truth, "real_idx", nullptr);
        Search<R>(real_idx, queryset, 10, truth);
        auto rec_idx = PerfBuild<R>(algo,
       Helper::Convert::ConvertToString<DistCalcMethod>(distMethod), rec_vecset,
       metaset, queryset, 10, truth, "rec_idx", nullptr); Search<R>(rec_idx,
       queryset, 10, truth); auto quan_idx = PerfBuild<std::uint8_t>(algo,
       Helper::Convert::ConvertToString<DistCalcMethod>(distMethod),
