...
bool SearchService::Initialize(int p_argNum, char* p_args[]) {
    ...
    if (!cmdOptions.m_logFile.empty()) {
        SetLogger(std::make_shared<Helper::FileLogger>(
            Helper::LogLevel::LL_Debug, cmdOptions.m_logFile.c_str()));
    }
...
void SearchService::RunSocketMode() {
    auto threadNum =
        max((SizeType)1, m_serviceContext->GetServiceSettings()->m_threadNum);
            ...
            [](std::shared_ptr<SearchExecutionContext> p_exeContext) {
                ...
                for (const auto& result : results) {
                    ...
                    for (const auto& res : result.m_results) {
                        std::cout << "------------------" << std::endl;
                        std::cout << "DocIndex: " << res.VID
                                  << " Distance: " << res.Dist;
                        if (result.m_results.WithMeta()) {
                            const auto& metadata =
                                result.m_results.GetMetadata(idx);
                            std::cout << " MetaData: "
                                      << std::string((char*)metadata.Data(),
                                                     metadata.Length());
                        }
                        std::cout << std::endl;
                        ++idx;
...
void SearchService::SearchHanlderCallback(
    ...
    if (nullptr == p_exeContext) {
        ...
        remoteResult.m_status =
            Socket::RemoteSearchResult::ResultStatus::Success;
