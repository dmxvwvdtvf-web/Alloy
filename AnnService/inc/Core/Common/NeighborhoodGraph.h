...
class NeighborhoodGraph {
   public:
    ...
    virtual void InsertNeighbors(VectorIndex* index, const SizeType node,
                                 SizeType insertNode, float insertDist) = 0;

    virtual void RebuildNeighbors(VectorIndex* index, const SizeType node,
                                  SizeType* nodes,
                                  const BasicResult* queryResults,
                                  const int numResults) = 0;

    virtual float GraphAccuracyEstimation(
        VectorIndex* index, const SizeType samples,
...
#pragma omp parallel for schedule(dynamic)
        for (SizeType i = 0; i < samples; i++) {
            ...
            for (SizeType y = 0; y < m_iGraphSize; y++) {
                if ((idmap != nullptr && idmap->find(y) != idmap->end()))
                    continue;
                float dist = index->ComputeDistance(index->GetSample(x),
                                                    index->GetSample(y));
    ...
    void BuildInitKNNGraph(
        VectorIndex* index,
        const std::unordered_map<SizeType, SizeType>* idmap) {
        SizeType initSize;
        SPTAG::Helper::Convert::ConvertStringTo(
            index->GetParameter("NumberOfInitialDynamicPivots").c_str(),
            initSize);

        // Build the entire RNG graph, both builds the KNN and refines it to RNG
        buildGraph<T>(index, m_iGraphSize, m_iNeighborhoodSize, m_iTPTNumber,
                      (int*)m_pNeighborhoodGraph[0], m_iGPURefineSteps,
                      m_iGPURefineDepth, m_iGPUGraphType, m_iGPULeafSize,
                      initSize, m_iheadNumGPUs, m_iTPTBalanceFactor);

        if (idmap != nullptr) {
            std::unordered_map<SizeType, SizeType>::const_iterator iter;
            for (SizeType i = 0; i < m_iGraphSize; i++) {
                for (DimensionType j = 0; j < m_iNeighborhoodSize; j++) {
                    if ((iter = idmap->find(m_pNeighborhoodGraph[i][j])) !=
                        idmap->end())
...
#else
    template <typename T>
    void PartitionByTptree(
        VectorIndex* index, std::vector<SizeType>& indices,
...
#undef DefineVectorValueType
        ...
        if (last - first <= m_iTPTLeafSize) {
            ...
            SizeType splitidx = first;
            {
                ...
                if (quantizer_exists) {
                    ...
                    indices_vectors.reset(new BasicVectorSet(
                        ByteArray::Alloc(sizeof(R) * cols * count),
                        GetEnumValueType<R>(), cols, count));
                    for (int i = 0; i < count; i++)
                        index->m_pQuantizer->ReconstructVector(
                            (uint8_t*)index->GetSample(indices[first + i]),
                ...
                for (SizeType j = first; j <= end; j++) {
                    R* v = (quantizer_exists)
                               ? (R*)indices_vectors->GetVector(j - first)
                ...
                while (i <= j) {
                    float val = 0;
                    R* v = (quantizer_exists)
                               ? (R*)indices_vectors->GetVector(i - first)
                ...
                // if all the points in the node are equal,equally split the
                // node into 2
                if ((i == first) || (i == last + 1)) {
                    i = (first + last + 1) / 2;
                }
            ...
            PartitionByTptreeCore<T, R>(index, indices, first, splitidx - 1,
                                        leaves);
    ...
    void BuildInitKNNGraph(
        VectorIndex* index,
        const std::unordered_map<SizeType, SizeType>* idmap) {
        COMMON::Dataset<float> NeighborhoodDists(
            m_iGraphSize, m_iNeighborhoodSize, index->m_iDataBlockSize,
            index->m_iDataCapacity);
        std::vector<std::vector<SizeType> > TptreeDataIndices(
            m_iTPTNumber, std::vector<SizeType>(m_iGraphSize));
        std::vector<std::vector<std::pair<SizeType, SizeType> > >
            TptreeLeafNodes(m_iTPTNumber,
...
#pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < m_iTPTNumber; i++) {
            auto rg_local = get_rg_local();
            for (SizeType j = 0; j < m_iGraphSize; j++)
                TptreeDataIndices[i][j] = j;
            std::shuffle(TptreeDataIndices[i].begin(),
                         TptreeDataIndices[i].end(), rg_local);
            PartitionByTptree<T>(index, TptreeDataIndices[i], 0,
                                 m_iGraphSize - 1, TptreeLeafNodes[i]);
            SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                         "Finish Getting Leaves for Tree %d\n", i);
        }
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "Parallel TpTree Partition done\n");
        auto t2 = std::chrono::high_resolution_clock::now();
        SPTAGLIB_LOG(
            Helper::LogLevel::LL_Info, "Build TPTree time (s): %lld\n",
...
#pragma omp parallel for schedule(dynamic)
            for (SizeType j = 0; j < (SizeType)TptreeLeafNodes[i].size(); j++) {
                ...
                if ((j * 5) % TptreeLeafNodes[i].size() == 0)
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Info, "Processing Tree %d %d%%\n",
                        i,
                        static_cast<int>(j * 1.0 / TptreeLeafNodes[i].size() *
                                         100));
                for (SizeType x = start_index; x < end_index; x++) {
                    for (SizeType y = x + 1; y <= end_index; y++) {
                        ...
                        float dist = index->ComputeDistance(
                            index->GetSample(p1), index->GetSample(p2));
                        if (idmap != nullptr) {
                            p1 = (idmap->find(p1) == idmap->end())
                                     ? p1
                                     : idmap->at(p1);
                            p2 = (idmap->find(p2) == idmap->end())
                                     ? p2
                                     : idmap->at(p2);
                        }
                        COMMON::Utils::AddNeighbor(
                            p2, dist, (m_pNeighborhoodGraph)[p1],
                            (NeighborhoodDists)[p1], m_iNeighborhoodSize);
                        COMMON::Utils::AddNeighbor(
                            p1, dist, (m_pNeighborhoodGraph)[p2],
        ...
        SPTAGLIB_LOG(
            Helper::LogLevel::LL_Info, "Process TPTree time (s): %lld\n",
    ...
    void BuildGraph(
        ...
        m_iNeighborhoodSize =
            (DimensionType)(ceil(m_iNeighborhoodSize * m_fNeighborhoodScale) *
                            (m_rebuild + 1));
        m_pNeighborhoodGraph.Initialize(m_iGraphSize, m_iNeighborhoodSize,
                                        index->m_iDataBlockSize,
                                        index->m_iDataCapacity);
        ...
        auto t2 = std::chrono::high_resolution_clock::now();
        SPTAGLIB_LOG(
            Helper::LogLevel::LL_Info, "BuildInitKNNGraph time (s): %lld\n",
            std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count());
        ...
        auto t3 = std::chrono::high_resolution_clock::now();
        SPTAGLIB_LOG(
            Helper::LogLevel::LL_Info, "BuildGraph time (s): %lld\n",
        ...
        if (m_rebuild) {
            ...
            SPTAGLIB_LOG(
                Helper::LogLevel::LL_Info, "ReBuildGraph time (s): %lld\n",
                std::chrono::duration_cast<std::chrono::seconds>(t4 - t3)
                    .count());
        }
        ...
        if (idmap != nullptr) {
            for (auto iter = idmap->begin(); iter != idmap->end(); iter++)
                if (iter->first < 0) {
                    m_pNeighborhoodGraph[-1 - iter->first]
                                        [m_iNeighborhoodSize - 1] =
                                            -2 - iter->second;
                }
        }
    }
...
#pragma omp parallel for schedule(dynamic)
        ...
        auto t1 = std::chrono::high_resolution_clock::now();
        SPTAGLIB_LOG(
            Helper::LogLevel::LL_Info, "Calculate Indegree time (s): %lld\n",
...
#pragma omp parallel for schedule(dynamic)
        for (SizeType i = 0; i < m_iGraphSize; i++) {
            ...
            for (SizeType j = rebuildstart; j < m_iNeighborhoodSize * 2; j++)
                if (outnodes[j] >= 0 &&
                    indegree[outnodes[j]] < rebuild_threshold) {
                    reserve[j] = true;
                    total++;
                }

            for (SizeType j = rebuildstart;
                 j < m_iNeighborhoodSize * 2 &&
                 total < m_iNeighborhoodSize - rebuildstart;
                 j++) {
                if (!reserve[j]) {
                    reserve[j] = true;
                    total++;
            ...
            for (SizeType j = rebuildstart, z = rebuildstart;
                 j < m_iNeighborhoodSize; j++) {
                while (!reserve[z]) z++;
                if (outnodes[j] >= 0)
        ...
        SPTAGLIB_LOG(
            ...
            "Rebuild RNG time (s): %lld Graph Acc: %f\n",
            std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count(),
...
#pragma omp parallel for schedule(dynamic)
            for (SizeType i = 0; i < m_iGraphSize; i++) {
                RefineNode<T>(index, i, false, false,
                              (int)(m_iCEF * m_fCEFScale));
                if ((i * 5) % m_iGraphSize == 0)
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Info, "Refine %d %d%%\n", iter,
                        static_cast<int>(i * 1.0 / m_iGraphSize * 100));
            }
            auto t2 = std::chrono::high_resolution_clock::now();
            SPTAGLIB_LOG(
                Helper::LogLevel::LL_Info,
                "Refine RNG time (s): %lld Graph Acc: %f\n",
        ...
        m_iNeighborhoodSize =
            (DimensionType)(m_iNeighborhoodSize / m_fNeighborhoodScale);
...
#pragma omp parallel for schedule(dynamic)
        for (SizeType i = 0; i < R; i++) {
            ...
            COMMON::QueryResultSet<T> query(
                (const T*)index->GetSample(indices[i]), m_iCEF + 1);
            index->RefineSearchIndex(query, false);
            RebuildNeighbors(index, indices[i], outnodes, query.GetResults(),
                             m_iCEF + 1);
            ...
            for (DimensionType j = 0; j < m_iNeighborhoodSize; j++) {
                ...
                if (idmap != nullptr &&
                    (iter = idmap->find(outnodes[j])) != idmap->end())
                    outnodes[j] = iter->second;
            }
            if (idmap != nullptr &&
                (iter = idmap->find(-1 - i)) != idmap->end())
    ...
    template <typename T>
    void RefineNode(VectorIndex* index, const SizeType node,
                    bool updateNeighbors, bool searchDeleted, int CEF) {
        ...
        if (index->m_pQuantizer) {
            ...
            index->m_pQuantizer->ReconstructVector(
                (const uint8_t*)query.GetTarget(), rec_query);
    ...
    ErrorCode LoadGraph(std::shared_ptr<Helper::DiskIO> input,
                        SizeType blockSize, SizeType capacity) {
        ErrorCode ret = ErrorCode::Success;
        if ((ret = m_pNeighborhoodGraph.Load(input, blockSize, capacity)) !=
            ErrorCode::Success)
            return ret;
    ...
    ErrorCode LoadGraph(std::string sGraphFilename, SizeType blockSize,
                        SizeType capacity) {
        ErrorCode ret = ErrorCode::Success;
        if ((ret = m_pNeighborhoodGraph.Load(sGraphFilename, blockSize,
                                             capacity)) != ErrorCode::Success)
            return ret;
    ...
    ErrorCode LoadGraph(char* pGraphMemFile, SizeType blockSize,
                        SizeType capacity) {
        ErrorCode ret = ErrorCode::Success;
        if ((ret = m_pNeighborhoodGraph.Load(pGraphMemFile, blockSize,
                                             capacity)) != ErrorCode::Success)
            return ret;
    ...
    ErrorCode SaveGraph(std::string sGraphFilename) const {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Save %s To %s\n",
                     m_pNeighborhoodGraph.Name().c_str(),
                     sGraphFilename.c_str());
        auto ptr = f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(sGraphFilename.c_str(),
    ...
    ErrorCode SaveGraph(std::shared_ptr<Helper::DiskIO> output) const {
        ...
        IOBINARY(output, WriteBinary, sizeof(DimensionType),
                 (char*)&m_iNeighborhoodSize);

        for (int i = 0; i < m_iGraphSize; i++)
            IOBINARY(output, WriteBinary,
                     sizeof(SizeType) * m_iNeighborhoodSize,
                     (char*)m_pNeighborhoodGraph[i]);
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Save %s (%d,%d) Finish!\n",
                     m_pNeighborhoodGraph.Name().c_str(), m_iGraphSize,
                     m_iNeighborhoodSize);
        return ErrorCode::Success;
    }
   ...
   public:
    ...
    int m_iRefineIter, m_iCEF, m_iAddCEF, m_iMaxCheckForRefineGraph,
        m_iGPUGraphType, m_iGPURefineSteps, m_iGPURefineDepth, m_iGPULeafSize,
        m_iheadNumGPUs, m_iTPTBalanceFactor, m_rebuild;
};
}  // namespace COMMON
}  // namespace SPTAG
#endif
