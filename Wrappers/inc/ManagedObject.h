...
public ref class ManagedObject {
   ...
   public:
    ...
    ManagedObject(T& instance) { m_Instance = new T(instance); }

    /// <summary>
    /// destructor, which is called whenever delete an object with delete
    /// keyword
    /// </summary>
    ...
    /// <summary>
    /// finalizer which is called by Garbage Collector whenever it destroys the
    /// wrapper object.
    /// </summary>
    !ManagedObject() {
        if (m_Instance != nullptr) {
    ...
    static T string_to(String ^ string) {
        T data;
        SPTAG::Helper::Convert::ConvertStringTo<T>(string_to_char_array(string),
                                                   data);
        return data;
    }
};
