...
public
ref class BasicResult : public ManagedObject<SPTAG::BasicResult> {
   public:
    BasicResult(SPTAG::BasicResult* p_instance) : ManagedObject(p_instance) {}

    property int VID {
       public:
        int get() { return m_Instance->VID; }
    ...
    property float Dist {
       public:
        float get() { return m_Instance->Dist; }
    ...
    property array<Byte> ^
        Meta {
           ...
           array<Byte> ^
               get() {
                   array<Byte> ^ buf =
                       gcnew array<Byte>(m_Instance->Meta.Length());
                   if (buf->LongLength != 0)
                       Marshal::Copy((IntPtr)m_Instance->Meta.Data(), buf, 0,
                                     (int)m_Instance->Meta.Length());
                   return buf;
               } private : void
                           set(array<Byte> ^ p_meta) {}
        }

        property bool RelaxedMono {
       public:
        bool get() { return m_Instance->RelaxedMono; }
...
public
ref class RIterator : public ManagedObject<std::shared_ptr<ResultIterator>> {
   public:
...
public
ref class AnnIndex : public ManagedObject<std::shared_ptr<SPTAG::VectorIndex>> {
   public:
    ...
    static AnnIndex ^
        Merge(String ^ p_indexFilePath1, String ^ p_indexFilePath2);
...
ref class MultiIndexScan
   ...
   public:
    ...
    MultiIndexScan(array<AnnIndex ^> ^ indice, array<array<Byte> ^> ^ p_data,
                   array<float> ^ weight, int p_resultNum, bool useTimer,
