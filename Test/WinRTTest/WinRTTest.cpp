...
template <size_t N>
JsonObject CreateJsonObject(const wchar_t (&v)[N]) {
...
winrt::array_view<uint8_t> Serialize(const T& value) {
    ...
    auto ibuffer = CryptographicBuffer::ConvertStringToBinary(
        str, BinaryStringEncoding::Utf16LE);
...
winrt::hstring Deserialize(winrt::array_view<uint8_t> v) {
    std::wstring_view sv{reinterpret_cast<wchar_t*>(v.begin()),
...
int main() {
    ...
    auto b = CryptographicBuffer::ConvertStringToBinary(
        L"first one", BinaryStringEncoding::Utf16LE);
    idx.AddWithMetadata(
        embedding_t{1, 0},
        winrt::com_array<uint8_t>(b.data(), b.data() + b.Length()));
    ...
    auto file =
        folder
            .CreateFileAsync(L"vector_index",
