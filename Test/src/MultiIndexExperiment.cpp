...
struct FinalData {
    ...
    bool operator<(const FinalData x) const { return Dist < x.Dist; }
};
...
template <typename T>
void BuildIndex(SPTAG::IndexAlgoType algo, std::string distCalcMethod,
...
template <typename T>
void GenerateVectorDataSet(SPTAG::IndexAlgoType algo,
                           std::string distCalcMethod, unsigned int id,
                           std::vector<T>& query, std::vector<T>& query_id,
    ...
    for (; getline(in1, line); n++) {
        ...
        int r = (int)vec.size();
        double sum = 0.0;
        for (int i = l; i < r; i++) sum += vec[i] * vec[i];
        sum = sqrt(sum);
        for (int i = l; i < r; i++) vec[i] = (T)(vec[i] / sum);
    }
    ...
    for (; getline(in2, line); q++) {
        ...
        double sum = 0.0;
        for (int i = l; i < r; i++) sum += query[i] * query[i];
        sum = sqrt(sum);
        for (int i = l; i < r; i++) query[i] = (T)(query[i] / sum);
    }
    ...
    std::shared_ptr<SPTAG::MetadataSet> metaset(new SPTAG::MemMetadataSet(
        n * sizeof(std::uint64_t), n * sizeof(std::uint64_t), n));
    for (auto x : meta) {
...
template <typename T>
void TestMultiIndexScanN(SPTAG::IndexAlgoType algo, std::string distCalcMethod,
                         unsigned int n) {
    std::vector<std::vector<T>> queries(n, std::vector<T>());
    std::vector<std::vector<T>> query_id(n, std::vector<T>());

    GenerateVectorDataSet<T>(algo, distCalcMethod, 0, queries[0], query_id[0],
                             "img");
    GenerateVectorDataSet<T>(algo, distCalcMethod, 1, queries[1], query_id[1],
                             "rec");

    for (auto KK : testSearchLimit) {
        std::string output_result_path =
            outputResult + std::to_string(KK) + "_qrels.txt";
        std::string output_lantency_path =
            outputResult + std::to_string(KK) + "_latency.txt";
        ...
        for (int i = 0; i < query_id[0].size(); i++) {
            ...
            for (unsigned int i = 0; i < n; i++) {
                ...
                BOOST_CHECK(SPTAG::ErrorCode::Success ==
                            SPTAG::VectorIndex::LoadIndex(indexName(i).c_str(),
                                                          vecIndex));
                BOOST_CHECK(nullptr != vecIndex);
            ...
            SPTAG::MultiIndexScan scan(vecIndices, p_targets, topk, &sumFunc,
                                       false, 10000000, KK);
            ...
            for (int i = 0; i < topk; i++) {
                ...
                FinalResult.insert(
                    FinalData(result.Dist, VID2rid[result.VID], result.VID));
            ...
            out2 << idquery << "\t" << (End_time - Begin_time) / CLOCKS_PER_SEC
                 << std::endl;
            int Rank = 0;
            for (auto i = FinalResult.begin(); i != FinalResult.end(); i++)
                out1 << idquery << " " << i->rid << " " << ++Rank << " "
                     << i->vid << " " << i->Dist << std::endl;

            for (int i = (int)FinalResult.size(); i < topk; i++)
                out1 << idquery << " " << -1 << " " << ++Rank << " " << -1
                     << " " << -1 << std::endl;
