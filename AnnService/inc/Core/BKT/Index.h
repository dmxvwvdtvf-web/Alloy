...
class Index : public VectorIndex {
    class RebuildJob : public Helper::ThreadPool::Job {
       public:
        RebuildJob(COMMON::Dataset<T>* p_data, COMMON::BKTree* p_tree,
                   COMMON::RelativeNeighborhoodGraph* p_graph,
                   DistCalcMethod p_distMethod)
            : m_data(p_data),
              m_tree(p_tree),
              m_graph(p_graph),
              m_distMethod(p_distMethod) {}
        void exec(IAbortOperation* p_abort) {
            COMMON::BKTree newTrees(*m_tree);
            newTrees.BuildTrees<T>(*m_data, m_distMethod, 1, nullptr, nullptr,
                                   false, p_abort);
            ...
            for (auto iter = idmap->begin(); iter != idmap->end(); iter++) {
                if (iter->first < 0) {
                    (*m_graph)[-1 - iter->first]
                              [m_graph->m_iNeighborhoodSize - 1] = -1;
                }
            }
            m_tree->SwapTree(newTrees);

            const std::unordered_map<SizeType, SizeType>* newidmap =
                &(m_tree->GetSampleMap());
            for (auto iter = newidmap->begin(); iter != newidmap->end();
                 iter++) {
                if (iter->first < 0) {
                    (*m_graph)[-1 - iter->first][m_graph->m_iNeighborhoodSize -
                                                 1] = -2 - iter->second;
                }
            }
        }
   ...
   private:
    ...
    int m_iHashTableExp;
    std::unique_ptr<SPTAG::COMMON::IWorkSpaceFactory<SPTAG::COMMON::WorkSpace>>
        m_workSpaceFactory;
...
#undef DefineBKTParameter
        ...
        m_iBaseSquare =
            (m_iDistCalcMethod == DistCalcMethod::Cosine)
                ...
                : 1;
        m_workSpaceFactory =
            std::make_unique<SPTAG::COMMON::ThreadLocalWorkSpaceFactory<
                SPTAG::COMMON::WorkSpace>>();
    }
    ...
    inline float AccurateDistance(const void* pX, const void* pY) const {
        ...
        float xy =
            m_iBaseSquare -
            m_fComputeDistance((const T*)pX, (const T*)pY, m_pSamples.C());
        float xx =
            m_iBaseSquare -
            m_fComputeDistance((const T*)pX, (const T*)pX, m_pSamples.C());
        float yy =
            m_iBaseSquare -
            m_fComputeDistance((const T*)pY, (const T*)pY, m_pSamples.C());
    ...
    inline bool NeedRefine() const {
        return m_deletedID.Count() >
               (size_t)(GetNumSamples() * m_fDeletePercentageForRefine);
    }
    ...
    ErrorCode SearchIndexIterativeNext(QueryResult& p_query,
    ...
    ErrorCode SetWorkSpaceFactory(
        ...
        SPTAG::COMMON::IWorkSpaceFactory<SPTAG::COMMON::WorkSpace>*
            raw_specialized_ptr = dynamic_cast<
                SPTAG::COMMON::IWorkSpaceFactory<SPTAG::COMMON::WorkSpace>*>(
                raw_generic_ptr);
   ...
   private:
    int SearchIndexIterative(COMMON::QueryResultSet<T>& p_query,
    ...
    void SearchIndex(
        ...
        std::function<bool(const ByteArray&)> filterFunc = nullptr) const;
