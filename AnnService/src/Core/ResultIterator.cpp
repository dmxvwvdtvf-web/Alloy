...
ResultIterator::ResultIterator(const void* p_index, const void* p_target,
                               bool p_searchDeleted, int p_workspaceBatch)
    : m_index((const VectorIndex*)p_index),
      m_target(p_target),
      m_searchDeleted(p_searchDeleted) {
    m_workspace = new UniqueHandler;
    ((UniqueHandler*)m_workspace)->m_handler =
        std::move(m_index->RentWorkSpace(p_workspaceBatch));
    m_isFirstResult = true;
}

ResultIterator::~ResultIterator() { Close(); }
...
std::shared_ptr<QueryResult> ResultIterator::Next(int batch) {
    ...
    int resultCount = 0;
    m_index->SearchIndexIterativeNext(
        *m_queryResult, (((UniqueHandler*)m_workspace)->m_handler).get(), batch,
        resultCount, m_isFirstResult, m_searchDeleted);
    m_isFirstResult = false;
    for (int i = 0; i < resultCount; i++) {
        m_queryResult->GetResult(i)->RelaxedMono =
            (((UniqueHandler*)m_workspace)->m_handler)->m_relaxedMono;
    }
...
void ResultIterator::Close() {
    if (m_workspace != nullptr) {
        m_index->SearchIndexIterativeEnd(
            std::move(((UniqueHandler*)m_workspace)->m_handler));
        delete m_workspace;
        m_workspace = nullptr;
    }
}

const void* ResultIterator::GetTarget() { return m_target; }
// namespace SPTAG
