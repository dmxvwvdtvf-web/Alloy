...
#define DefineBKTParameter(VarName, VarType, DefaultValue, RepresentStr)     \
    SetParameter(RepresentStr, p_reader                                      \
                                   .GetParameter("Index", RepresentStr,      \
...
ErrorCode Index<T>::LoadIndexDataFromMemory(
    ...
    if (m_pSamples.Load((char*)p_indexBlobs[0].Data(), m_iDataBlockSize,
                        m_iDataCapacity) != ErrorCode::Success)
        return ErrorCode::FailedParseValue;
    if (m_pTrees.LoadTrees((char*)p_indexBlobs[1].Data()) != ErrorCode::Success)
        return ErrorCode::FailedParseValue;
    if (m_pGraph.LoadGraph((char*)p_indexBlobs[2].Data(), m_iDataBlockSize,
                           m_iDataCapacity) != ErrorCode::Success)
        return ErrorCode::FailedParseValue;
    if (p_indexBlobs.size() <= 3)
        m_deletedID.Initialize(
            m_pSamples.R(), m_iDataBlockSize, m_iDataCapacity,
            COMMON::Labelset::InvalidIDBehavior::AlwaysContains);
    else if (m_deletedID.Load(
                 (char*)p_indexBlobs[3].Data(), m_iDataBlockSize,
                 m_iDataCapacity,
                 COMMON::Labelset::InvalidIDBehavior::AlwaysContains) !=
             ErrorCode::Success)
        return ErrorCode::FailedParseValue;
    ...
    if (m_pSamples.R() != m_pGraph.R() || m_pSamples.R() != m_deletedID.R()) {
        SPTAGLIB_LOG(SPTAG::Helper::LogLevel::LL_Error,
                     "Index data is corrupted, please rebuild the index. "
                     "Samples: %i, Graph: %i, DeletedID: %i.",
...
ErrorCode Index<T>::LoadIndexData(
    ...
    if (p_indexStreams[0] == nullptr ||
        (ret = m_pSamples.Load(p_indexStreams[0], m_iDataBlockSize,
                               m_iDataCapacity)) != ErrorCode::Success)
        return ret;
    if (p_indexStreams[1] == nullptr ||
        (ret = m_pTrees.LoadTrees(p_indexStreams[1])) != ErrorCode::Success)
        return ret;
    if (p_indexStreams[2] == nullptr ||
        (ret = m_pGraph.LoadGraph(p_indexStreams[2], m_iDataBlockSize,
                                  m_iDataCapacity)) != ErrorCode::Success)
        return ret;
    if (p_indexStreams[3] == nullptr)
        m_deletedID.Initialize(
            m_pSamples.R(), m_iDataBlockSize, m_iDataCapacity,
            COMMON::Labelset::InvalidIDBehavior::AlwaysContains);
    else if ((ret = m_deletedID.Load(
                  p_indexStreams[3], m_iDataBlockSize, m_iDataCapacity,
                  COMMON::Labelset::InvalidIDBehavior::AlwaysContains)) !=
             ErrorCode::Success)
        return ret;
...
#define DefineBKTParameter(VarName, VarType, DefaultValue, RepresentStr)     \
    IOSTRING(p_configOut, WriteString,                                       \
             (RepresentStr + std::string("=") + GetParameter(RepresentStr) + \
              std::string("\n"))                                             \
                 .c_str());
...
lock(*(m_pTrees.m_lock)); \
        m_pTrees.InitSearchTrees(m_pSamples, m_fComputeDistance, p_query,
p_space); \
...
p_space.m_iNumberOfCheckedLeaves > p_space.m_iMaxCheck) { \
            ...
            for (DimensionType i = 0; i <= checkPos; i++) { \
                ...
                float distance2leaf =
m_fComputeDistance(p_query.GetQuantizedTarget(), (m_pSamples)[nn_index],
GetFeatureDim()); \
                p_space.m_iNumberOfCheckedLeaves++; \
                if (p_space.m_Results.insert(distance2leaf)) { \
                    p_space.m_NGQueue.insert(NodeDistPair(nn_index,
distance2leaf)); \
                } \
            } \
...
p_space, m_iNumberOfOtherDynamicPivots + p_space.m_iNumberOfCheckedLeaves); \
            } \
        } \
        p_query.SortResult(); \
*/
...
p_space.m_iNumberOfCheckedLeaves > p_space.m_iMaxCheck) { \
            ...
            if (checkNode < -1) { \
                ...
                while (i < tnode.childEnd) { \
                    ...
                    { \
                        float distance2leaf =
m_fComputeDistance(p_query.GetQuantizedTarget(), (m_pSamples)[tmpNode],
GetFeatureDim()); \
                        if (!p_space.CheckAndSet(tmpNode)) { \
                            p_space.m_NGQueue.insert(NodeDistPair(tmpNode,
distance2leaf)); \
                        } \
                    } \
                    i++; \
...
void Index<T>::Search(COMMON::QueryResultSet<T>& p_query,
    ...
    while (!p_space.m_NGQueue.empty()) {
                ...
                do {
                    if (notDeleted(m_deletedID, tmpNode)) {
                        if (checkFilter(m_pMetadata, tmpNode, filterFunc)) {
                            if (isDup(p_query, tmpNode, gnode.distance)) break;
                        }
                    }
                    if (i <= 0) break;
                    tmpNode = m_pTrees[i].centerid;
        ...
        for (DimensionType i = 0; i <= checkPos; i++) {
            ...
            if (p_space.CheckAndSet(nn_index)) continue;
            float distance2leaf =
                m_fComputeDistance(p_query.GetQuantizedTarget(),
            ...
            while (i < tnode.childEnd) {
                ...
                if (notDeleted(m_deletedID, tmpNode)) {
                    float distance2leaf = m_fComputeDistance(
                        p_query.GetQuantizedTarget(), (m_pSamples)[tmpNode],
                        GetFeatureDim());
                    if (!p_space.CheckAndSet(tmpNode)) {
...
void Index<T>::SearchIndex(
    ...
    switch (flags) {
        case 0b000:
            Search<StaticDispatch::CheckIfNotDeleted, StaticDispatch::NeverDup,
                   StaticDispatch::CheckFilter>(p_query, p_space, filterFunc);
            break;
        case 0b001:
            Search<StaticDispatch::CheckIfNotDeleted, StaticDispatch::NeverDup,
                   StaticDispatch::AlwaysTrue>(p_query, p_space, filterFunc);
            break;
        case 0b010:
            Search<StaticDispatch::CheckIfNotDeleted, StaticDispatch::CheckDup,
                   StaticDispatch::CheckFilter>(p_query, p_space, filterFunc);
            break;
        case 0b011:
            Search<StaticDispatch::CheckIfNotDeleted, StaticDispatch::CheckDup,
            ...
            break;
        case 0b100:
            Search<StaticDispatch::AlwaysTrue, StaticDispatch::NeverDup,
                   StaticDispatch::CheckFilter>(p_query, p_space, filterFunc);
            break;
        case 0b101:
            Search<StaticDispatch::AlwaysTrue, StaticDispatch::NeverDup,
            ...
            break;
        case 0b110:
            Search<StaticDispatch::AlwaysTrue, StaticDispatch::CheckDup,
                   StaticDispatch::CheckFilter>(p_query, p_space, filterFunc);
            break;
        case 0b111:
            Search<StaticDispatch::AlwaysTrue, StaticDispatch::CheckDup,
...
template <typename T>
int Index<T>::SearchIndexIterative(COMMON::QueryResultSet<T>& p_query,
                                   COMMON::WorkSpace& p_space, bool p_isFirst,
                                   int batch, bool p_searchDeleted,
                                   bool p_searchDuplicated) const {
    int count = 0;
    // bitflags for which dispatch to take
    uint8_t flags = 0;
    ...
    switch (flags) {
        case 0b00:
            count = SearchIterative<StaticDispatch::CheckIfNotDeleted,
                                    StaticDispatch::NeverDup>(p_query, p_space,
                                                              p_isFirst, batch);
            break;
        case 0b01:
            count = SearchIterative<StaticDispatch::CheckIfNotDeleted,
                                    StaticDispatch::CheckDup>(p_query, p_space,
                                                              p_isFirst, batch);
            break;
        case 0b10:
            count = SearchIterative<StaticDispatch::AlwaysTrue,
                                    StaticDispatch::NeverDup>(p_query, p_space,
                                                              p_isFirst, batch);
            break;
        case 0b11:
...
bool Index<T>::SearchIndexIterativeFromNeareast(QueryResult& p_query,
    ...
    if (p_isFirst) {
        ...
        SearchIndex(*((COMMON::QueryResultSet<T>*)&p_query), *p_space,
        ...
        for (int i = 0; i < p_query.GetResultNum(); ++i) {
            ...
            for (DimensionType i = 0; i <= checkPos; i++) {
                auto futureNode = node[i];
                if (futureNode < 0) break;
                _mm_prefetch((const char*)(m_pSamples)[futureNode],
                             _MM_HINT_T0);
            }
            for (DimensionType i = 0; i <= checkPos; i++) {
                SizeType nn_index = node[i];
                ...
                if (p_space->CheckAndSet(nn_index)) continue;
                float distance2leaf =
                    m_fComputeDistance((const T*)p_query.GetQuantizedTarget(),
                                       (m_pSamples)[nn_index], GetFeatureDim());
                p_space->m_NGQueue.insert(
                    NodeDistPair(nn_index, distance2leaf));
            }
        }

    } else {
        p_space->ResetResult(m_iMaxCheck, p_query.GetResultNum());
        SearchIndexIterative(*((COMMON::QueryResultSet<T>*)&p_query), *p_space,
                             p_isFirst, p_query.GetResultNum(), p_searchDeleted,
                             true);
    }
...
ErrorCode Index<T>::SearchIndex(QueryResult& p_query,
    ...
    SearchIndex(*((COMMON::QueryResultSet<T>*)&p_query), *workSpace,
                p_searchDeleted, true);
...
ErrorCode Index<T>::SearchIndexWithFilter(
    ...
    workSpace->Reset(maxCheck == 0 ? m_iMaxCheck : maxCheck,
                     p_query.GetResultNum());

    SearchIndex(*((COMMON::QueryResultSet<T>*)&p_query), *workSpace,
                p_searchDeleted, true, filterFunc);
...
std::shared_ptr<ResultIterator> Index<T>::GetIterator(
    ...
    std::shared_ptr<ResultIterator> resultIterator =
        std::make_shared<ResultIterator>((const void*)this, p_target,
                                         p_searchDeleted, 1);
...
ErrorCode Index<T>::SearchIndexIterativeNext(QueryResult& p_query,
    ...
    resultCount = SearchIndexIterative(*((COMMON::QueryResultSet<T>*)&p_query),
                                       *workSpace, p_isFirst, p_batch,
                                       p_searchDeleted, true);
...
ErrorCode Index<T>::SearchIndexIterativeEnd(
    ...
    if (!m_bReady) return ErrorCode::EmptyIndex;
    if (nullptr != space) m_workSpaceFactory->ReturnWorkSpace(std::move(space));
    return ErrorCode::Success;
}
...
template <typename T>
ErrorCode Index<T>::RefineSearchIndex(QueryResult& p_query,
                                      bool p_searchDeleted) const {
    auto workSpace = m_workSpaceFactory->GetWorkSpace();
    if (!workSpace) {
        workSpace.reset(new COMMON::WorkSpace());
...
ErrorCode Index<T>::SearchTree(QueryResult& p_query) const {
    ...
    m_pTrees.InitSearchTrees(m_pSamples, m_fComputeDistance, *p_results,
                             *workSpace);
    m_pTrees.SearchTrees(m_pSamples, m_fComputeDistance, *p_results, *workSpace,
                         m_iNumberOfInitialDynamicPivots);
...
ErrorCode Index<T>::BuildIndex(const void* p_data, SizeType p_vectorNum,
    ...
    m_pSamples.Initialize(p_vectorNum, p_dimension, m_iDataBlockSize,
                          m_iDataCapacity, (T*)p_data, p_shareOwnership);
...
#pragma omp parallel for
    ...
    SPTAGLIB_LOG(
        Helper::LogLevel::LL_Info, "Build Tree time (s): %lld\n",
    ...
    SPTAGLIB_LOG(
        Helper::LogLevel::LL_Info, "Build Graph time (s): %lld\n",
...
#undef DefineBKTParameter
    ...
    ErrorCode ret = ErrorCode::Success;
    if ((ret = m_pSamples.Refine(indices, ptr->m_pSamples)) !=
        ErrorCode::Success)
        return ret;
    if (nullptr != m_pMetadata &&
        (ret = m_pMetadata->RefineMetadata(
             indices, ptr->m_pMetadata, m_iDataBlockSize, m_iDataCapacity,
             m_iMetaRecordSize)) != ErrorCode::Success)
        return ret;

    ptr->m_deletedID.Initialize(
        newR, m_iDataBlockSize, m_iDataCapacity,
        COMMON::Labelset::InvalidIDBehavior::AlwaysContains);
    COMMON::BKTree* newtree = &(ptr->m_pTrees);
    (*newtree).BuildTrees<T>(ptr->m_pSamples, ptr->m_iDistCalcMethod,
                             omp_get_num_threads());
    m_pGraph.RefineGraph<T>(this, indices, reverseIndices, nullptr,
                            &(ptr->m_pGraph), &(ptr->m_pTrees.GetSampleMap()));
...
ErrorCode Index<T>::RefineIndex(
    ...
    ErrorCode ret = ErrorCode::Success;
    if ((ret = m_pSamples.Refine(indices, p_indexStreams[0])) !=
        ErrorCode::Success)
        return ret;
    ...
    newTrees.BuildTrees<T>(m_pSamples, m_iDistCalcMethod, omp_get_num_threads(),
    ...
    if ((ret = m_pGraph.RefineGraph<T>(
             this, indices, reverseIndices, p_indexStreams[2], nullptr,
             &(newTrees.GetSampleMap()))) != ErrorCode::Success)
        return ret;

    COMMON::Labelset newDeletedID;
    newDeletedID.Initialize(
        newR, m_iDataBlockSize, m_iDataCapacity,
        COMMON::Labelset::InvalidIDBehavior::AlwaysContains);
    ...
    if (nullptr != m_pMetadata) {
        ...
        if ((ret = m_pMetadata->RefineMetadata(indices, p_indexStreams[4],
                                               p_indexStreams[5])) !=
            ErrorCode::Success)
            return ret;
    }
...
ErrorCode Index<T>::AddIndex(const void* p_data, SizeType p_vectorNum,
    ...
    ErrorCode ret;
    {
        ...
        if (begin == 0) {
            if (p_metadataSet != nullptr) {
                m_pMetadata.reset(new MemMetadataSet(
                    m_iDataBlockSize, m_iDataCapacity, m_iMetaRecordSize));
                m_pMetadata->AddBatch(*p_metadataSet);
                if (p_withMetaIndex) BuildMetaMapping(false);
            }
            if ((ret = BuildIndex(p_data, p_vectorNum, p_dimension,
                                  p_normalized)) != ErrorCode::Success)
                return ret;
            return ErrorCode::Success;
        }
        ...
        if (m_pSamples.AddBatch((const T*)p_data, p_vectorNum) !=
                ErrorCode::Success ||
    ...
    if (end - m_pTrees.sizePerTree() >= m_addCountForRebuild &&
        m_threadPool.jobsize() == 0) {
...
#undef DefineBKTParameter
    ...
    if (SPTAG::Helper::StrUtils::StrEqualIgnoreCase(p_param,
        ...
        m_fComputeDistance =
            m_pQuantizer
                ? m_pQuantizer->DistanceCalcSelector<T>(m_iDistCalcMethod)
                : COMMON::DistanceCalcSelector<T>(m_iDistCalcMethod);
        auto base = m_pQuantizer ? m_pQuantizer->GetBase()
                                 : COMMON::Utils::GetBase<T>();
        m_iBaseSquare =
            (m_iDistCalcMethod == DistCalcMethod::Cosine) ? base * base : 1;
    }
    return ErrorCode::Success;
}
