...
sptag::ByteArray GetByteArray(const winrt::array_view<const T>& data) {
    ...
    auto byteArray =
        sptag::ByteArray(reinterpret_cast<byte*>(copy), byteSize, true);
    return byteArray;
}

SPTAG::SearchResult AnnIndex::GetResultFromMetadata(
    const sptag::BasicResult& r) const {
    return winrt::make<SearchResult>(r.Meta.Data(), r.Meta.Length(), r.Dist);
}

winrt::Windows::Foundation::Collections::IVector<SPTAG::SearchResult>
AnnIndex::Search(EmbeddingVector p_data, uint32_t p_resultNum) const {
    auto vec = std::vector<SPTAG::SearchResult>{};
    p_resultNum = (std::min)(static_cast<sptag::SizeType>(p_resultNum),
                             m_index->GetNumSamples());
    auto results =
        std::make_shared<sptag::QueryResult>(p_data.data(), p_resultNum, true);
...
void AnnIndex::Load(winrt::Windows::Storage::StorageFile file) {
    auto path = file.Path();
    if (sptag::ErrorCode::Success != sptag::VectorIndex::LoadIndexFromFile(
                                         winrt::to_string(path), m_index)) {
...
void AnnIndex::Save(winrt::Windows::Storage::StorageFile file) {
    ...
    if (sptag::ErrorCode::Success !=
        m_index->SaveIndexToFile(winrt::to_string(path))) {
        throw winrt::hresult_error{};
    }
}
...
void AnnIndex::_AddWithMetadataImpl(EmbeddingVector p_data, T metadata) {
    ...
    std::uint64_t* offsets =
        new std::uint64_t[p_num + 1]{0, metadata.size() * sizeof(metadata[0])};
    sptag::ByteArray offsetsArray(reinterpret_cast<std::uint8_t*>(offsets),
                                  p_num + 1, true);
    std::shared_ptr<sptag::MemMetadataSet> meta(
        new sptag::MemMetadataSet(p_meta, offsetsArray, p_num));
...
void AnnIndex::AddWithMetadata(array_view<float const> data,
                               array_view<uint8_t const> metadata) {
