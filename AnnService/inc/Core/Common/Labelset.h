...
class Labelset {
   ...
   public:
    ...
    void Initialize(SizeType size, SizeType blockSize, SizeType capacity,
                    InvalidIDBehavior invalidIDBehaviorSetting =
                        InvalidIDBehavior::Passthrough) {
    ...
    inline ErrorCode Save(std::string filename) {
        ...
        auto ptr = f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(filename.c_str(),
                             std::ios::binary | std::ios::out))
            return ErrorCode::FailedCreateFile;
        return Save(ptr);
    }

    inline ErrorCode Load(std::shared_ptr<Helper::DiskIO> input,
    ...
    inline ErrorCode Load(std::string filename, SizeType blockSize,
                          SizeType capacity,
                          InvalidIDBehavior invalidIDBehaviorSetting =
                              InvalidIDBehavior::Passthrough) {
        ...
        auto ptr = f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(filename.c_str(), std::ios::binary | std::ios::in))
            return ErrorCode::FailedOpenFile;
        return Load(ptr, blockSize, capacity, invalidIDBehaviorSetting);
    }

    inline ErrorCode Load(char* pmemoryFile, SizeType blockSize,
                          SizeType capacity,
                          InvalidIDBehavior invalidIDBehaviorSetting =
                              InvalidIDBehavior::Passthrough) {
        m_invalidIDBehaviorSetting = invalidIDBehaviorSetting;
    ...
    inline ErrorCode AddBatch(SizeType num) { return m_data.AddBatch(num); }

    inline std::uint64_t BufferSize() const {
        return m_data.BufferSize() + sizeof(SizeType);
    }

    inline void SetR(SizeType num) { m_data.SetR(num); }

    inline SizeType R() const { return m_data.R(); }
};
}  // namespace COMMON
}  // namespace SPTAG
