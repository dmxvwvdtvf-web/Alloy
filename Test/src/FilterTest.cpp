...
template <typename T>
void SearchWithFilter(const std::string folder, T* vec, SPTAG::SizeType n,
                      int k) {
    ...
    BOOST_CHECK(nullptr != vecIndex);
    std::string value = "2";
    std::function<bool(const SPTAG::ByteArray&)> filterFunction =
        [value](const SPTAG::ByteArray& meta) -> bool {
...
BOOST_AUTO_TEST_CASE(BKTTest) { FTest<float>(SPTAG::IndexAlgoType::BKT, "L2"); }
