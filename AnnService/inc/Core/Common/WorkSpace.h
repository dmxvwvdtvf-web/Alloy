   ...
   public:
    ...
    void clear() {
        if (!m_secondHash) {
            ...
            // Clear all blocks.
            m_secondHash = false;
            memset(m_hashTable.get(), 0,
                   2 * sizeof(SizeType) * (m_poolSize + 1));
        }
    }
    ...
    inline int _CheckAndSet(SizeType* hashTable, int poolSize,
                            bool isFirstTable, SizeType idx) {
        ...
        SPTAGLIB_LOG(
            ...
            "Hash table is full! Set HashTableExponent to larger value "
            "(default is 2). NewHashTableExponent=%d NewPoolSize=%d\n",
            m_exp, m_poolSize);
...
class DistPriorityQueue {
   ...
   public:
    ...
    inline float worst() { return m_data[1]; }
};
...
struct WorkSpace : public IWorkSpace {
    ...
    WorkSpace(WorkSpace& other) {
        Initialize(other.m_iMaxCheck,
                   other.nodeCheckStatus.HashTableExponent());
    }
