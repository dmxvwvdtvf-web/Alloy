...
class TruthSet {
   public:
    static void LoadTruthTXT(std::shared_ptr<SPTAG::Helper::DiskIO>& ptr,
                             std::vector<std::set<SizeType>>& truth, int K,
                             int& originalK, SizeType& p_iTruthNumber) {
        ...
        for (int i = 0; i < p_iTruthNumber; ++i) {
            ...
            if (ptr->ReadString(lineBufferSize, currentLine, '\n') == 0) {
                SPTAGLIB_LOG(
                    ...
                    "Truth number(%d) and query number(%d) are not match!\n", i,
                    p_iTruthNumber);
                exit(1);
            }
            ...
            for (int j = 0; j < K; ++j) {
                if (tmp == nullptr) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                 "Truth number(%d, %d) and query number(%d) "
                                 "are not match!\n",
                                 i, j, p_iTruthNumber);
                    exit(1);
                }
    ...
    static void LoadTruthXVEC(std::shared_ptr<SPTAG::Helper::DiskIO>& ptr,
                              std::vector<std::set<SizeType>>& truth, int K,
                              int& originalK, SizeType& p_iTruthNumber) {
        truth.clear();
        truth.resize(p_iTruthNumber);
        ...
        for (int i = 0; i < p_iTruthNumber; i++) {
            if (ptr->ReadBinary(4, (char*)&originalK) != 4 || originalK < K) {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                             "Error: Xvec file has No.%d vector whose dims are "
                             "fewer than expected. Expected: %d, Fact: %d\n",
                             i, K, originalK);
                exit(1);
            }
            if (originalK > K) vec.resize(originalK);
            if (ptr->ReadBinary(originalK * 4, (char*)vec.data()) !=
                originalK * 4) {
                SPTAGLIB_LOG(
                    Helper::LogLevel::LL_Error,
                    "Truth number(%d) and query number(%d) are not match!\n", i,
    ...
    static void LoadTruthDefault(std::shared_ptr<SPTAG::Helper::DiskIO>& ptr,
                                 std::vector<std::set<SizeType>>& truth, int K,
                                 int& originalK, SizeType& p_iTruthNumber) {
        if (ptr->TellP() == 0) {
            int row;
            if (ptr->ReadBinary(4, (char*)&row) != 4 ||
                ptr->ReadBinary(4, (char*)&originalK) != 4) {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                             "Fail to read truth file!\n");
                exit(1);
            }
        }
        truth.clear();
        ...
        for (int i = 0; i < p_iTruthNumber; i++) {
            if (ptr->ReadBinary(4 * originalK, (char*)vec.data()) !=
                4 * originalK) {
                SPTAGLIB_LOG(
                    Helper::LogLevel::LL_Error,
                    "Truth number(%d) and query number(%d) are not match!\n", i,
    ...
    static void LoadTruth(std::shared_ptr<SPTAG::Helper::DiskIO>& ptr,
                          std::vector<std::set<SizeType>>& truth,
                          SizeType& NumQuerys, int& originalK, int K,
                          TruthFileType type) {
        if (type == TruthFileType::TXT) {
            ...
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "TruthFileType Unsupported.\n");
            exit(1);
        }
    }

    static void writeTruthFile(
        const std::string truthFile, SizeType queryNumber, const int K,
        std::vector<std::vector<SPTAG::SizeType>>& truthset,
        std::vector<std::vector<float>>& distset, SPTAG::TruthFileType TFT) {
        auto ptr = SPTAG::f_createIO();
        if (ptr == nullptr ||
            !ptr->Initialize(truthFile.c_str(),
                             std::ios::out | std::ios::binary)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Fail to create the file:%s\n", truthFile.c_str());
            exit(1);
        }
        ...
        if (TFT == SPTAG::TruthFileType::TXT) {
            for (SizeType i = 0; i < queryNumber; i++) {
                for (int k = 0; k < K; k++) {
                    if (ptr->WriteString(
                            (std::to_string(truthset[i][k]) + " ").c_str()) ==
                        0) {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                     "Fail to write the truth file!\n");
                        exit(1);
                    }
                }
                if (ptr->WriteString("\n") == 0) {
            ...
            for (SizeType i = 0; i < queryNumber; i++) {
                if (ptr->WriteBinary(sizeof(K), (char*)&K) != sizeof(K) ||
                    ptr->WriteBinary(K * 4, (char*)(truthset[i].data())) !=
                        K * 4) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                 "Fail to write the truth file!\n");
                    exit(1);
            ...
            for (SizeType i = 0; i < queryNumber; i++) {
                if (ptr->WriteBinary(K * 4, (char*)(truthset[i].data())) !=
                    K * 4) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                 "Fail to write the truth file!\n");
                    exit(1);
            ...
            for (SizeType i = 0; i < queryNumber; i++) {
                if (ptr->WriteBinary(K * 4, (char*)(distset[i].data())) !=
                    K * 4) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                 "Fail to write the truth file!\n");
                    exit(1);
            ...
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "Found unsupported file type for generating truth.");
            exit(-1);
        }
    }

    template <typename T>
    static void GenerateTruth(std::shared_ptr<VectorSet> querySet,
                              std::shared_ptr<VectorSet> vectorSet,
                              const std::string truthFile,
                              const SPTAG::DistCalcMethod distMethod,
                              const int K,
                              const SPTAG::TruthFileType p_truthFileType,
                              const std::shared_ptr<IQuantizer>& quantizer);

    template <typename T>
    static float CalculateRecall(VectorIndex* index,
                                 std::vector<QueryResult>& results,
                                 const std::vector<std::set<SizeType>>& truth,
                                 int K, int truthK,
                                 std::shared_ptr<SPTAG::VectorSet> querySet,
                                 std::shared_ptr<SPTAG::VectorSet> vectorSet,
                                 SizeType NumQuerys,
                                 std::ofstream* log = nullptr,
                                 bool debug = false, float* MRR = nullptr) {
                ...
                for (int j = 0; j < K; j++) {
                    ...
                    if (results[i].GetResult(j)->VID == id) {
                        ...
                        float dist = COMMON::DistanceUtils::ComputeDistance(
                            (const T*)querySet->GetVector(i),
                            (const T*)vectorSet->GetVector(
                                results[i].GetResult(j)->VID),
                            vectorSet->Dimension(), index->GetDistCalcMethod());
                        float truthDist =
                            COMMON::DistanceUtils::ComputeDistance(
                                (const T*)querySet->GetVector(i),
                                (const T*)vectorSet->GetVector(id),
                                vectorSet->Dimension(),
                                index->GetDistCalcMethod());
                        if (index->GetDistCalcMethod() ==
                                SPTAG::DistCalcMethod::Cosine &&
                            fabs(dist - truthDist) < Epsilon) {
                            thisrecall[i] += 1;
                            visited[j] = true;
                            break;
                        } else if (index->GetDistCalcMethod() ==
            ...
            if (debug) {
                ...
                for (SizeType id : truth[i]) {
                    ...
                    if (vectorSet != nullptr) {
                        truthDist = COMMON::DistanceUtils::ComputeDistance(
                            (const T*)querySet->GetVector(i),
                            (const T*)vectorSet->GetVector(id),
                            querySet->Dimension(), index->GetDistCalcMethod());
                    }
                    truthvec.emplace_back(id, truthDist);
                }
                std::sort(truthvec.begin(), truthvec.end());
                for (int j = 0; j < truthvec.size(); j++)
                    ll += std::to_string(truthvec[j].node) + "@" +
                          std::to_string(truthvec[j].distance) + ",";
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "%s\n", ll.c_str());
                ll = "ann:";
                for (int j = 0; j < K; j++)
                    ll += std::to_string(results[i].GetResult(j)->VID) + "@" +
                          std::to_string(results[i].GetResult(j)->Dist) + ",";
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "%s\n", ll.c_str());
            }
        }
        ...
        for (SizeType i = 0; i < NumQuerys; i++) {
            stdrecall +=
                (thisrecall[i] - meanrecall) * (thisrecall[i] - meanrecall);
        }
        stdrecall = std::sqrt(stdrecall / NumQuerys);
        if (log)
            (*log) << meanrecall << " " << stdrecall << " " << minrecall << " "
                   << maxrecall << std::endl;
    ...
    static float CalculateRecall(VectorIndex* index, T* query, int K) {
        ...
        if (index->m_pQuantizer) {
            reconstructVector =
                ALIGN_ALLOC(index->m_pQuantizer->ReconstructSize());
            index->m_pQuantizer->ReconstructVector((const uint8_t*)query,
                                                   reconstructVector);
        ...
        for (SizeType y = 0; y < index->GetNumSamples(); y++) {
            float dist = index->ComputeDistance(
                sampleTruth.GetQuantizedTarget(), index->GetSample(y));
        ...
        for (SizeType y = 0; y < K; y++) {
            for (SizeType z = 0; z < K; z++) {
                ...
                if (fabs(sampleANN.GetResult(z)->Dist -
                         sampleTruth.GetResult(y)->Dist) < Epsilon) {
                    recalls += 1;
                    visited[z] = true;
                    break;
                }
