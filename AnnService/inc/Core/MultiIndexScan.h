...
class MultiIndexScan {
   public:
    ...
    MultiIndexScan(std::vector<std::shared_ptr<VectorIndex>> vecIndices,
                   ...
                   float (*rankFunction)(std::vector<float>), bool useTimer,
                   int termCondVal, int searchLimit);
    ~MultiIndexScan();
    void Init(std::vector<std::shared_ptr<VectorIndex>> vecIndices,
              std::vector<ByteArray> p_targets, std::vector<float> weight,
              unsigned int k, bool useTimer, int termCondVal, int searchLimit);
