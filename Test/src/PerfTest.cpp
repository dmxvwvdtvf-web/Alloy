...
void Search(std::shared_ptr<VectorIndex>& vecIndex,
    ...
    std::vector<QueryResult> res(queryset->Count(),
                                 QueryResult(nullptr, k, true));
    ...
    for (SizeType i = 0; i < queryset->Count(); i++) {
        ...
        for (int j = 0; j < truthDimension; j++) {
            ...
            ByteArray truthmeta = ByteArray((std::uint8_t*)(truthstr.c_str()),
                                            truthstr.length(), false);
            float truthdist = vecIndex->ComputeDistance(
                queryset->GetVector(i),
                vecIndex->GetSample(truthmeta, deleted));
            for (int l = 0; l < k; l++) {
                if (fabs(truthdist - res[i].GetResult(l)->Dist) <=
                    eps * (fabs(truthdist) + eps)) {
                    recall += 1.0;
                    break;
                }
            }
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Recall %d@%d: %f\n", k,
                 truthDimension, recall / queryset->Count() / truthDimension);
}

template <typename T>
void PerfAdd(IndexAlgoType algo, std::string distCalcMethod,
             std::shared_ptr<VectorSet>& vec,
    ...
    for (SizeType i = 0; i < vec->Count(); i++) {
        ...
        std::shared_ptr<MetadataSet> metaset(new MemMetadataSet(
            metaarr,
            ByteArray((std::uint8_t*)offset, 2 * sizeof(std::uint64_t), false),
            1));
        ErrorCode ret = vecIndex->AddIndex(vec->GetVector(i), 1,
                                           vec->Dimension(), metaset, true);
        if (ErrorCode::Success != ret)
            std::cerr << "Error AddIndex(" << (int)(ret) << ") for vector " << i
                      << std::endl;
    }
    auto t2 = std::chrono::high_resolution_clock::now();
    std::cout << "Add time: "
...
template <typename T>
void PerfBuild(IndexAlgoType algo, std::string distCalcMethod,
               std::shared_ptr<VectorSet>& vec,
               std::shared_ptr<MetadataSet>& meta,
               std::shared_ptr<VectorSet>& queryset, int k,
               std::shared_ptr<VectorSet>& truth, std::string out) {
...
template <typename T>
void GenerateData(std::shared_ptr<VectorSet>& vecset,
                  std::shared_ptr<MetadataSet>& metaset,
                  std::shared_ptr<VectorSet>& queryset,
                  std::shared_ptr<VectorSet>& truth, std::string distCalcMethod,
                  int k) {
    SizeType n = 2000, q = 2000;
    DimensionType m = 128;

    if (fileexists("perftest_vector.bin") && fileexists("perftest_meta.bin") &&
        fileexists("perftest_metaidx.bin") &&
        fileexists("perftest_query.bin")) {
        std::shared_ptr<Helper::ReaderOptions> options(
            new Helper::ReaderOptions(GetEnumValueType<T>(), m,
                                      VectorFileType::DEFAULT));
        auto vectorReader = Helper::VectorSetReader::CreateInstance(options);
        if (ErrorCode::Success !=
            vectorReader->LoadFile("perftest_vector.bin")) {
        ...
        metaset.reset(
            new MemMetadataSet("perftest_meta.bin", "perftest_metaidx.bin",
                               vecset->Count() * 2, vecset->Count() * 2, 10));

        if (ErrorCode::Success !=
            vectorReader->LoadFile("perftest_query.bin")) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
        ...
        ByteArray metaoffset =
            ByteArray::Alloc((n + 1) * sizeof(std::uint64_t));
        ...
        metaset.reset(
            new MemMetadataSet(meta, metaoffset, n, n * 2, n * 2, 10));
    ...
    if (fileexists(("perftest_truth." + distCalcMethod).c_str())) {
        ...
        auto vectorReader = Helper::VectorSetReader::CreateInstance(options);
        if (ErrorCode::Success !=
            vectorReader->LoadFile("perftest_truth." + distCalcMethod)) {
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
        ...
        if (distMethod == DistCalcMethod::Cosine) {
            ...
            COMMON::Utils::BatchNormalize((T*)(vecset->GetData()),
                                          vecset->Count(), vecset->Dimension(),
        ...
        for (SizeType i = 0; i < queryset->Count(); ++i) {
            ...
            for (SizeType j = 0; j < vecset->Count(); j++) {
                float dist = COMMON::DistanceUtils::ComputeDistance(
                    res.GetTarget(), reinterpret_cast<T*>(vecset->GetVector(j)),
                    queryset->Dimension(), distMethod);
                res.AddPoint(j, dist);
            }
            res.SortResult();
...
void PTest(IndexAlgoType algo, std::string distCalcMethod) {
    ...
    PerfAdd<T>(algo, distCalcMethod, vecset, metaset, queryset, 10, truth,
               "testindices");
    PerfBuild<T>(algo, distCalcMethod, vecset, metaset, queryset, 10, truth,
                 "testindices");
    std::shared_ptr<VectorIndex> vecIndex;
    BOOST_CHECK(ErrorCode::Success ==
                VectorIndex::LoadIndex("testindices", vecIndex));
    BOOST_CHECK(nullptr != vecIndex);
