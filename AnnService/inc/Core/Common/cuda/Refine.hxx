...
#define MAX_CHECK_COUNT \
    32  // Max number of neighbors checked during each step of refinement.

#define LISTCAP \
    2048  // Maximum size of buffer for each threadblock during refinemnt
#define LISTSIZE \
    1024  // Maximum size of nearest neighbors stored during refinement
...
template<typename T>
void getCandidates(SPTAG::VectorIndex* index, int numVectors, int
candidatesPerVector, int* candidates) {

#pragma omp parallel for schedule(dynamic)
  for (SizeType i = 0; i < numVectors; i++)
  {
    SPTAG::COMMON::QueryResultSet<T> query((const T*)index->GetSample(i),
candidatesPerVector); index->SearchTree(query); for (SPTAG::DimensionType j = 0;
j < candidatesPerVector; j++) { candidates[i*candidatesPerVector+j] =
query.GetResult(j)->VID;
     }
  }
}
...
__device__ void loadRegisters( ListElt<SUMTYPE>* regs,  ListElt<SUMTYPE>*
listMem, int* listSize) { for(int i=0; i<NUM_REGS; i++) { if(i*NUM_THREADS +
threadIdx.x < *listSize) { regs[i] = listMem[i*NUM_THREADS + threadIdx.x];
    }
    else {
      regs[i].id = INFTY<int>();
...
template<typename SUMTYPE, int NUM_REGS, int NUM_THREADS>
__device__ void storeRegisters( ListElt<SUMTYPE>* regs,  ListElt<SUMTYPE>*
listMem, int* listSize) { for(int i=0; i<NUM_REGS; i++) { if(i*NUM_THREADS +
threadIdx.x < *listSize) { listMem[i*NUM_THREADS + threadIdx.x] = regs[i];
    }
    else {
...
__device__ void sortListById( ListElt<SUMTYPE>* listMem, int* listSize, void*
temp_storage) {

   ListElt<SUMTYPE> sortMem[LISTCAP/NUM_THREADS];
  int sortKeys[LISTCAP/NUM_THREADS];

  loadRegisters<SUMTYPE,LISTCAP/NUM_THREADS, NUM_THREADS>(sortMem, listMem,
listSize);
  ...
  __syncthreads();
  typedef cub::BlockRadixSort<int, NUM_THREADS, LISTCAP/NUM_THREADS,
ListElt<SUMTYPE>> BlockRadixSort; BlockRadixSort(*(static_cast<typename
BlockRadixSort::TempStorage*>(temp_storage))).SortBlockedToStriped(sortKeys,
sortMem);

  __syncthreads();


  storeRegisters<SUMTYPE,LISTCAP/NUM_THREADS, NUM_THREADS>(sortMem, listMem,
listSize);
  __syncthreads();
...
template<typename T, typename SUMTYPE, int MAX_DIM, int NUM_THREADS>
__device__ void sortListByDist( ListElt<SUMTYPE>* listMem, int* listSize, void*
temp_storage) {
...
listSize);
  ...
  typedef cub::BlockRadixSort<SUMTYPE, NUM_THREADS, LISTCAP/NUM_THREADS,
ListElt<SUMTYPE>> BlockRadixSort; BlockRadixSort(*(static_cast<typename
BlockRadixSort::TempStorage*>(temp_storage))).SortBlockedToStriped(sortKeys,
sortMem);
  __syncthreads();
...
template<typename T, typename SUMTYPE, int MAX_DIM, int NUM_THREADS>
__device__ void removeDuplicatesAndCompact( ListElt<SUMTYPE>* listMem, int*
listSize, void *temp_storage, int* borderVals, int src) {

  ListElt<SUMTYPE> sortMem[LISTCAP/NUM_THREADS];
  int sortKeys[LISTCAP/NUM_THREADS];

  for(int i=threadIdx.x; i<*listSize-1 && listMem[i].id != INFTY<int>();
i+=NUM_THREADS) { for(int j=i+1; j < *listSize && listMem[i].id ==
listMem[j].id; j++) { listMem[i].checkedFlag = (listMem[i].checkedFlag ||
listMem[j].checkedFlag);
    }
  }
  ...
  if(threadIdx.x==0) {
    ...
    sortKeys[0] = (sortMem[0].id != listMem[threadIdx.x*(LISTCAP/NUM_THREADS) -
1].id);
  }
  ...
  BlockScan(*(static_cast<typename
BlockScan::TempStorage*>(temp_storage))).InclusiveSum(sortKeys, sortKeys);
...
template<typename T, typename SUMTYPE, int MAX_DIM, int NUM_THREADS>
__device__ void checkClosestNeighbors(Point<T,SUMTYPE,MAX_DIM>* d_points, int
src, int* d_graph,  ListElt<SUMTYPE>* listMem, int* listSize, int KVAL, int
metric) {
...
template<typename T, typename SUMTYPE, int MAX_DIM, int NUM_THREADS>
__device__ void shrinkListRNG_sequential(Point<T,SUMTYPE,MAX_DIM>* d_points,
int* d_graph, int src, ListElt<SUMTYPE>* listMem, int listSize, int KVAL, int
metric) {
...
template<typename T, typename SUMTYPE, int MAX_DIM, int NUM_THREADS>
__device__ void shrinkListRNG(Point<T,SUMTYPE,MAX_DIM>* d_points, int* d_graph,
int src, ListElt<SUMTYPE>* listMem, int listSize, int KVAL, int metric) {
  ...
  for(read_idx=0; write_idx < KVAL && read_idx < listSize; read_idx++) {
    ...
    if(metric == 0) {
      for(int j=threadIdx.x; j<write_idx && good; j+=NUM_THREADS) {
        if(listMem[read_idx].dist >=
d_points[listMem[read_idx].id].l2(&d_points[listMem[j].id])) { good=false;
        }
      }
    }
    else {
      for(int j=threadIdx.x; j<write_idx && good; j+=NUM_THREADS) {
        if(listMem[read_idx].dist >=
d_points[listMem[read_idx].id].cosine(&d_points[listMem[j].id])) { good=false;
        }
      }
    }
    __syncthreads();
...
template<typename T, typename SUMTYPE, int MAX_DIM, int NUM_THREADS>
__global__ void refineBatch_kernel(Point<T,SUMTYPE,MAX_DIM>* d_points, int
batchSize, int batchOffset, int* d_graph, int* candidates, ListElt<SUMTYPE>*
listMemAll, int candidatesPerVector, int KVAL, int refineDepth, int metric) {
  ...
  typedef cub::BlockRadixSort<SUMTYPE, NUM_THREADS, LISTCAP/NUM_THREADS,
ListElt<SUMTYPE>> BlockRadixSortT;
...
//    }
    ...
    // Place all candidates into the list of neighbors to check during
refinement for(int i=threadIdx.x; i<candidatesPerVector; i+=NUM_THREADS) {
    ...
    sortListByDist<T,SUMTYPE,MAX_DIM,NUM_THREADS>(listMem, &listSize,
&temp_storage);
...
// Perform DFS refinement up to refineDepth distance
    for(int iter=0; iter < refineDepth; iter++) {
      ...
      __syncthreads();
      checkClosestNeighbors<T,SUMTYPE,MAX_DIM,NUM_THREADS>(d_points, src,
d_graph, listMem, &listSize, KVAL, metric);
      sortListById<T,SUMTYPE,MAX_DIM,NUM_THREADS>(listMem, &listSize,
&temp_storage);
      removeDuplicatesAndCompact<T,SUMTYPE,MAX_DIM,NUM_THREADS>(listMem,
&listSize, &temp_storage, borderVals, src);
...
//     Prune nearest RNG vectors and write them to d_graph
    shrinkListRNG<T,SUMTYPE,MAX_DIM,NUM_THREADS>(d_points, d_graph, src,
listMem, listSize, KVAL, metric);
    __syncthreads();
  }
...
void refineGraphGPU(SPTAG::VectorIndex* index, Point<T,SUMTYPE,MAX_DIM>*
d_points, int* d_graph, int dataSize, int KVAL, int candidatesPerVector, int
refineDepth, int refines, int metric) {
  ...
  cudaMemcpy(d_candidates, candidates, dataSize*candidatesPerVector*sizeof(int),
cudaMemcpyHostToDevice);

  auto t2 = std::chrono::high_resolution_clock::now();
  SPTAGLIB_LOG(SPTAG::Helper::LogLevel::LL_Info, "find candidates time (ms):
...
// Kernel that refines a batch of points' KNN neighbors into RNG neighbors
      refineBatch_kernel<T,SUMTYPE,MAX_DIM,
REFINE_THREADS><<<REFINE_BLOCKS,REFINE_THREADS>>>(d_points, batch_size,
i*batch_size, d_graph, d_candidates, listMem, candidatesPerVector, KVAL,
refineDepth, metric); cudaError_t status = cudaDeviceSynchronize(); if(status !=
cudaSuccess) { SPTAGLIB_LOG(SPTAG::Helper::LogLevel::LL_Error, "Refine error
code:%d\n", status);
      }
    }
